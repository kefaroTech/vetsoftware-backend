...
- **Todo `@RequestBody` cuyo tipo declare restricciones lleva `@Valid`** (`CUERPO_CON_RESTRICCIONES_SE_VALIDA`, dura, incidencia #135). Sin `@Valid` delante del parámetro, el binder de Spring **no dispara el validador**: el `@NotBlank`, el `@Size` o el `@Email` que el DTO declara están escritos y no se evalúan nunca. Lo que hace al defecto invisible es que **las tres señales que mira un humano dicen que la validación existe** — el compilador calla, la anotación se lee perfecta en el diff, y el `api/openapi.json` sigue anunciando el `maxLength` al front, porque springdoc lo deriva del `@Size` con `@Valid` o sin él. Solo se nota en ejecución, y disfrazado de otro problema.
  ...
  - **Mira el tipo del parámetro, nunca su nombre.** Un `XxxRequest` sin una sola restricción no tiene nada que validar y queda fuera a propósito: de los tres `@RequestBody` sin `@Valid` que quedaban, dos son legítimos —el `String` crudo del webhook de la DIAN y el `RefreshTokenRequest`, que dejó de exigir su campo por escrito— y la regla no los toca. Por eso **nace dura y en cero**, sin `freeze(...)`.

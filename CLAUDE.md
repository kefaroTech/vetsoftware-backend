...
Ojo con la palabra «duras»: quince de las diecinueve lo son porque el código ya las cumple —las tres de BE-26 entre ellas, escritas justo después de que la campaña versionara las entidades, que es el criterio normal del repo para que una regla nazca dura; la de #53 se escribió en paralelo a la campaña que movía las ~70 consultas y quedó en verde el mismo día en que esa campaña cerró, y la de #135 nació en cero porque su única violación —una palabra— se arregló en el mismo cambio—. Las **cuatro de BE-COV nacieron duras con la deuda abierta**, a propósito y contra la costumbre del repo: la campaña que las motivó estaba corrigiéndose en el mismo momento en que se escribieron, así que congelarlas habría fotografiado un estado transitorio y metido al store deuda que iba a desaparecer sola —exactamente la trampa que este documento advierte sobre `allowStoreCreation`—. Mientras el contador no esté a cero, la salida es **corregir**, nunca envolverlas en `freeze(...)`.

**Son dos clases porque miran universos distintos.** `HexagonalArchitectureTest` declara `ImportOption.DoNotIncludeTests` — sus veintidós reglas hablan de código de producción, incluidas las cuatro de BE-COV, las tres de BE-26, la de #53 y la de #135: los puertos, el SQL de los adaptadores, los casos de uso, los controllers y las entidades JPA son todos `src/main`, así que ninguna necesita ver el árbol de test. Las **tres** que necesitan ver `src/main` y `src/test` a la vez viven en `PiramideDeTestsTest`, con su propio `@AnalyzeClasses` sin esa opción: las dos de BE-10 (**cada adaptador con su rodaja**) y `DOBLE_DE_TEST_NO_ESCANEABLE`.
...
- **Toda `@Query` de `UPDATE` sobre una tabla versionada mueve `version` en su `SET`** (`UPDATE_MASIVO_MUEVE_LA_VERSION`, dura, incidencia #53). Es la grieta que BE-26 dejó abierta sin saberlo: **`@Version` protege un solo camino**, el ciclo leer-modificar-guardar de una entidad gestionada, donde Hibernate compara la versión en el `WHERE` y la incrementa en el `SET`. Una `@Query` de `UPDATE` **no pasa por ahí** —va directa a la base, ni comprueba ni incrementa nada— y deja la fila modificada con su versión intacta, así que el `save` concurrente que llegue con la versión vieja **casa igual** y pisa el cambio. Sin excepción, sin log y sin 409: el mismo fallo silencioso que las tres reglas de BE-26 creían haber cerrado.
  - **Ya cobró dos veces**, y por eso la regla no es preventiva sino tardía: la revocación de sesión que se deshacía sola (#54, arreglado) y la suspensión de programaciones de medicación que se perdía. En los dos casos el `UPDATE` masivo hizo su trabajo y un `save` que venía de una lectura anterior lo deshizo. La campaña que acompaña a la regla añadió `version = version + 1` al `SET` de las ~70 consultas afectadas; la regla es lo que impide que entre la 71.
  - **La versión va en el `SET`, nunca en el `WHERE`** —al revés que en `BORRADO_LOGICO_RESPETA_LA_VERSION`, y la diferencia es toda la intención—. Aquel es el SQL *de* Hibernate para una entidad concreta que alguien acaba de leer, y ahí la versión es el candado; este es un `UPDATE` de conjunto que nadie leyó antes, y condicionarlo por versión solo conseguiría que actualizara cero filas y que el servicio lo interpretara como «no existe». La regla denuncia **las dos** mitades: la que falta en el `SET` y la que sobra en el `WHERE`.
  - **El mapa tabla → ¿versionada? se levanta del censo de `@Entity`**, `@Table(name = …)` para el SQL nativo y nombre simple para el JPQL, **nunca de una lista literal**: BE-26 ya enseñó lo que le pasa a una lista que nadie mantiene, y aquí bastaría versionar una entidad para que su tabla saliera de la vigilancia en silencio. Cubre las dos formas de `UPDATE` con la misma consulta al censo; los `DELETE` quedan fuera a propósito —se llevan la fila, no hay versión que proteger—.
  - **Y trocea el `SET` en vez de buscar una subcadena.** `auth_version = auth_version + 1` es invalidación de sesión y no bloqueo optimista —misma advertencia que en BE-26—, y el alias decide a qué tabla se escribe: `UPDATE role_permissions rp JOIN roles r … SET rp.enabled = true` toca la tabla puente exenta y no los `roles` versionados con los que hace `JOIN`. Sin resolver el alias serían cuatro falsos positivos y la regla se habría muerto de ruido el primer día.
  - **Hoy no hay ninguna `@Query` perdonada, y por eso no hay lista de exenciones.** No es un descuido: la única escritura masiva exenta del repositorio no lleva anotación. Si hiciera falta perdonar una, el código sería `E6_YA_PROTEGIDO` del vocabulario de BE-26 y la exención iría escrita con el mismo formato que `ENTIDADES_EXENTAS_DE_VERSION`.
  - **Limitación conocida, con nombre y apellido: el SQL crudo por `JdbcTemplate`.** La regla escanea `@Query` y solo `@Query`. `JdbcDianJobLeasePort.leaseByDianStatus` hace `UPDATE electronic_documents SET dian_leased_until = ?` con `jdbcTemplate.update(...)`, sin anotación, sobre una tabla que **sí** va versionada —y es una exención legítima: las filas vienen de un `SELECT … FOR UPDATE SKIP LOCKED` en la misma transacción, ya serializadas por bloqueo pesimista—. **Por contenido no se puede cubrir**, y no por esfuerzo sino por el modelo: ArchUnit expone el *valor* de una anotación (de ahí que `@Query` sea automatizable) pero **no los argumentos constantes de un punto de llamada**; el literal vive en el pool de constantes del llamador y sacarlo a un `static final String` tampoco ayuda. Lo que sí sale —**propuesto, no implementado**— es un tripwire estructural al estilo BE-26: nadie llama a `JdbcTemplate.update`/`batchUpdate`/`execute` salvo las clases de una lista con su código y su motivo, hoy con exactamente **dos** entradas (`JdbcDianJobLeasePort` y `TokenCleanupRepository`, que solo borra), y con su gemela anti-podredumbre. Para el chequeo *de contenido* la herramienta no es ArchUnit sino un `RegexpMultiline` de Checkstyle, que sí ve el literal esté donde esté.

- **Todo `@RequestBody` cuyo tipo declare restricciones lleva `@Valid`** (`CUERPO_CON_RESTRICCIONES_SE_VALIDA`, dura, incidencia #135). Sin `@Valid` delante del parámetro, el binder de Spring **no dispara el validador**: el `@NotBlank`, el `@Size` o el `@Email` que el DTO declara están escritos y no se evalúan nunca. Lo que hace al defecto invisible es que **las tres señales que mira un humano dicen que la validación existe** — el compilador calla, la anotación se lee perfecta en el diff, y el `api/openapi.json` sigue anunciando el `maxLength` al front, porque springdoc lo deriva del `@Size` con `@Valid` o sin él. Solo se nota en ejecución, y disfrazado de otro problema.
  - **El caso real, y por qué el daño no se ve en la base de datos.** `PATCH /appointments/{id}/cancel` recibía `CancelAppointmentRequest(@Size(max = 300) String reason)` sin `@Valid`. El dominio vuelve a medir la longitud y la columna es de 300, así que no entraba basura; lo que se rompía era **la forma del error**. En vez del error de campo sobre `reason` que el front sabe pintar bajo el textarea, salía una excepción de dominio con otro `errorCode` y otra forma, y quien cancelaba la cita leía un mensaje genérico sin saber qué corregir. Que era omisión y no decisión lo demostraba el endpoint de la línea de arriba, `changeStatus`, que sí lo llevaba.
  - **Mira el tipo del parámetro, nunca su nombre.** Un `XxxRequest` sin una sola restricción no tiene nada que validar y queda fuera a propósito: de los tres `@RequestBody` sin `@Valid` que quedaban, uno es legítimo —el `String` crudo del webhook de la DIAN— y la regla no lo toca (el `RefreshTokenRequest` también lo era mientras su cuerpo fue opcional; desde que exige `type` con `@NotNull`, lleva `@Valid` como cualquier otro). Por eso **nace dura y en cero**, sin `freeze(...)`.
  - **Baja a los tipos anidados** —los campos del cuerpo y los argumentos genéricos de sus colecciones, que es donde vive el tipo interesante de una `List<LineaRequest>`—: un `@NotBlank` en una línea de detalle tampoco se evalúa si el cuerpo que la transporta no lleva `@Valid`.
  - **`@Valid` sobre un `@RequestBody(required = false)` es seguro**: `RequestResponseBodyMethodProcessor` guarda la validación tras un `if (arg != null)`, así que un cuerpo ausente sigue resolviendo a `null` y la cancelación sin cuerpo —caso legítimo y en uso— no cambia.
...
2. **Crea escribiendo el cuerpo en un fichero.** Las comillas de PowerShell destrozan los
   ...
   gh issue create --repo kefaroTech/vetsoftware-backend --title "<el problema, en una frase>" --body-file cuerpo.md
   ```
...
- ❌ Declarar un `@RequestBody` sin `@Valid` cuando el DTO trae restricciones (`@Size`, `@NotBlank`, `@Email`…) — el binder no dispara el validador y la restricción no se evalúa nunca, aunque el OpenAPI la siga anunciando al front (`CUERPO_CON_RESTRICCIONES_SE_VALIDA`, #135)
...
- ❌ Escribir una `@Query` de `UPDATE` sobre una tabla versionada sin `version = version + 1` en el `SET` — `@Version` no cubre el UPDATE masivo y un `save` concurrente pisa el cambio sin ruido (`UPDATE_MASIVO_MUEVE_LA_VERSION`, #53). Y tampoco lo pongas en el `WHERE`: ahí no protege, actualiza cero filas

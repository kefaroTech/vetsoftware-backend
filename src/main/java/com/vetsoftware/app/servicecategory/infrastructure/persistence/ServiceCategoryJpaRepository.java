...
public interface ServiceCategoryJpaRepository
    extends JpaRepository<ServiceCategoryJpaEntity, Long> {
  ...
  @org.springframework.data.jpa.repository.Query(
      value =
          "UPDATE service_categories SET enabled = true WHERE id = :id AND company_id = :companyId",
      nativeQuery = true)
  int reactivate(
  ...
  // @SQLRestriction("enabled = true") aplica: solo cuenta categorías ACTIVAS (un name desactivado
  // se reusa).

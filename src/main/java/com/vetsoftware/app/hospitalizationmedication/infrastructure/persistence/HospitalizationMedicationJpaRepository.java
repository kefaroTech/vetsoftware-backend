...
public interface HospitalizationMedicationJpaRepository
     ...
     * por id a secas resucitaba la orden borrada de cualquier tenant para quien
     ...
     *
     * <p>
     * La empresa no cuelga de {@code hospitalization_medications}: cuelga de la
    ...
    @org.springframework.data.jpa.repository.Query(value = """
            UPDATE hospitalization_medications m
            SET m.enabled = true, m.version = m.version + 1
            WHERE m.id = :id
              AND EXISTS (SELECT 1
                          FROM hospitalizations h
                          WHERE h.id = m.hospitalization_id

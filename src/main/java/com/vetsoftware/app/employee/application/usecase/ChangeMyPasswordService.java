...
/**
 * Cambio de la propia contraseña (primer login forzado). Hashea la nueva contraseña, la asigna y
 * limpia {@code mustChangePassword}. No toca {@code authVersion}: la sesión en curso sigue válida y
 * el empleado pasa directo al panel sin re-login.
 */
...
public class ChangeMyPasswordService implements ChangeMyPasswordUseCase {
  ...
  public boolean execute(ChangeMyPasswordCommand command) {
    ...
    // Antes de cambiar: si estaba obligado a cambiarla, este cambio ES la aceptación de la
    // invitación.

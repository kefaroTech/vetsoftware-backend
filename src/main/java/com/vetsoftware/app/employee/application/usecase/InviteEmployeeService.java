...
/**
 * Alta de staff por un admin, en una sola transacción: crea el empleado (verificado, con contraseña temporal
...
public class InviteEmployeeService implements InviteEmployeeUseCase {
    ...
    public EmployeeDto execute(InviteEmployeeCommand command) {
        // Regla: un empleado no puede crearse sin sede (quedaría bloqueado de citas/cuentas/POS/reportes). Se valida
        // antes de tocar nada; el detalle (sedes ∈ empresa) lo revalida el assigner en la misma transacción.
        ...
        // Staff invitado: verificado (lo crea el admin) y obligado a cambiar la contraseña en el primer login.
        Employee employee = Employee.create(command.employeeCode(), hashed, command.name(), command.email(),
            company, true, true);
        // IDENTITY → INSERT inmediato: si el código ya existe, falla aquí (antes de asignar roles / enviar correo).
        ...
        // Sedes en la misma transacción: si algo falla (sede inexistente/ajena), revierte el alta completa.

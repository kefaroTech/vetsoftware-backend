...
/**
 * Reenvía la invitación a un empleado en estado INVITED: le asigna una nueva
 * contraseña provisional (la anterior ya está ofuscada en hash, por eso el
 * admin escribe una nueva), vuelve a exigir el cambio en el primer login y
 * reenvía el correo con sus datos de acceso. El correo es async/best-effort (no
 * bloquea).
 */
...
public class ResendInvitationService implements ResendInvitationUseCase {
    ...
    public ResendInvitationService(EmployeeRepository repository,
            EmployeeRolesQueryPort rolesQueryPort, PasswordHasher passwordHasher,
    ...
    public EmployeeDto execute(ResendInvitationCommand command) {
        ...
        List<RoleSnapshot> roles = rolesQueryPort.findRolesByEmployeeIds(List.of(saved.getId()))

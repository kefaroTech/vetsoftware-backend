...
/**
 * Reenvía la invitación a un empleado en estado INVITED: le asigna una nueva contraseña provisional (la
...
public class ResendInvitationService implements ResendInvitationUseCase {
    ...
    public ResendInvitationService(EmployeeRepository repository, EmployeeRolesQueryPort rolesQueryPort,
    ...
    public EmployeeDto execute(ResendInvitationCommand command) {
        ...
        // El empleado debe pertenecer a la company del solicitante (cross-tenant → 404 sin filtrar datos).
        if (!employee.getCompany().id().equals(command.companyId()))
            throw new EmployeeNotFoundException(command.employeeId());

        // Solo se reenvía a quien sigue invitado (nunca inició sesión). Un empleado ya activo no se re-invita.
        if (employee.getStatus() != EmployeeStatus.INVITED)
            throw new IllegalArgumentException("Solo se puede reenviar la invitación a empleados en estado invitado");

        employee.reinvite(passwordHasher.hash(command.password()));
        Employee saved = repository.save(employee);

        List<RoleSnapshot> roles = rolesQueryPort
        ...
        invitationEmailSender.send(saved.getEmail(), saved.getName(), saved.getCompany().name(),
            saved.getEmployeeCode(), command.password(), roleNames);

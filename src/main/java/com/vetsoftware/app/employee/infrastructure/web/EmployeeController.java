...
public class EmployeeController {
    ...
    public EmployeeController(InviteEmployeeUseCase inviteUseCase,
            ...
            ChangeMyPasswordUseCase changeMyPasswordUseCase, UpdateEmployeeUseCase updateUseCase,
            FindEmployeeUseCase findUseCase, ListEmployeesUseCase listUseCase,
            ListEmployeesByCompanyUseCase listByCompanyUseCase,
            SearchEmployeesUseCase searchUseCase, DeleteEmployeeUseCase deleteUseCase,
            ReactivateEmployeeUseCase reactivateUseCase,
            SuggestEmployeeCodeUseCase suggestCodeUseCase,
            CheckEmployeeCodeAvailabilityUseCase checkCodeAvailabilityUseCase, Authz authz,
    ...
    public EmployeeResponse create(@Valid @RequestBody CreateEmployeeRequest request) {
        // Un no-admin solo puede asignar al nuevo empleado sedes que él mismo tiene
        // (admin: cualquiera
        // de la empresa).
        authz.requireAssignableBranches(request.branchIds());
        EmployeeDto dto = inviteUseCase.execute(new InviteEmployeeCommand(request.employeeCode(),
                request.password(), request.name(), request.email(), authz.currentCompanyId(),
                request.roleIds(), request.branchIds()));
        auditLogger.employeeInvited(dto.id(), dto.employeeCode(), dto.company().id());
        return toResponse(dto);
    }

    /**
     * Reenvía la invitación a un empleado invitado con una nueva contraseña
     * provisional.
     */
    ...
    /**
     * Cambio de la propia contraseña (primer login forzado). El empleado sale del
    ...
    /**
     * Listado paginado + búsqueda server-side (nombre/código/correo) de los
    ...
    /**
     * Sugiere un código de empleado disponible a partir del nombre (prefijo =
    ...
    private EmployeeResponse toResponse(EmployeeDto dto) {
        ...
        return new EmployeeResponse(dto.id(), dto.employeeCode(), dto.name(), dto.email(),
                new CompanySummary(c.id(), c.name(), c.identifier()), roles, branches,
                dto.createdDate(), dto.enabled(), dto.mustChangePassword(), dto.status());
    }
}

...
public class JpaEmployeeRepository implements EmployeeRepository {
  ...
  public PageResult<Employee> search(SearchEmployeesCommand command) {
    String q =
        command.query() == null || command.query().isBlank()
            ? null
            : "%" + command.query().trim() + "%";
    ...
    Page<EmployeeJpaEntity> page =
        jpaRepository.searchByCompanyIncludingDisabled(command.companyId(), q, pageRequest);
  ...
  public void delete(Long id) {
    // Soft-delete vía UPDATE nativo (equivalente al @SQLDelete). No usamos deleteById para no
    // disparar
    // el flush de entidades: si hay employee_roles gestionados del empleado en la sesión,
    // provocaría un

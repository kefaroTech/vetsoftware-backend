...
/**
 * Carga clínica + paciente + propietario para la fórmula. Cruce cross-feature permitido: solo
 * importa {@code AnimalJpaRepository}/{@code WeightRecordJpaRepository} (persistencia).
 */
@Component
public class JpaPrescriptionReportQueryPort implements PrescriptionReportQueryPort {
  ...
  public JpaPrescriptionReportQueryPort(
  ...
  public Optional<PrescriptionSignalment> loadByAnimal(Long animalId, Long companyId) {
    return animalJpaRepository
        .findByIdAndCompany_Id(animalId, companyId)
        .map(a -> toSignalment(a, companyId));
  }

  private PrescriptionSignalment toSignalment(AnimalJpaEntity a, Long companyId) {
    LatestWeightProjection weight =
        weightRecordJpaRepository.findLatestByAnimalId(a.getId(), companyId).orElse(null);

...
public interface SupplierJpaRepository extends JpaRepository<SupplierJpaEntity, Long>,
    ...
    // única vía para listar los proveedores PAUSADOS (enabled=false) y poder reactivarlos desde la UI.
    // Las asociaciones se hidratan perezosamente dentro de la transacción de lectura del caso de uso.
    @org.springframework.data.jpa.repository.Query(
        value = "SELECT * FROM suppliers WHERE company_id = :companyId AND enabled = false ORDER BY name",
        nativeQuery = true)
    ...
    // @SQLRestriction("enabled = true") aplica: solo cuenta proveedores ACTIVOS (un name desactivado se reusa).

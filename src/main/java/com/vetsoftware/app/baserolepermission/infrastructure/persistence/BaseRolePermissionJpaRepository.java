...
public interface BaseRolePermissionJpaRepository
        extends
            JpaRepository<BaseRolePermissionJpaEntity, Long> {
    ...
    @org.springframework.data.jpa.repository.Query(value = "SELECT id FROM base_role_permissions WHERE base_role_id = :baseRoleId AND"
            + " base_permission_id = :basePermissionId AND enabled = false LIMIT 1", nativeQuery = true)

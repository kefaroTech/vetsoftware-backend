...
/**
 * Correo de verificación de cuenta (auto-registro Opción B) enviado con una <b>plantilla de
 * Resend</b> ({@code template: { id, variables }}). El enlace apunta a la página del front ({@code
 * verification-base-url}) con el token plano; el front llama a {@code POST /register/verify}. Si el
 * envío está deshabilitado (dev), deja el enlace + código en el log. El envío es asíncrono y no
 * bloquea el registro (si Resend falla, se registra un warning y el registro continúa).
 *

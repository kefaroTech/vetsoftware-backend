...
/**
 * Adapter de reCAPTCHA (Google). Verifica el token server-side contra {@code siteverify}. Soporta
 * v2 (solo {@code success}) y v3 (ademas {@code score}, comparado contra {@code min-score}).
 *
 * <p>Se controla por configuracion:
 *
 * <ul>
 *   <li>{@code vetsoftware.recaptcha.enabled=false} (por defecto) → no-op, util en dev/local y
 *       tests.
 *   <li>{@code vetsoftware.recaptcha.secret} → llave secreta del proveedor (por env, sin
 *       versionar).
 * </ul>
 */
@Component
public class GoogleRecaptchaVerifier implements CaptchaVerifier {
  ...
  public GoogleRecaptchaVerifier(
      ...
      @Value("${vetsoftware.recaptcha.verify-url:https://www.google.com/recaptcha/api/siteverify}")
          String verifyUrl,
  ...
  private record SiteVerifyResponse(
      @JsonProperty("success") boolean success, @JsonProperty("score") Double score) {}
}

...
public class RegisterUserService implements RegisterUserUseCase {
  ...
  public RegisterUserService(
      ...
      @Value("${vetsoftware.registration.verification-token-ttl-hours:24}")
  ...
  public RegistrationDto execute(RegisterUserCommand command) {
    // 1) Anti-abuso: captcha antes de tocar la BD (no-op si el captcha esta deshabilitado por
    // config).
    ...
    // Identidad fiscal del emisor: toda venta (incluido el tiquete POS) la requiere. Tipo NIT,
    // número =
    // identificador de la empresa, DV autocalculado, razón social = nombre; régimen y correo del
    // signup.
    ...
    // Hereda ciudad/dirección/teléfono del registro, igual que el backfill de las empresas
    // existentes.
    ...
    // El dueño ELIGE su código de acceso (Opción A). Revalidamos aquí que siga libre (defensa ante
    // la
    // El usuario de acceso ES el correo del administrador (un email = una veterinaria). Como
    // employee_code
    // es único global, si el correo ya está registrado se rechaza aquí. El dueño se crea SIN
    // verificar (Opción B).
    ...
    // Se pasa la contraseña CRUDA: CreateEmployeeService la hashea una sola vez (igual que el alta
    // de
    // staff por el admin). No pre-hashear aquí, o quedaría doble-hasheada y el login nunca haría
    // match.
    ...
    for (BaseRoleData baseRole : baseRoleProvider.findAll()) {
      ...
      rolePermissionInitializationPort.initializeForRole(
          role.id(), company.id(), baseRole.id(), membershipId);
      if (baseRole.mandatory()) {
        employeeRoleAssigner.assign(employee.id(), role.id());
      }
    }

    // 2) Token de verificación de un solo uso: se guarda el HASH, se envía el valor plano por
    // correo.
    ...
    verificationEmailSender.send(
        command.employeeEmail(), command.employeeName(), command.companyName(), rawToken);

    return new RegistrationDto(
        company.id(), employee.id(), command.employeeEmail(), STATUS_PENDING_VERIFICATION);
  }
}

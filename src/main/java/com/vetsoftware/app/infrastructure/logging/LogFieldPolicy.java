 ...
 *       token embebido (nombre comercial, ruta HTTP, User-Agent).
 * </ul>
 *
 * <p>El único cruce con {@link MdcKeys} es intencional: esta clase declara la política de salida de
 * esas mismas claves más las de los eventos {@code AUDIT} ({@link
 * com.vetsoftware.app.infrastructure.audit.AuditLogger}).
 *
 * @see LogRedactor
...
public final class LogFieldPolicy {
   ...
   * Micrometer Tracing. {@code company.identifier} es el NIT de la <em>empresa cliente</em> (dato
   * mercantil público del tenant, auditado a propósito), no el documento de una persona. {@code
   * actor.identifier} y {@code employee.identifier} son códigos de acceso de empleado, que {@code
   * AuditLogger} documenta explícitamente como no secretos.
   */
  private static final Set<String> VERBATIM =
      Set.of(
  ...
  private static final Set<String> SCANNED =
      Set.of(MdcKeys.HTTP_PATH, MdcKeys.USER_AGENT, "company.name");

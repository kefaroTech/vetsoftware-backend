...
/**
 * Vista redactada de un evento de log. Delega todo en el evento original salvo
 * las cuatro superficies por las que puede escapar un dato sensible: mensaje,
 * MDC, pares clave-valor y la cadena de excepciones.
 *
 * <p>
 * Se sustituye el evento en lugar de intentar mutarlo porque
 * {@code LoggingEvent} es efectivamente inmutable en lo que importa, y porque
 * así el appender destino recibe exactamente lo que se va a emitir, sin poder
 * recomponer el mensaje desde los argumentos originales: {@link #getMessage()}
 * devuelve ya el texto formateado y redactado y {@link #getArgumentArray()}
 * devuelve {@code null}. Eso es deliberado — un encoder que reformatee obtiene
 * el mismo texto redactado, y no una versión limpia reconstruida a partir de
 * los argumentos.
 *
 * <p>
 * Redactar sobre el mensaje <em>ya formateado</em> (y no argumento a argumento)
 * es lo que permite detectar secretos que solo existen al unir plantilla y
 * argumento: en {@code
 * log.info("password={}", secret)} el argumento aislado es un texto anodino; el
 * mensaje formateado es {@code password=hunter2}.
 *
...
final class RedactedLoggingEvent implements ILoggingEvent {
    ...
    /**
     * Redacta el evento, devolviendo el <b>original</b> si no hubo nada que
     * cambiar. El caso normal es que no haya cambios, así que no se paga ninguna
     * copia.
     */
    static ILoggingEvent of(ILoggingEvent event) {
        ...
        boolean unchanged = Objects.equals(originalMessage, redactedMessage)
                && redactedMdc == originalMdc && redactedPairs == originalPairs
                && redactedThrowable == originalThrowable;
        if (unchanged) {
            return event;
        }
        return new RedactedLoggingEvent(event, redactedMessage, redactedMdc, redactedPairs,
                redactedThrowable);
    }

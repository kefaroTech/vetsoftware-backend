 ...
 *
 * <p>El mensaje de una excepción es un vector de fuga real y difícil de auditar: un {@code
 * DataIntegrityViolationException} arrastra el SQL con sus parámetros, y un error de cliente HTTP
 * puede arrastrar la cabecera {@code Authorization} que se envió.
 *
 * <p><b>Por qué se clona la excepción en vez de envolver el {@code IThrowableProxy}:</b> el
 * appender de OpenTelemetry solo reconoce la clase concreta {@link ThrowableProxy} y obtiene el
 * {@link Throwable} real de ella; un {@code IThrowableProxy} propio haría que OTel <em>descartara
 * la excepción por completo</em>, perdiendo tipo, mensaje y stacktrace en Loki. Así que se
 * construye un {@code Throwable} de verdad con el mensaje ya redactado y se envuelve en un {@code
 * ThrowableProxy} legítimo.
 *
 * <p><b>Fidelidad:</b> se conservan el stacktrace, la cadena de causas y las excepciones
 * suprimidas. El {@code toString()} sobrescrito devuelve el nombre de la clase original, así que la
 * consola y el atributo {@code exception.stacktrace} siguen mostrando el tipo real; solo el
 * atributo {@code exception.type} de OTel pasa a ser esta clase. Ese coste se paga
 * <b>únicamente</b> cuando la redacción cambió algo: si el mensaje estaba limpio, {@link
 * #redact(IThrowableProxy)} devuelve el proxy original sin tocarlo, que es el caso normal.
 *
 * @see LogRedactor
 */
final class RedactedThrowable extends Throwable {
  ...
  private RedactedThrowable(
      String originalType, String message, StackTraceElement[] stackTrace, Throwable cause) {

...
/**
 * Adaptador único de los contratos de telemetría comercial. Todos sus tags provienen de enums o
 * transformaciones cerradas y son validados además por {@link BusinessMetricCardinalityFilter}.
 */
@Component
public class MicrometerBusinessMetrics
    ...
    recorder.recordAfterCommit(
        () -> {
          ...
          salesOperations
              ...
              .increment();
          salesAmount
              .withTags("channel", channel.value(), "document.type", documentTypeValue)
    ...
    recorder.recordAfterCommit(
        () -> {
          ...
          dianTransmissions
              ...
              .increment();
          dianTransmissionDuration
              .withTags("result", result, "origin", origin.value())
    ...
    recorder.recordNow(
        () -> {
          dianTransmissions
              ...
              .increment();
          dianTransmissionDuration
              .withTags("result", "error", "origin", origin.value())
              .record(immutableDuration);
        });
  }
  ...
  public void movement(StockMovementType movementType, InventoryMetrics.Result result, int units) {
    Runnable action =
        () -> {
          ...
          inventoryMovements.withTags("movement.type", type, "result", result.value()).increment();
  ...
  public void closed(List<BigDecimal> differences) {
    List<BigDecimal> immutableDifferences =
        differences == null
            ? List.of()
    ...
    recorder.recordAfterCommit(
        () -> {
          boolean hasDifference =
              immutableDifferences.stream().anyMatch(value -> value.signum() != 0);
          cashSessions
          ...
          for (BigDecimal difference : immutableDifferences) {
            cashClosingDifference
                .withTags("direction", direction(difference))
  ...
  private static String dianResult(DianStatus status) {
    return switch (status) {
      ...
      case NO_ELECTRONICO ->
          throw new IllegalArgumentException("NO_ELECTRONICO no representa una transmisión DIAN");
    };
  }

 ...
 *
 * <p>donde {@code previous_hash(n)} es {@code chain_hash(n-1)} y el eslabón cero usa {@link
 * #GENESIS_HASH}. Encadenar así hace que suprimir, reordenar o alterar cualquier evento invalide
 * todos los eslabones posteriores, de modo que la manipulación es detectable aunque el atacante
 * tenga permisos de escritura sobre la tabla.
 *
...
public final class AuditChainHash {
  ...
  /**
   * Hash del payload tal como se serializó. Se calcula sobre los bytes UTF-8 exactos, por lo que la
   * columna debe conservarlos sin normalizar (ver migración 215: {@code LONGTEXT}, no {@code
   * JSON}).
   */

 ...
 *
 * <p>Escribe al logger {@code "AUDIT"} para consulta en Grafana y, en producción, persiste el mismo
 * hecho en el outbox que termina en S3 Object Lock mediante Firehose. Los campos del actor ({@code
 * actor.type} / {@code actor.companyId} / {@code actor.employeeId} / {@code actor.systemUserId},
 * poblados en {@code AuthFilter}) y el contexto HTTP de la request ({@code client.ip} / {@code
 * user_agent.original} / {@code http.method} / {@code http.path}, poblados en {@code
 * RequestLoggingContextFilter} para toda request — ver {@link
 * com.vetsoftware.app.infrastructure.logging.MdcKeys}) viajan por el MDC, y el encoder estructurado
 * de Spring Boot los emite como campos JSON; aquí solo se añaden los campos propios del evento (no
 * se duplican {@code http.method}/{@code http.path} ya presentes en el MDC).
 *
 * <p>Convención de campos: notación con punto ({@code http.*}, {@code actor.*}) alineada con
 * OpenTelemetry. Nota: {@code actor.identifier} (en login) es el <em>código</em> de
 * empleado/usuario tal como llega en la request — distinto de {@code actor.employeeId} del MDC, que
 * es el id numérico del principal ya autenticado.
 *
...
public class AuditLogger {
  ...
  /**
   * Alta de una nueva empresa (veterinaria) por auto-registro público. Sin datos sensibles; {@code
   * ownerCode} es el código de acceso del dueño, no un secreto. client.ip/http.path vía MDC.
   */
  public void companyRegistered(
      Long companyId,
      String companyName,
  ...
  /**
   * Un admin (o quien tenga el permiso) envió una invitación de acceso a un nuevo empleado. El
   * actor que invita viaja en el MDC (actor.employeeId / actor.companyId). {@code invitedCode} es
   * el código de acceso del invitado, no un secreto.
   */
  public void employeeInvited(Long invitedEmployeeId, String invitedCode, Long companyId) {
    persist(
        "employee_invited", "SUCCESS",
  ...
  /**
   * Reenvío de la invitación (nueva contraseña provisional) por un admin/con permiso. Actor vía
   * MDC.
   */
  public void employeeInvitationResent(Long invitedEmployeeId, String invitedCode, Long companyId) {
    persist(
        "employee_invitation_resent", "SUCCESS",
        "employee.id", invitedEmployeeId,
        "employee.identifier", invitedCode,
        "company.id", companyId);
    audit
   ...
   * {@code identifier} es el código de empleado intentado (no un secreto). http.path/client.ip vía
   * MDC.
   */
  public void loginBlockedEmailNotVerified(String identifier) {
    persist(
        "login_blocked_email_not_verified", "DENIED",
        "actor.identifier", identifier,

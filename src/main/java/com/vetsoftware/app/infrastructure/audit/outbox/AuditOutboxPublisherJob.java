...
final class AuditOutboxPublisherJob {
  ...
  private ScheduledJobTelemetry.Outcome applyResponse(
    ...
    if (failures > 0) {
      log.warn(
          "Firehose aceptó parcialmente auditoría; publicados={} fallidos={}",
          publishedIds.size(),
          failures);
    }
   ...
   * orden de claves o el formato y el registro archivado dejaría de corresponder a su {@code
   * payloadHash}. Los cuatro valores insertados son un entero y tres hexadecimales de la base, así
   * que no requieren escapado.
   */

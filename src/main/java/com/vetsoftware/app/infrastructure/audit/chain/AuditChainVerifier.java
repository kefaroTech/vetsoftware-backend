 ...
 *
 * <p>Función pura sobre una lista de eslabones consecutivos: no toca base de datos ni reloj, de
 * modo que las reglas de integridad se pueden probar de forma aislada.
 */
public final class AuditChainVerifier {
  ...
  /**
   * @param links eslabones ordenados por posición ascendente
   * @param expectedFirstSequence posición que debe tener el primer eslabón
   * @param expectedPreviousHash {@code chain_hash} del eslabón anterior al primero de la lista, o
   *     {@link AuditChainHash#GENESIS_HASH} si se arranca desde el origen
   */
  public static Result verify(
      List<AuditChainRepository.Link> links,
    ...
    for (AuditChainRepository.Link link : links) {
      ...
      if (link.sequence() != expectedSequence) {
        return Result.broken(
            ...
            previousHash,
            link.sequence(),
            "hueco en la cadena: se esperaba la posición "
                + expectedSequence
                + " y se encontró "
                + link.sequence());
      }
      ...
      String recomputedChainHash =
          AuditChainHash.chainHash(link.previousHash(), link.sequence(), link.payloadHash());
  ...
  /**
   * @param intact si no se encontró divergencia
   * @param checkedCount eslabones verificados correctamente
   * @param lastVerifiedSequence última posición válida; punto de partida de la siguiente pasada
   * @param lastVerifiedHash {@code chain_hash} de esa posición
   * @param failureSequence posición donde se detectó la divergencia, 0 si no hubo
   * @param failureReason motivo de la divergencia, nulo si no hubo
   */
  public record Result(
    ...
    static Result broken(
        ...
        String reason) {
      return new Result(
          false, checked, lastVerifiedSequence, lastVerifiedHash, failureSequence, reason);
    }
  }
}

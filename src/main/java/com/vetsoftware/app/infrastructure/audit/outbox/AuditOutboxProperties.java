...
public class AuditOutboxProperties {
    ...
    /**
     * Cuántas filas secuencia cada ciclo del publicador. Debe cubrir el ritmo de
     * inserción.
     */
    ...
    public int getSequenceBatchSize() {
        return sequenceBatchSize;
    }

    public void setSequenceBatchSize(int sequenceBatchSize) {
    ...
    void validate() {
        if (batchSize < 1 || batchSize > 500) {
            throw new IllegalStateException(
                    "vetsoftware.audit.outbox.batch-size debe estar entre 1 y 500");
        }
        if (cleanupBatchSize < 1) {
            throw new IllegalStateException(
                    "vetsoftware.audit.outbox.cleanup-batch-size debe ser positivo");
        }
        ...
        // Si el secuenciador avanzara menos que el publicador, la cola de no
        // secuenciados crecería
        ...
        if (leaseDuration.isNegative() || leaseDuration.isZero() || baseRetryDelay.isNegative()
                || baseRetryDelay.isZero() || maxRetryDelay.compareTo(baseRetryDelay) < 0

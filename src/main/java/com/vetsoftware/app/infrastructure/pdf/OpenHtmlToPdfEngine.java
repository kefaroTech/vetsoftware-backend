 ...
 *
 * <p>La concurrencia y el tamaño de salida están acotados para proteger la memoria del proceso del
 * backend frente a reportes anormalmente grandes.
 */
@Component
public class OpenHtmlToPdfEngine implements HtmlToPdfEngine {
  ...
  private void acquireRenderSlot() {
    try {
      boolean acquired =
          renderSlots.tryAcquire(properties.acquireTimeout().toMillis(), TimeUnit.MILLISECONDS);
      if (!acquired) {

...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  @ExceptionHandler({
    ...
    MembershipSubModuleNotFoundException.class,
    ModuleNotFoundException.class,
    PermissionNotFoundException.class,
    SubModuleNotFoundException.class,
    BasePermissionNotFoundException.class,
    BaseRoleNotFoundException.class,
    BaseRolePermissionNotFoundException.class,
    ...
    SystemPermissionNotFoundException.class,
    ...
    AnimalAlertNotFoundException.class,
    AppointmentNotFoundException.class,
    ConsultationTypeNotFoundException.class,
    ConsultationNotFoundException.class,
    VaccinationTypeNotFoundException.class,
    VaccinationNotFoundException.class,
    ...
    LaboratoryTestTypeNotFoundException.class,
    LaboratoryTestNotFoundException.class,
    ...
    SpaTypeNotFoundException.class,
    SpaNotFoundException.class,
    MedicamentPrescriptionNotFoundException.class,
    MedicamentNotFoundException.class,
    SurgeryTypeNotFoundException.class,
    SurgeryNotFoundException.class,
    DiagnosticImagingTypeNotFoundException.class,
    ...
    OpenAccountNotFoundException.class,
    DebtOpenAccountNotFoundException.class,
    ...
    EconomicActivityNotFoundException.class,
    CompanyTaxProfileNotFoundException.class,
    NumberingResolutionNotFoundException.class,
    ElectronicDocumentNotFoundException.class,
    DianProviderConfigNotFoundException.class,
    WithholdingConfigNotFoundException.class,
    BranchNotFoundException.class,
    InventoryCountNotFoundException.class,
  ...
  // Compras: transición de estado inválida en orden de compra o recepción (p. ej. editar una PO ya
  // recibida,
  // confirmar una recepción no-borrador, cancelar una recepción no-confirmada). Código propio
  // derivado por clase
  // (INVALID_PURCHASE_ORDER_STATUS_TRANSITION / INVALID_GOODS_RECEIPT_STATUS_TRANSITION) para el
  // front.
  @ExceptionHandler({
  ...
  // CxP (F3): operación no válida para el estado de la factura de proveedor (editar/anular con
  // abonos, abonar una
  ...
  // Unicidad de NOMBRE por empresa (migraciones 151-154). errorCode(ex) deriva el código correcto
  // por
  ...
  // F5: correccion por nota credito/debito sobre un documento en estado invalido (no VALIDADO o ya
  // reversado).
  ...
  // Inventario: la venta/consumo no alcanza y la empresa no permite stock negativo. 409 con código
  // propio
  ...
  // Caja: conflictos de estado de la sesión. Códigos propios (derivados por clase) para que el
  // front distinga
  ...
  // Detección temprana del mismo conflicto: la versión que envió el front (expectedVersion) ya no
  // es
  ...
  // Más específico que el handler de AccessDeniedException de abajo: Spring elige este para la
  // subclase.
  // Devuelve el motivo concreto (la sede) y un código propio para que el front lo distinga del 403
  // genérico.
  @ExceptionHandler(BranchAccessDeniedException.class)
  public ProblemDetail handleBranchAccessDenied(
      BranchAccessDeniedException ex, HttpServletRequest request) {
  ...
  public ProblemDetail handleDataIntegrity(DataIntegrityViolationException ex) {
    ...
    if (cause != null
        && (cause.contains("uq_open_accounts_active_owner_branch")
    ...
    // concurrente que pasó el check del service la atrapa la BD. Se mapea al mismo código de
    // negocio.
    ...
    // Carrera en la unicidad del número de factura de proveedor por (empresa, proveedor) (migración
    // 203).
    ...
    // Carrera en la idempotencia de abonos (constraint de la migración 135): doble-submit
    // concurrente con
    // la misma clave; la BD rechaza el 2º. El cliente reintenta y el check de idempotencia devuelve
    // el abono.
    ...
    // Carrera en la idempotencia de cargos (constraints de las migraciones 139/140/141):
    // doble-submit
    // concurrente con la misma clave; la BD rechaza el 2º. El cliente reintenta y el check de
    // idempotencia
    // devuelve el cargo ya creado.
    if (cause != null
        && (cause.contains("uq_product_charge_open_accounts_request")
    ...
    // Carrera/reactivación en la unicidad de "una sola resolución activa por (company, tipo)"
    // (migración
    ...
    // Carrera en la unicidad de employee_code (código de empleado / correo del dueño en el
    // registro):
    ...
    if (cause != null && cause.contains("employee_code")) {
      return problem(
          HttpStatus.CONFLICT, "EMPLOYEE_CODE_TAKEN", "Ese usuario ya está en uso. Elige otro.");
    }
    ...
    // Carrera en "una sola caja OPEN por (empresa, sede, terminal)" (índice único condicional de la
    // migración
    // 195): la 2ª apertura concurrente que pasó el check del service la atrapa la BD. Mismo código
    // de negocio.
    ...
    return problem(
        HttpStatus.CONFLICT, "DATA_INTEGRITY_VIOLATION", "Database constraint violation");
  }
  ...
  public ProblemDetail handlePdfRender(PdfRenderException ex, HttpServletRequest request) {
    ...
    return problem(HttpStatus.BAD_GATEWAY, "PDF_RENDER_FAILED", "Failed to generate PDF document");
  }
  ...
  public ProblemDetail handleS3Storage(S3StorageException ex, HttpServletRequest request) {
    ...
    return problem(HttpStatus.BAD_GATEWAY, "FILE_STORAGE_FAILED", "Failed to access file storage");
  }

 ...
 * de entorno {@code DIAN_ENC_KEY}. Hay una clave de desarrollo embebida en el repo que SOLO se
 * acepta cuando un perfil dev/test/local está <b>explícitamente</b> activo (property {@code
 * spring.profiles.active} o env {@code SPRING_PROFILES_ACTIVE}). Un perfil ausente/por defecto NO
 * habilita la clave insegura: el arranque falla (fail-fast) para cerrar la trampa de un deploy
 * productivo que olvida definir el perfil o la clave y terminaría cifrando los secretos DIAN con la
 * clave pública del repositorio.
 *
 * <p>Rotación: el formato persistido lleva un prefijo de versión de clave ({@code
 * "<version>:base64(iv||ct)}"). El cifrado usa la clave y versión activas; el descifrado selecciona
 * la clave por versión, admitiendo una clave anterior opcional ({@code DIAN_ENC_KEY_PREVIOUS} +
 * {@code DIAN_ENC_KEY_PREVIOUS_VERSION}) mientras se re-cifran los datos. Los valores en formato
 * legado (sin prefijo, escritos antes de esta versión) se descifran con la clave activa. El
 * separador {@code ':'} nunca aparece en base64, así que el prefijo es inequívoco. Pensado para
 * secretos reversibles (credenciales/tokens del proveedor DIAN); NO para contraseñas de usuario
 * (esas usan BCrypt, no reversible).
 *
 * <p>Hibernate instancia este converter (no es un bean de Spring), por eso la clave se resuelve de
 * forma estática desde el entorno.
 */
@Converter
public class EncryptedStringConverter implements AttributeConverter<String, String> {
  ...
  // Default SOLO para desarrollo (32 bytes base64). En produccion DEBE sobreescribirse con
  // DIAN_ENC_KEY.
  ...
  /**
   * Clave activa (para cifrar) + versiones conocidas (para descifrar, incluida la anterior en
   * rotación).
   */
  ...
  private static KeyMaterial loadKeys() {
    ...
    if (missing) {
      ...
      // dev/test/local está EXPLÍCITAMENTE activo. Un perfil ausente/por defecto NO la habilita:
      // así
      // un arranque productivo que olvida SPRING_PROFILES_ACTIVE falla en vez de cifrar con la
      // clave
      // pública del repo.
      if (!isExplicitDevOrTestProfile()) {
        throw new IllegalStateException(
            "DIAN_ENC_KEY no está definida. Defina DIAN_ENC_KEY (32 bytes en base64) para cifrar"
                + " los secretos del proveedor DIAN, o active EXPLÍCITAMENTE un perfil"
                + " dev/test/local (SPRING_PROFILES_ACTIVE) para usar la clave de desarrollo"
                + " embebida.");
      }
      log.warn(
          "DIAN_ENC_KEY no definida: usando la clave de desarrollo embebida del repo (version"
    ...
    if (DEV_KEY_BASE64.equals(configured.trim()) && !isExplicitDevOrTestProfile()) {
      throw new IllegalStateException(
          "DIAN_ENC_KEY es la clave de desarrollo pública del repositorio y el perfil activo no es"
              + " dev/test/local. Genere una clave AES-256 propia (32 bytes en base64) para"
              + " producción.");
    }
    String activeVersion =
        resolveVersion(System.getenv("DIAN_ENC_KEY_VERSION"), DEFAULT_KEY_VERSION);
    keys.put(activeVersion, toKey(configured));
    // Clave anterior opcional: permite descifrar los valores aún cifrados con ella durante una
    // rotación.
      ...
      if (normalized.equals(activeVersion)) {
        throw new IllegalStateException(
            "DIAN_ENC_KEY_PREVIOUS_VERSION no puede coincidir con DIAN_ENC_KEY_VERSION ("
                + activeVersion
                + "): use una etiqueta de versión distinta para la clave anterior.");
      }
  ...
  private static String resolveVersion(String configured, String fallback) {
    ...
    if (version.indexOf(VERSION_SEPARATOR) >= 0) {
      throw new IllegalStateException(
          "la versión de clave no puede contener '" + VERSION_SEPARATOR + "'");
    }
    return version;
  }
  ...
  // El converter se inicializa de forma estatica (antes de que Spring resuelva el Environment), por
  // eso el
  // perfil se lee directo de la system property / env var. A diferencia de application.yml, aquí NO
  // hay
  // default a "dev": un perfil ausente no cuenta como dev para habilitar la clave insegura (ver
  // loadKeys).

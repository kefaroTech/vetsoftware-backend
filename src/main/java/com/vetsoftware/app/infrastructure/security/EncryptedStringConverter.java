...
/**
 * Cifra/descifra columnas de texto con AES-GCM. La clave (32 bytes en base64)
 * se lee de la variable de entorno {@code DIAN_ENC_KEY}. No existe una clave
 * predeterminada en el código: todos los perfiles, incluidos local y test,
 * deben inyectar una clave propia de 32 bytes en base64. Si falta, el arranque
 * falla inmediatamente para impedir que datos sensibles se cifren con material
 * público o compartido.
 *
 * <p>
 * Rotación: el formato persistido lleva un prefijo de versión de clave
 * ({@code "<version>:base64(iv||ct)}"). El cifrado usa la clave y versión
 * activas; el descifrado selecciona la clave por versión, admitiendo una clave
 * anterior opcional ({@code DIAN_ENC_KEY_PREVIOUS} +
 * {@code DIAN_ENC_KEY_PREVIOUS_VERSION}) mientras se re-cifran los datos. Los
 * valores en formato legado (sin prefijo, escritos antes de esta versión) se
 * descifran con la clave activa. El separador {@code ':'} nunca aparece en
 * base64, así que el prefijo es inequívoco. Pensado para secretos reversibles
 * (credenciales/tokens del proveedor DIAN); NO para contraseñas de usuario
 * (esas usan BCrypt, no reversible).
 *
 * <p>
 * Hibernate instancia este converter (no es un bean de Spring), por eso la
 * clave se resuelve de forma estática desde el entorno.
 */
@Converter
public class EncryptedStringConverter implements AttributeConverter<String, String> {
    ...
    // Version por defecto de la clave productiva si no se define
    // DIAN_ENC_KEY_VERSION.
    private static final String DEFAULT_KEY_VERSION = "v1";
    // Separa "<version>:<payload base64>" en el formato persistido. El alfabeto
    // base64 no incluye ':'.
    ...
    /**
     * Clave activa (para cifrar) + versiones conocidas (para descifrar, incluida la
    ...
    private static KeyMaterial loadKeys() {
        ...
        String activeVersion = resolveVersion(System.getenv("DIAN_ENC_KEY_VERSION"),
                DEFAULT_KEY_VERSION);
        keys.put(activeVersion, toKey(configured));
        // Clave anterior opcional: permite descifrar los valores aún cifrados con ella
    ...
    private static String resolveVersion(String configured, String fallback) {
        String version = (configured == null || configured.isBlank())
                ? fallback
                : configured.trim();
    ...
    public String convertToDatabaseColumn(String attribute) {
        ...
        try {
            ...
            cipher.init(Cipher.ENCRYPT_MODE, MATERIAL.activeKey,
                    new GCMParameterSpec(TAG_BITS, iv));
            byte[] ciphertext = cipher.doFinal(attribute.getBytes(StandardCharsets.UTF_8));
            byte[] out = new byte[iv.length + ciphertext.length];
            System.arraycopy(iv, 0, out, 0, iv.length);
            System.arraycopy(ciphertext, 0, out, iv.length, ciphertext.length);
            return MATERIAL.activeVersion + VERSION_SEPARATOR
    ...
    public String convertToEntityAttribute(String dbData) {
        if (dbData == null)
            return null;
        try {
            // Selecciona la clave por el prefijo de versión. Formato legado (sin ':') →
            // clave activa.
            ...
            if (separator >= 0) {
                ...
                if (key == null) {
                    throw new IllegalStateException("No hay clave para la versión '" + version
                            + "' del valor cifrado; "

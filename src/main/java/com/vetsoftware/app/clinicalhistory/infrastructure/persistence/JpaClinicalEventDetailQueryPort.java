...
/**
 * Carga el detalle clínico rico de cada evento desde la entidad fuente de su feature. Cruce
 * cross-feature permitido: solo importa los {@code XxxJpaRepository} de otras features (adaptadores
 * de persistencia), nunca su dominio ni su application.
 */
@Component
public class JpaClinicalEventDetailQueryPort implements ClinicalEventDetailQueryPort {
  ...
  public JpaClinicalEventDetailQueryPort(
      ConsultationJpaRepository consultationRepo,
  ...
  private DetailTable medicamentTable(Long prescriptionId) {
    List<List<String>> rows =
        medicamentPrescriptionRepo.findByPrescriptionId(prescriptionId).stream()
            .map(
                m ->
                    List.of(
                        nz(m.getName()),
  ...
  private DetailTable medicationTable(Long hospitalizationId) {
    List<List<String>> rows =
        hospitalizationMedicationRepo.findByHospitalizationId(hospitalizationId).stream()
  ...
  private DetailTable procedureTable(Long hospitalizationId) {
    List<List<String>> rows =
        hospitalizationProcedureRepo.findByHospitalizationId(hospitalizationId).stream()
  ...
  private DetailTable progressNoteTable(Long hospitalizationId) {
    List<List<String>> rows =
        hospitalizationProgressNoteRepo.findByHospitalizationId(hospitalizationId).stream()
            ...
            .map(
                n ->
                    List.of(
                        ...
                        nz(n.getDescription())))
            .toList();
    return new DetailTable("Notas de evolución", List.of("Fecha", "Registró", "Evolución"), rows);
  }

  private DetailTable observationTable(Long hospitalizationId) {
    List<List<String>> rows =
        hospitalizationObservationRepo.findByHospitalizationId(hospitalizationId).stream()
  ...
  private static String startAt(LocalDate startDate, LocalTime startTime) {
    ...
    return startTime == null ? startDate.format(DATE) : startDate.format(DATE) + " " + startTime;
  }

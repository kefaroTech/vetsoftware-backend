...
/**
 * Carga el detalle clínico rico de cada evento desde la entidad fuente de su feature.
 * Cruce cross-feature permitido: solo importa los {@code XxxJpaRepository} de otras features
 * (adaptadores de persistencia), nunca su dominio ni su application.
 */
@Component
public class JpaClinicalEventDetailQueryPort implements ClinicalEventDetailQueryPort {
    ...
    public JpaClinicalEventDetailQueryPort(ConsultationJpaRepository consultationRepo,
                                           SurgeryJpaRepository surgeryRepo,
                                           VaccinationJpaRepository vaccinationRepo,
                                           DewormingJpaRepository dewormingRepo,
                                           HospitalizationJpaRepository hospitalizationRepo,
    ...
    public Optional<ClinicalEventDetail> load(ClinicalEventType type, Long sourceId, Long companyId) {
        return switch (type) {
    ...
    private Optional<ClinicalEventDetail> consultation(Long id, Long companyId) {
        return consultationRepo.findById(id)
                .filter(e -> sameCompany(e.getCompany() == null ? null : e.getCompany().getId(), companyId))
                .map(e -> {
    ...
    private Optional<ClinicalEventDetail> surgery(Long id, Long companyId) {
        return surgeryRepo.findById(id)
                .filter(e -> sameCompany(e.getCompany() == null ? null : e.getCompany().getId(), companyId))
                .map(e -> {
                    List<DetailField> f = new ArrayList<>();
    ...
    private Optional<ClinicalEventDetail> deworming(Long id, Long companyId) {
        return dewormingRepo.findById(id)
                .filter(e -> sameCompany(e.getCompany() == null ? null : e.getCompany().getId(), companyId))
                .map(e -> {
                    List<DetailField> f = new ArrayList<>();
    ...
    private Optional<ClinicalEventDetail> hospitalization(Long id, Long companyId) {
        return hospitalizationRepo.findById(id)
                .filter(e -> sameCompany(e.getCompany() == null ? null : e.getCompany().getId(), companyId))
                .map(e -> {
                    List<DetailField> f = new ArrayList<>();
                    ...
                    return new ClinicalEventDetail(hospitalizationType(enumName(e.getType())), f, tables);
                });
    }

    private Optional<ClinicalEventDetail> laboratoryTest(Long id, Long companyId) {
        return laboratoryTestRepo.findById(id)
                .filter(e -> sameCompany(e.getCompany() == null ? null : e.getCompany().getId(), companyId))
                .map(e -> {
                    List<DetailField> f = new ArrayList<>();
                    add(f, "Indicación", e.getDiagnosis());
                    add(f, "Cantidad", e.getQuantity() == null ? null : String.valueOf(e.getQuantity()));
                    add(f, "Prioridad", e.getPrioridad());
                    add(f, "Estado", e.getStatus());
                    add(f, "Procesado", e.getProcessedDate() == null ? null
    ...
    private Optional<ClinicalEventDetail> diagnosticImaging(Long id, Long companyId) {
        return diagnosticImagingRepo.findById(id)
                .filter(e -> sameCompany(e.getCompany() == null ? null : e.getCompany().getId(), companyId))
                .map(e -> {
                    List<DetailField> f = new ArrayList<>();
    ...
    private DetailTable medicamentTable(Long prescriptionId) {
        List<List<String>> rows = medicamentPrescriptionRepo.findByPrescriptionId(prescriptionId).stream()
                .map(m -> List.of(
                        nz(m.getName()),
    ...
    private DetailTable medicationTable(Long hospitalizationId) {
        List<List<String>> rows = hospitalizationMedicationRepo.findByHospitalizationId(hospitalizationId).stream()
                ...
                .map(m -> List.of(
                        ...
                        startAt(m.getStartDate(), m.getStartTime()),
                        status(m.getSuspensionDate())))
                .toList();
        return new DetailTable("Medicaciones",
                List.of("Medicamento", "Dosis", "Frecuencia", "Duración", "Inicio", "Estado"), rows);
    }

    private DetailTable procedureTable(Long hospitalizationId) {
        List<List<String>> rows = hospitalizationProcedureRepo.findByHospitalizationId(hospitalizationId).stream()
                ...
                .map(p -> List.of(
                        ...
                        startAt(p.getStartDate(), p.getStartTime()),
                        status(p.getSuspensionDate())))
                .toList();
        return new DetailTable("Procedimientos",
                List.of("Procedimiento", "Dosis", "Frecuencia", "Duración", "Inicio", "Estado"), rows);
    }

    private DetailTable progressNoteTable(Long hospitalizationId) {
        List<List<String>> rows = hospitalizationProgressNoteRepo.findByHospitalizationId(hospitalizationId).stream()
                .sorted(Comparator.comparing(n -> n.getCreatedDate()))
                .map(n -> List.of(
                        n.getCreatedDate().toLocalDate().format(DATE),
                        author(n.getCreatedBy() == null ? null : n.getCreatedBy().getName()),
                        nz(n.getDescription())))
                .toList();
        return new DetailTable("Notas de evolución",
                List.of("Fecha", "Registró", "Evolución"), rows);
    }

    private DetailTable observationTable(Long hospitalizationId) {
        List<List<String>> rows = hospitalizationObservationRepo.findByHospitalizationId(hospitalizationId).stream()
                .sorted(Comparator.comparing(o -> o.getCreatedDate()))
                .map(o -> List.of(
                        o.getCreatedDate().toLocalDate().format(DATE),
                        author(o.getCreatedBy() == null ? null : o.getCreatedBy().getName()),
                        nz(o.getDescription())))
                .toList();
        return new DetailTable("Observaciones",
                List.of("Fecha", "Registró", "Observación"), rows);
    }
    ...
    /** Nombre del enum sin importar su tipo de dominio (respeta el vertical slicing). */
    ...
    private static String hospitalizationType(String type) {
        if (type == null) return "Hospitalización";
        return switch (type) {

...
public interface ProductChargeOpenAccountJpaRepository
  ...
  @Override
  @EntityGraph(
      attributePaths = {"animal", "product", "tax", "openAccount", "createdBy", "voidedBy"})
  ...
  @EntityGraph(
      attributePaths = {"animal", "product", "tax", "openAccount", "createdBy", "voidedBy"})
  Optional<ProductChargeOpenAccountJpaEntity> findByIdAndOpenAccount_Company_Id(
  ...
  @org.springframework.data.jpa.repository.Query(
      value =
          "UPDATE product_charge_open_accounts SET enabled = true WHERE id = :id AND EXISTS (SELECT"
              + " 1 FROM open_accounts oa WHERE oa.id ="
              + " product_charge_open_accounts.open_account_id AND oa.company_id = :companyId)",
      nativeQuery = true)
  int reactivate(

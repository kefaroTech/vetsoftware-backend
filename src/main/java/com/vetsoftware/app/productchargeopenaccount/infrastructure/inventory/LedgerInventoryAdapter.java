...
/**
 * Adapter de orquestación cuenta-abierta → inventario. Es el ÚNICO punto de esta feature que conoce el
 * {@code StockLedgerUseCase} de {@code inventory}; traduce el cargo/anulación a movimientos de kardex con
 * referencia {@link StockReferenceType#OPEN_ACCOUNT_CHARGE} y el id del cargo (idempotencia + compensación).
 */
...
public class LedgerInventoryAdapter implements InventoryLedgerPort {
    ...
    @Override
    public void recordSale(Long companyId, Long branchId, Long productId, int quantity, Long chargeId, Long createdBy) {
        // allowNegative=false: la cuenta abierta respeta el flag de stock negativo de la empresa.
    ...
    @Override
    public void reverseSale(Long chargeId, Long actorId) {
        stockLedger.reverse(StockReferenceType.OPEN_ACCOUNT_CHARGE, chargeId, actorId);
    }
}

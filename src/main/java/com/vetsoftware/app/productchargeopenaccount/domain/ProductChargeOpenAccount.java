...
public class ProductChargeOpenAccount {
    ...
    /**
     * Impuesto congelado heredado del catálogo del producto; null si el producto no
     * aplica impuesto.
     */
    ...
    /**
     * Esquema tributario congelado del catálogo: "IVA" o "INC"; null si el producto
     * no aplica impuesto.
     */
    ...
    public ProductChargeOpenAccount(Long id, AnimalRef animal, ProductRef product,
            BigDecimal unitPrice, int quantity, TaxRef tax, boolean hasTax,
    ...
    public ProductChargeOpenAccount(Long id, AnimalRef animal, ProductRef product,
        ...
        this(id, animal, product, unitPrice, openAccount, createdBy, createdDate, enabled, false,
                null, null, null);
    }

    public static ProductChargeOpenAccount create(AnimalRef animal, ProductRef product,
            int quantity, OpenAccountRef openAccount, EmployeeRef createdBy,
            String clientRequestId) {
        // Congela el precio de venta vigente del producto: el total de la cuenta no
        // debe
        // cambiar si el catálogo se edita después.
        BigDecimal unitPrice = product == null || product.salePrice() == null
                ? BigDecimal.ZERO
                : product.salePrice();
        // El impuesto se hereda del catálogo del producto y se congela. El precio
        // incluye IVA.
    ...
    public void voidCharge(EmployeeRef voidedBy, String reason) {
        if (this.voided)
            throw new ProductChargeOpenAccountAlreadyVoidedException(this.id);
        if (voidedBy == null)

...
public class CreateProductChargeOpenAccountService
        implements
            CreateProductChargeOpenAccountUseCase {
    ...
    public CreateProductChargeOpenAccountService(ProductChargeOpenAccountRepository repository,
            ...
            InventoryLedgerPort inventoryLedger, BranchResolverPort branchResolver) {
    ...
    public ProductChargeOpenAccountDto execute(CreateProductChargeOpenAccountCommand command) {
        ...
        if (command.clientRequestId() != null && !command.clientRequestId().isBlank()) {
            Optional<ProductChargeOpenAccount> existing = repository
        ...
        // Sede que descuenta el inventario: la pedida (validada activa/empresa) o la
        // Principal por
        // defecto.
        ...
        ProductChargeOpenAccount charge = ProductChargeOpenAccount.create(animal, product,
                command.quantity(), openAccount, createdBy, command.clientRequestId());
        ...
        // Registra la salida de inventario en el kardex de la sede (FEFO + costo por
        // lote), idempotente
        // por el id del
        // cargo. Dentro de la misma transacción: si algo falla después, el movimiento
        // se revierte con
        // el resto. Puede
        // lanzar InsufficientStockException si no hay stock y la empresa no permite
        // negativo.
        inventoryLedger.recordSale(command.companyId(), branchId, command.productId(),
                command.quantity(), saved.getId(), command.createdById());

...
public interface MedicamentJpaRepository extends JpaRepository<MedicamentJpaEntity, Long> {
  ...
  // Native: los pausados (enabled = false) NO pasan el @SQLRestriction; se listan crudos para
  // reactivar.
  @Query(
      value = "SELECT * FROM medicaments WHERE enabled = false AND company_id = :companyId",
      nativeQuery = true)

...
public class CreateMedicamentService implements CreateMedicamentUseCase {
     ...
     * para un medicamento propio, el vademécum de plataforma para uno general— y,
     * si lo ocupa una fila pausada, la reactiva en vez de insertar otra.
     *
     * <p>
     ...
     * rojo (#559). Sin la rama de reactivación el mensaje era además engañoso: el
     * medicamento que ocupaba el nombre estaba pausado, no aparecía en el catálogo
     * activo —{@code @SQLRestriction("enabled = true")}— y el sistema hablaba de un
     * conflicto con algo invisible. La pausa LIBERA el nombre (el índice único
     * cubre solo los activos), así que reactivar es la respuesta correcta y no un
     * apaño (#432).
     *
     * <p>
    ...
    public MedicamentDto execute(CreateMedicamentCommand command) {
        ...
        if (existing.isPresent()) {
            return MedicamentDto.from(reactivate(existing.get(), command, company));
        }
     ...
     */
    private Medicament reactivate(Medicament existing, CreateMedicamentCommand command,
            CompanyRef company) {

...
public class MedicationScheduleController {
  ...
  public MedicationScheduleController(
      GenerateMedicationScheduleUseCase generateUseCase,
  ...
  public List<MedicationScheduleResponse> generate(@PathVariable Long hospitalizationMedicationId) {
    return generateUseCase
        .execute(
            new GenerateMedicationScheduleCommand(
                hospitalizationMedicationId, authz.currentEmployeeId()))
        .stream()
        .map(this::toResponse)
        .toList();
  }
  ...
  public List<MedicationScheduleResponse> reschedule(
      @PathVariable Long id, @Valid @RequestBody RescheduleMedicationScheduleRequest request) {
    return rescheduleUseCase
        .execute(new RescheduleMedicationScheduleCommand(id, request.newDateTime(), request.mode()))
        .stream()
        .map(this::toResponse)
        .toList();
  }
  ...
  private MedicationScheduleResponse toResponse(MedicationScheduleDto dto) {
    return new MedicationScheduleResponse(
        dto.id(),
        new HospitalizationMedicationSummary(
            dto.hospitalizationMedicationId(), dto.hospitalizationMedicationName()),

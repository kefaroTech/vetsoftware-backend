...
public interface MedicationScheduleJpaRepository
    ...
    @org.springframework.data.jpa.repository.Query(value = "UPDATE medication_schedules SET enabled = false WHERE hospitalization_medication_id ="
            + " :medicationId AND enabled = true", nativeQuery = true)
    ...
    @org.springframework.transaction.annotation.Transactional
    @org.springframework.data.jpa.repository.Query(value = "UPDATE medication_schedules SET enabled = false WHERE hospitalization_medication_id ="
            + " :medicationId AND enabled = true AND (applied_status IS NULL OR applied_status <>"
            + " 'APPLIED')", nativeQuery = true)
    int disablePendingByHospitalizationMedicationId(
            @org.springframework.data.repository.query.Param("medicationId") Long medicationId);

    @org.springframework.data.jpa.repository.Modifying(flushAutomatically = true, clearAutomatically = true)
    @org.springframework.transaction.annotation.Transactional
    @org.springframework.data.jpa.repository.Query(value = "UPDATE medication_schedules SET enabled = true WHERE id = :id", nativeQuery = true)

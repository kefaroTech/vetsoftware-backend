...
/**
 * Servicio de dominio puro: genera las tomas (MedicationSchedule) de una orden de medicación a
 * partir de su frecuencia, duración y hora de inicio. Sin dependencias de Spring ni de
 * infraestructura.
 */
public final class MedicationScheduleGenerator {
  ...
  public static List<MedicationSchedule> generate(
      MedicationOrderParams params, EmployeeRef createdBy) {
    ...
    HospitalizationMedicationRef ref = new HospitalizationMedicationRef(params.id(), params.name());
  ...
  /**
   * Regla de integridad: regenera SOLO las tomas pendientes conservando las ya aplicadas. - Pauta
   * INTERVALO con aplicadas: re-cronometra desde la hora real de la última aplicada
   * (`lastAppliedRealDateTime`) + intervalo. - Pauta FIJA (o INTERVALO sin aplicadas): usa las
   * horas de reloj saltando las primeras `appliedCount` posiciones (ya cubiertas por aplicadas). -
   * DOSES: el total objetivo manda; pendientes = total - appliedCount (nunca negativo).
   */
  public static List<MedicationSchedule> generatePending(
  ...
  private static MedicationSchedule slotAt(
      HospitalizationMedicationRef ref, LocalDateTime dt, EmployeeRef createdBy) {
  ...
  private static int doseCount(
      String durationMeasure, Integer durationQuantity, int intervalHours) {

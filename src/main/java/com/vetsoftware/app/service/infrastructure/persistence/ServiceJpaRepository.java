...
public interface ServiceJpaRepository
    ...
    @org.springframework.data.jpa.repository.Query(value = "SELECT * FROM services WHERE company_id = :companyId AND enabled = false ORDER BY name", nativeQuery = true)
    List<ServiceJpaEntity> findAllDisabledByCompany_Id(
            @org.springframework.data.repository.query.Param("companyId") Long companyId);

    @org.springframework.data.jpa.repository.Modifying(flushAutomatically = true, clearAutomatically = true)
    @org.springframework.transaction.annotation.Transactional
    @org.springframework.data.jpa.repository.Query(value = "UPDATE services SET enabled = true WHERE id = :id AND company_id = :companyId", nativeQuery = true)

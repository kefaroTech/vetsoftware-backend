...
/**
 * Sucursal (sede) de una empresa. Es el ancla del multi-sucursal: los recursos operativos (stock, caja,
 * ventas, numeración fiscal, citas) se scopean a una {@code Branch}. {@code active} es un estado de negocio
 * (operativa / no operativa), independiente del borrado — una sucursal inactiva sigue visible para poder
 * reactivarla y para no perder el histórico que la referencia.
 */
public class Branch {
    private Long id;
    private String name;
    ...
    public Branch(Long id, String name, String code, String address, String phone,
                  CityRef city, CompanyRef company, LocalDateTime createdDate, boolean active) {
    ...
    public static Branch create(String name, String code, String address, String phone,
        ...
        return new Branch(null, name, code, address, phone, city, company, LocalDateTime.now(), true);
    }
    ...
    /** Desactiva la sucursal (deja de estar operativa, pero no se borra). Idempotente. */
    public void deactivate() { this.active = false; }

    private static void validate(String name, String code, String address, String phone,
                                 CityRef city, CompanyRef company) {
        if (name == null || name.isBlank()) throw new IllegalArgumentException("name is required");
        if (name.length() > 120) throw new IllegalArgumentException("name must be 120 chars or less");
        if (code == null || code.isBlank()) throw new IllegalArgumentException("code is required");
        if (code.length() > 30) throw new IllegalArgumentException("code must be 30 chars or less");

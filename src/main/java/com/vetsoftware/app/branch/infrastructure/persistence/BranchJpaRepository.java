...
public interface BranchJpaRepository extends JpaRepository<BranchJpaEntity, Long> {
    ...
    // ¿Existe la sede en la empresa, esté activa o no? Sirve para distinguir "inactiva" de "inexistente"
    ...
    // ¿Hay OTRA sede activa en la empresa (distinta de :id)? Guarda contra desactivar la última sede activa.
    boolean existsByCompany_IdAndActiveTrueAndIdNot(Long companyId, Long id);

    // Sede por defecto de una empresa (resolución de branchId cuando el request no lo trae): la "Principal".
    Optional<BranchJpaEntity> findFirstByCompany_IdAndCodeIgnoreCase(Long companyId, String code);

    // Sede por defecto para ESCRIBIR: la "Principal" pero solo si está ACTIVA. Si la Principal fue
    // desactivada/renombrada se cae al siguiente fallback; una sede inactiva nunca es la sede por defecto.
    Optional<BranchJpaEntity> findFirstByCompany_IdAndCodeIgnoreCaseAndActiveTrue(Long companyId, String code);

    // Fallback si no hubiese "Principal" (p. ej. renombrada): la primera sede activa.

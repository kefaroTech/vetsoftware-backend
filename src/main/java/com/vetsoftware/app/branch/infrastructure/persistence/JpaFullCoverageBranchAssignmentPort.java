...
/**
 * Adapter del auto-registro por sede. Cruce permitido de vertical slicing: infrastructure/persistence de {@code branch}
 * usa el repo JPA de {@code employeebranch} para materializar filas. La propagación a las sesiones vivas de los
 * empleados afectados es por TTL del cache {@code employee-branch-ids} (≤5 min); no se evicta explícitamente porque
 * crear una sede es una acción administrativa poco frecuente y de efecto no inmediato.
 */
@Component
public class JpaFullCoverageBranchAssignmentPort implements FullCoverageBranchAssignmentPort {
    ...
    public JpaFullCoverageBranchAssignmentPort(EmployeeBranchJpaRepository employeeBranchJpaRepository) {
    ...
    public void assignNewBranchToFullCoverageEmployees(Long companyId, Long newBranchId) {
        List<Long> employeeIds = employeeBranchJpaRepository.findFullCoverageEmployeeIds(companyId, newBranchId);

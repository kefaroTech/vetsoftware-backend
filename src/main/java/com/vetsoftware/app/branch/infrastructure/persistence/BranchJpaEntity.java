...
/**
 * Sucursal (sede) de una empresa. {@code active} es un estado de negocio, NO un soft-delete: no lleva
 * {@code @SQLRestriction}/{@code @SQLDelete}, así una sucursal inactiva sigue siendo visible (para
 * reactivarla y para no romper el histórico que la referencia).
 */
@Entity
@Table(name = "branches", uniqueConstraints = {
    @UniqueConstraint(name = "uq_branches_company_code", columnNames = {"company_id", "code"})
})

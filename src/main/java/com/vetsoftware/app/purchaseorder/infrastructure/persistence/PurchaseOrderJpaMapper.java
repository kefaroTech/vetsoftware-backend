...
public class PurchaseOrderJpaMapper {
    ...
    public PurchaseOrderJpaEntity toJpa(PurchaseOrder order, CompanyJpaEntity company,
            BranchJpaEntity branch, SupplierJpaEntity supplier,
            LongFunction<ProductJpaEntity> productRef) {
    ...
    // Read path — el @EntityGraph ya hidrató company/branch/supplier y las líneas
    // con su producto.
    public PurchaseOrder toDomain(PurchaseOrderJpaEntity entity) {
        ...
        List<PurchaseOrderLine> lines = entity.getLines().stream().map(le -> {
            ...
            return new PurchaseOrderLine(le.getId(),
                    new ProductRef(p.getId(), p.getName(), p.getCode()), le.getQuantityOrdered(),
                    le.getUnitCost(), le.getQuantityReceived());
        }).toList();
    ...
    // Write path — reusa los refs precargados; los productos de las líneas se
    // resuelven por id sin
    // inicializar el proxy de getReferenceById (el id está disponible en el proxy
    // sin SELECT).
    public PurchaseOrder toDomain(PurchaseOrderJpaEntity entity, CompanyRef companyRef,
            BranchRef branchRef, SupplierRef supplierRef, Map<Long, ProductRef> productRefs) {
        List<PurchaseOrderLine> lines = entity.getLines().stream()
                .map(le -> new PurchaseOrderLine(le.getId(),
                        productRefs.get(le.getProduct().getId()), le.getQuantityOrdered(),
                        le.getUnitCost(), le.getQuantityReceived()))
                .toList();
        return toDomain(entity, companyRef, branchRef, supplierRef, lines);
    }

    private PurchaseOrder toDomain(PurchaseOrderJpaEntity entity, CompanyRef companyRef,
            BranchRef branchRef, SupplierRef supplierRef, List<PurchaseOrderLine> lines) {
        return new PurchaseOrder(entity.getId(), companyRef, branchRef, supplierRef,
                entity.getStatus(), entity.getOrderDate(), entity.getExpectedDate(),
                entity.getNotes(), lines, entity.getCreatedDate(), entity.getCreatedBy(),
                entity.getUpdatedDate(), entity.getUpdatedBy(), entity.getVersion(),
                entity.isEnabled());
    }
}

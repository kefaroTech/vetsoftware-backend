...
public interface PurchaseOrderJpaRepository extends JpaRepository<PurchaseOrderJpaEntity, Long>,
    ...
    @EntityGraph(attributePaths = {"company", "branch", "supplier", "lines", "lines.product"})
    List<PurchaseOrderJpaEntity> findAllByCompany_IdOrderByOrderDateDescCreatedDateDesc(Long companyId);

    // Query nativa: el @SQLRestriction("enabled = true") NO aplica a SQL nativo, así que ésta es la
    // única vía para listar las órdenes PAUSADAS (enabled=false) y poder reactivarlas desde la UI.
    // Las asociaciones se hidratan perezosamente dentro de la transacción de lectura del caso de uso.
    @org.springframework.data.jpa.repository.Query(
        value = "SELECT * FROM purchase_orders WHERE company_id = :companyId AND enabled = false "
    ...
    @org.springframework.data.jpa.repository.Query(
        value = "UPDATE purchase_orders SET enabled = true WHERE id = :id AND company_id = :companyId",
        nativeQuery = true)

...
public interface PurchaseOrderJpaRepository
    ...
    @org.springframework.data.jpa.repository.Query(value = "SELECT * FROM purchase_orders WHERE company_id = :companyId AND enabled = false "
            + "ORDER BY order_date DESC, created_date DESC", nativeQuery = true)
    List<PurchaseOrderJpaEntity> findAllDisabledByCompany_Id(
            @org.springframework.data.repository.query.Param("companyId") Long companyId);

    @org.springframework.data.jpa.repository.Modifying(flushAutomatically = true, clearAutomatically = true)
    @org.springframework.transaction.annotation.Transactional
    @org.springframework.data.jpa.repository.Query(value = "UPDATE purchase_orders SET enabled = true WHERE id = :id AND company_id = :companyId", nativeQuery = true)

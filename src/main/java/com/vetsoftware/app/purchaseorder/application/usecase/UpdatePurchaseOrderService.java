...
public class UpdatePurchaseOrderService implements UpdatePurchaseOrderUseCase {
  ...
  public UpdatePurchaseOrderService(
  ...
  public PurchaseOrderDto execute(UpdatePurchaseOrderCommand command) {
    ...
    BranchRef branch =
        branchQueryPort
            .findById(command.branchId(), command.companyId())
            .orElseThrow(
  ...
  private List<PurchaseOrderLine> toLines(
      List<PurchaseOrderLineCommand> lineCommands, Long companyId) {
    ...
    return lineCommands.stream()
        .map(
            l -> {
              ProductRef product =
                  productQueryPort
                      .findById(l.productId(), companyId)
                      .orElseThrow(
                          () ->

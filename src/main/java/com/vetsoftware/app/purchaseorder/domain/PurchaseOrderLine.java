...
/**
 * Value object dentro del agregado {@link PurchaseOrder}: una línea de producto pedido. Es mutable en el
 * único aspecto que evoluciona tras emitir la orden — {@code quantityReceived}, que crece con cada recepción
 * de mercancía. El resto de campos (producto, cantidad pedida, costo) son inmutables una vez creada la línea.
 */
public class PurchaseOrderLine {
    ...
    public PurchaseOrderLine(Long id, ProductRef product, int quantityOrdered, BigDecimal unitCost,
    ...
    public static PurchaseOrderLine create(ProductRef product, int quantityOrdered, BigDecimal unitCost) {
        return new PurchaseOrderLine(null, product, quantityOrdered, unitCost, 0);
    }

    private static void validate(ProductRef product, int quantityOrdered, BigDecimal unitCost, int quantityReceived) {
        if (product == null) throw new IllegalArgumentException("line product is required");
        if (quantityOrdered <= 0) throw new IllegalArgumentException("quantityOrdered must be greater than zero");
        ...
        if (quantityReceived < 0) throw new IllegalArgumentException("quantityReceived cannot be negative");
    }
    ...
    void revertReceive(int quantity) {
        if (quantity <= 0) throw new IllegalArgumentException("reverted quantity must be greater than zero");

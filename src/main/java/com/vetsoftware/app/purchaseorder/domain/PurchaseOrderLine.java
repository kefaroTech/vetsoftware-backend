...
/**
 * Value object dentro del agregado {@link PurchaseOrder}: una línea de producto
 * pedido. Es mutable en el único aspecto que evoluciona tras emitir la orden —
 * {@code quantityReceived}, que crece con cada recepción de mercancía. El resto
 * de campos (producto, cantidad pedida, costo) son inmutables una vez creada la
 * línea.
 */
public class PurchaseOrderLine {
    ...
    public static PurchaseOrderLine create(ProductRef product, int quantityOrdered,
            BigDecimal unitCost) {
        return new PurchaseOrderLine(null, product, quantityOrdered, unitCost, 0);
    }

    private static void validate(ProductRef product, int quantityOrdered, BigDecimal unitCost,
            int quantityReceived) {
        if (product == null)
            throw new IllegalArgumentException("line product is required");
    ...
    /**
     * Incrementa lo recibido; rechaza cantidades no positivas o que excedan lo
     * pendiente.
     */
    void receive(int quantity) {
        ...
        if (quantity > pendingQuantity()) {
            throw new IllegalArgumentException("received quantity " + quantity

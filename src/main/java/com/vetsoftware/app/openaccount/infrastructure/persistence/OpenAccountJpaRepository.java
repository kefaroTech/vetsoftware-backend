...
public interface OpenAccountJpaRepository
    extends JpaRepository<OpenAccountJpaEntity, Long>,
  ...
  // Multi-sucursal (Fase C): lista de la empresa con filtro OPCIONAL por sede (branch es
  // @ManyToOne).
  // branchId null = todas las sedes.
  @EntityGraph(attributePaths = {"owner", "company", "branch", "createdBy"})
  @Query(
      "SELECT o FROM OpenAccountJpaEntity o WHERE o.company.id = :companyId "
          + "AND (:branchId IS NULL OR o.branch.id = :branchId)")
  List<OpenAccountJpaEntity> findByCompanyIdAndOptionalBranch(
      @Param("companyId") Long companyId, @Param("branchId") Long branchId);

  // Bloqueo pesimista de la fila de la cuenta para serializar el recálculo de totales bajo
  // concurrencia
  // (cargos/abonos simultáneos). Sin @EntityGraph a propósito: FOR UPDATE no combina con
  // join-fetch; las
  ...
  // evitando bloquear (o leer) una cuenta de otro tenant. Mismo motivo del sin-@EntityGraph que
  // arriba.
  @Lock(LockModeType.PESSIMISTIC_WRITE)
  @Query("select o from OpenAccountJpaEntity o where o.id = :id and o.company.id = :companyId")
  Optional<OpenAccountJpaEntity> findByIdForUpdateAndCompanyId(
  ...
  @org.springframework.data.jpa.repository.Query(
      value = "UPDATE open_accounts SET enabled = true WHERE id = :id",
      nativeQuery = true)

...
public class AssertOpenAccountVersionService implements AssertOpenAccountVersionUseCase {
    ...
    public void assertVersion(Long companyId, Long openAccountId, Long expectedVersion) {
        // Opt-in: sin versión esperada no hay chequeo temprano (se conserva el
        // optimistic lock al
        // flush).
        if (expectedVersion == null)
            return;
        // Lectura scoped a la empresa: una cuenta ajena lanza NotFound en vez de
        // comparar/filtrar su
        // versión.
        ...
        if (!expectedVersion.equals(account.getVersion())) {
            throw new OpenAccountVersionConflictException(openAccountId, expectedVersion,
                    account.getVersion());
        }
    }
}

...
public class ChangeOpenAccountStatusService implements ChangeOpenAccountStatusUseCase {
  ...
  public ChangeOpenAccountStatusService(
      OpenAccountRepository repository,
  ...
  public OpenAccountDto execute(ChangeOpenAccountStatusCommand command) {
    ...
    // concurrentes (un cargo no se puede colar mientras se cierra), cerrando el TOCTOU sobre el
    // estado y
    // el saldo. Bajo el lock solo se persiste el documento PENDIENTE; la transmisión a la DIAN +
    // entrega
    ...
    // Cobro/cierre de la venta: construye+persiste el documento PENDIENTE bajo el lock (un fallo de
    // config
    // como "sin perfil fiscal" que valida el builder hace fallar el cierre atómicamente) y programa
    // la
    // transmisión a la DIAN + entrega para DESPUÉS del commit, ya sin el lock (ver A1 /
    // EmitOnClose).

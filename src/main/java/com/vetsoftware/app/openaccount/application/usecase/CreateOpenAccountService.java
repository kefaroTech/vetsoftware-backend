...
public class CreateOpenAccountService implements CreateOpenAccountUseCase {
    ...
    public CreateOpenAccountService(OpenAccountRepository repository, OwnerQueryPort ownerQueryPort,
            CompanyQueryPort companyQueryPort, EmployeeQueryPort employeeQueryPort,
    ...
    // NOTA (regla de negocio): una cuenta abierta no debe existir sin cargos. Como
    // los
    // cargos (product/service/general charge) son features aparte que referencian
    // la cuenta
    // ya creada, esta invariante la GARANTIZA EL FLUJO DEL FRONT (siempre crea
    // cuenta + ≥1
    // cargo juntos; nunca llama a este create de forma aislada sin cargos a
    // continuación).
    // El back no la enforza atómicamente para no acoplar openaccount con las
    // features de cargos.
    // Si un fallo parcial deja una cuenta vacía, el get-or-create de abajo la
    // REUTILIZA (no se
    // crea otra ni bloquea el cupo de 1-por-dueño-y-sede).
    @Override
    public OpenAccountDto execute(CreateOpenAccountCommand command) {
        // La cuenta se administra por sede: primero se resuelve la sede solicitada y el
        // get-or-create
        // se limita a (empresa, sede, propietario). El mismo propietario puede tener
        // una cuenta OPEN

...
public interface OpenAccountRepository {
  ...
  /**
   * Igual que {@link #findByIdForUpdate} pero scoped a la empresa: el FOR UPDATE solo bloquea la
   * fila si pertenece a {@code companyId}. Evita que un caller (o bug futuro) tome el lock
   * pesimista sobre una cuenta ajena (defensa en profundidad contra IDOR / DoS por lock
   * cross-tenant).
   */

...
public class CreateQuoteService implements CreateQuoteUseCase {
    ...
    private List<QuoteLine> freezeLines(CreateQuoteCommand command, PriceListRef priceList,
        ...
        for (QuoteLineCommand requested : command.lines()) {
            ...
            CatalogPriceRef price = catalogPriceQueryPort
                    .findApplicable(priceList.id(), requested.catalogItemId(), billingCycle,
                            requested.quantity())
                    .orElseThrow(() -> new IllegalArgumentException("No price for catalog item "
                            + requested.catalogItemId() + " in price list " + priceList.id()
                            + " for cycle " + billingCycle));
            // R15: lo incluido en la tarifa se resta antes de fijar la cantidad, y la
            // resta la hace el dominio -QuoteLine.billableQuantity- porque es quien
            // resuelve el precio quien tiene el included_quantity. Si lo contratado no
            // supera a lo incluido no hay nada que cobrar y la linea no se emite:
            // chk_quote_lines_quantity exige quantity > 0. Las tres cifras
            // -contratada, incluida y cobrada- quedan congeladas en la linea que si se
            // emite, para que la oferta se explique sin volver a la tarifa.
            int billable = QuoteLine.billableQuantity(item.itemType(), requested.quantity(),
                    price.includedQuantity());
            if (billable > 0) {
                lines.add(QuoteLine.freeze(lineNumber, item, price, requested.quantity(),
                        requested.discountPercent(), now));
                lineNumber++;
            }
        }
        return lines;

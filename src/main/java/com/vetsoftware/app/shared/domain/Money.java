 ...
 * sin Spring, sin FKs a otra feature) que centraliza el redondeo a escala monetaria y el desglose
 * de la base gravable. Existe para que las features de cargo (general/producto/servicio) y la
 * facturación electrónica no dupliquen la matemática del dinero. No agregar aquí lógica de negocio
 * de ninguna feature concreta: solo aritmética monetaria genérica.
 */
public final class Money {
  ...
  /**
   * Extrae la base gravable de un total CON impuesto incluido: {@code base = total / (1 +
   * pct/100)}. {@code pct} null o 0 → el total ya es la base (sin impuesto).
   */
  ...
  /**
   * Porcentaje sobre un monto: {@code amount · rate / 100} (2 dec, HALF_UP). Devuelve 0 si {@code
   * amount}/{@code rate} son null o {@code rate ≤ 0}.
   */
  ...
  /**
   * Por mil sobre un monto: {@code amount · rate / 1000} (2 dec, HALF_UP). Para tarifas expresadas
   * en ‰ (p. ej. la retención de ICA: 9,66 ‰ = 0,966 %). Devuelve 0 si {@code amount}/{@code rate}
   * son null o {@code rate ≤ 0}.
   */

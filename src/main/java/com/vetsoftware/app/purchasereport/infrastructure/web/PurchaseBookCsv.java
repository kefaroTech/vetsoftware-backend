...
/**
 * Serializa el libro de compras a CSV (UTF-8 con BOM para que Excel respete los
 * acentos).
 */
final class PurchaseBookCsv {
    ...
    static byte[] purchaseBook(PurchaseBookDto b) {
        ...
        row(sb, "Desde", b.dateFrom() == null ? "" : D.format(b.dateFrom()), "Hasta",
                b.dateTo() == null ? "" : D.format(b.dateTo()));
        row(sb);
        row(sb, "Proveedor", "NIT", "Factura", "Fecha", "Vence", "Base", "Impuesto", "Retención",
                "Total", "Pagado", "Saldo", "Estado");
        for (PurchaseBookDto.EntryDto e : b.entries()) {
            row(sb, e.supplierName(), nn(e.supplierTaxId()), e.invoiceNumber(),
                    e.issueDate() == null ? "" : D.format(e.issueDate()),
                    e.dueDate() == null ? "" : D.format(e.dueDate()), plain(e.subtotal()),
                    plain(e.taxAmount()), plain(e.withholdingAmount()), plain(e.total()),
                    plain(e.paidAmount()), plain(e.balance()), e.status());
        }
        PurchaseBookDto.TotalsDto t = b.totals();
        row(sb);
        row(sb, "TOTAL (" + t.invoiceCount() + ")", "", "", "", "", plain(t.subtotal()),
                plain(t.taxAmount()), plain(t.withholdingAmount()), plain(t.total()),
                plain(t.paidAmount()), plain(t.balance()), "");

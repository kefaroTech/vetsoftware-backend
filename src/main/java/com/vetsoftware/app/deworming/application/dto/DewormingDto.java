...
public record DewormingDto(Long id, LocalDate date, LocalDate lastDeworming, DewormingType type,
    ...
    public static DewormingDto from(Deworming deworming) {
        return new DewormingDto(deworming.getId(), deworming.getDate(),
                deworming.getLastDeworming(), deworming.getType(), deworming.getProduct(),
                deworming.getDosage(), deworming.getNextControl(), deworming.getObservations(),
                AnimalSummaryDto.from(deworming.getAnimal()),
                deworming.getConsultation() == null
                        ? null
                        : ConsultationSummaryDto.from(deworming.getConsultation()),
                CompanySummaryDto.from(deworming.getCompany()), deworming.getCreatedDate(),
                deworming.isEnabled());
    }
}

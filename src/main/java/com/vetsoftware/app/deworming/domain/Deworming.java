...
public class Deworming {
    ...
    public void update(LocalDate date, LocalDate lastDeworming, DewormingType type, String product,
            String dosage, LocalDate nextControl, String observations, AnimalRef animal,
    ...
    private static void validate(LocalDate date, DewormingType type, String product, String dosage,
        ...
        if (product.length() > 200)
            throw new IllegalArgumentException("product must be 200 chars or less");
        if (dosage == null || dosage.isBlank())
            throw new IllegalArgumentException("dosage is required");
        if (dosage.length() > 200)
            throw new IllegalArgumentException("dosage must be 200 chars or less");

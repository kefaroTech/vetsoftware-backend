...
public interface ProcedureScheduleJpaRepository
    ...
    @EntityGraph(attributePaths = {"hospitalizationProcedure", "createdBy"})
    List<ProcedureScheduleJpaEntity> findByHospitalizationProcedureHospitalizationId(Long hospitalizationId);

    @org.springframework.data.jpa.repository.Modifying(flushAutomatically = true, clearAutomatically = true)
    @org.springframework.transaction.annotation.Transactional
    @org.springframework.data.jpa.repository.Query(
        value = "UPDATE procedure_schedules SET enabled = false WHERE hospitalization_procedure_id = :procedureId AND enabled = true",
        nativeQuery = true)
    ...
    @org.springframework.transaction.annotation.Transactional
    @org.springframework.data.jpa.repository.Query(
        value = "UPDATE procedure_schedules SET enabled = false WHERE hospitalization_procedure_id = :procedureId AND enabled = true AND (applied_status IS NULL OR applied_status <> 'APPLIED')",
        nativeQuery = true)

...
public interface ProcedureScheduleJpaRepository
    ...
    @org.springframework.data.jpa.repository.Query(value = "UPDATE procedure_schedules SET enabled = false WHERE hospitalization_procedure_id ="
            + " :procedureId AND enabled = true", nativeQuery = true)
    ...
    @org.springframework.transaction.annotation.Transactional
    @org.springframework.data.jpa.repository.Query(value = "UPDATE procedure_schedules SET enabled = false WHERE hospitalization_procedure_id ="
            + " :procedureId AND enabled = true AND (applied_status IS NULL OR applied_status <>"
    ...
    @org.springframework.data.jpa.repository.Modifying(flushAutomatically = true, clearAutomatically = true)
    @org.springframework.transaction.annotation.Transactional
    @org.springframework.data.jpa.repository.Query(value = "UPDATE procedure_schedules SET enabled = true WHERE id = :id", nativeQuery = true)

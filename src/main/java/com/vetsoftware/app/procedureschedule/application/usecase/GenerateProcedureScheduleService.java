...
public class GenerateProcedureScheduleService implements GenerateProcedureScheduleUseCase {
  ...
  public GenerateProcedureScheduleService(
      ProcedureScheduleRepository repository,
  ...
  public List<ProcedureScheduleDto> execute(GenerateProcedureScheduleCommand command) {
    ProcedureOrderParams params =
        procedureQueryPort
            .findById(command.hospitalizationProcedureId())
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "Hospitalization procedure not found: "
    ...
    List<ProcedureSchedule> applied =
        repository.findByHospitalizationProcedureId(params.id()).stream()
    ...
    if (applied.isEmpty()) {
      ...
      LocalDateTime lastApplied =
          applied.stream()
              ...
              .orElse(null);
      List<ProcedureSchedule> pending =
          ProcedureScheduleGenerator.generatePending(

...
public record AnimalDto(
    ...
    public static AnimalDto from(Animal animal) {
        // weight es el peso ACTUAL derivado del último WeightRecord (nullable si el animal aún no tiene
        // registros). weightType refleja la unidad de ese último registro; si no hay, cae a la unidad
        // preferida del animal. La serie completa vive en GET /animals/{id}/weight-records.
        WeightType displayUnit = animal.getCurrentWeightUnit() != null
            ? animal.getCurrentWeightUnit() : animal.getWeightType();
        return new AnimalDto(
            ...
            animal.getGender(), displayUnit, animal.getAnimalType(),
            animal.getReproductiveState(),
            AnimalColorSummaryDto.from(animal.getColor()),
            animal.getBod(),
            animal.getCurrentWeight(), animal.getCurrentWeightMeasuredAt(),
            animal.getSize(), animal.isDeceased(), animal.getDeceasedDate(),

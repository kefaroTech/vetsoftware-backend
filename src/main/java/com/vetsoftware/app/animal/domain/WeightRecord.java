...
/**
 * Serie temporal del peso de un animal. Cada registro es un punto (valor +
 * unidad + fecha) con su origen (manual / consulta / hospitalización). El "peso
 * actual" del animal se deriva del último registro habilitado, por lo que un
 * WeightRecord —no un escalar en Animal— es la fuente de verdad.
 */
public class WeightRecord {
    ...
    public WeightRecord(Long id, AnimalRef animal, BigDecimal value, WeightType unit,
            LocalDate measuredAt, WeightSource source, Long sourceId, String note,
    ...
    public static WeightRecord create(AnimalRef animal, BigDecimal value, WeightType unit,
            LocalDate measuredAt, WeightSource source, Long sourceId, String note,
            CompanyRef company) {
        return new WeightRecord(null, animal, value, unit, measuredAt, source, sourceId, note,
                company, LocalDateTime.now(), true);
    }

    private static void validate(AnimalRef animal, BigDecimal value, WeightType unit,
            LocalDate measuredAt, WeightSource source, Long sourceId, String note,
            CompanyRef company) {
        if (animal == null)
        ...
        if (measuredAt == null)
            throw new IllegalArgumentException("measuredAt is required");

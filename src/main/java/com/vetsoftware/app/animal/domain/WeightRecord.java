...
public class WeightRecord {
    ...
    public static WeightRecord create(AnimalRef animal, BigDecimal value, WeightType unit,
                                      LocalDate measuredAt, WeightSource source, Long sourceId,
                                      String note, CompanyRef company) {
    ...
    private static void validate(AnimalRef animal, BigDecimal value, WeightType unit,
        ...
        if (measuredAt.isAfter(LocalDate.now())) throw new IllegalArgumentException("measuredAt cannot be in the future");
        ...
        if (note != null && note.length() > 500) throw new IllegalArgumentException("note must be 500 chars or less");

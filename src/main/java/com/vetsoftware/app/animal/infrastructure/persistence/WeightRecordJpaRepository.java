...
public interface WeightRecordJpaRepository extends JpaRepository<WeightRecordJpaEntity, Long> {
  ...
  List<WeightRecordJpaEntity> findByAnimal_IdAndCompany_IdOrderByMeasuredAtDescIdDesc(
      Long animalId, Long companyId);

  @EntityGraph(attributePaths = {"animal", "company"})
  Optional<WeightRecordJpaEntity> findTopByAnimal_IdAndCompany_IdOrderByMeasuredAtDescIdDesc(
      Long animalId, Long companyId);

  @EntityGraph(attributePaths = {"animal", "company"})
  Optional<WeightRecordJpaEntity> findByIdAndAnimal_IdAndCompany_Id(
  ...
  @Query(
      value =
          "SELECT w.animal_id AS animalId, w.weight_value AS value, w.weight_unit AS unit, "
      ...
      nativeQuery = true)
  Optional<LatestWeightProjection> findLatestByAnimalId(
      @Param("animalId") Long animalId, @Param("companyId") Long companyId);
  ...
  @Query(
      value =
          "SELECT w.animal_id AS animalId, w.weight_value AS value, w.weight_unit AS unit,"
              + " w.measured_at AS measuredAt FROM weight_records w WHERE w.company_id = :companyId"
              + " AND w.enabled = true AND w.animal_id IN (:animalIds) AND w.id = (SELECT w2.id"
              + " FROM weight_records w2 WHERE w2.animal_id = w.animal_id AND w2.enabled = true"
              + " ORDER BY w2.measured_at DESC, w2.id DESC LIMIT 1)",
      nativeQuery = true)
  List<LatestWeightProjection> findLatestByAnimalIds(
      @Param("companyId") Long companyId, @Param("animalIds") Collection<Long> animalIds);

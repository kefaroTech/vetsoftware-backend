...
public class JpaAnimalRepository implements AnimalRepository {
    ...
    public JpaAnimalRepository(AnimalJpaRepository jpaRepository, AnimalJpaMapper mapper,
            SpecieJpaRepository specieJpaRepository, BreedJpaRepository breedJpaRepository,
            OwnerJpaRepository ownerJpaRepository, CompanyJpaRepository companyJpaRepository,
    ...
    public Animal save(Animal animal) {
        ...
        AnimalColorJpaEntity color = animalColorJpaRepository
                .getReferenceById(animal.getColor().id());
        AnimalJpaEntity saved = jpaRepository
                .save(mapper.toJpa(animal, specie, breed, owner, company, color));
    ...
    public Optional<Animal> findById(Long id) {
        return jpaRepository.findById(id).map(mapper::toDomain)
                .map(a -> enrich(a, a.getCompany().id()));
    }
    ...
    public List<Animal> findAllByCompanyId(Long companyId) {
        return enrichAll(jpaRepository.findAllByCompany_Id(companyId).stream().map(mapper::toDomain)
                .toList());
    }

    @Override
    public List<Animal> findByOwnerIdAndCompanyId(Long ownerId, Long companyId) {
        return enrichAll(jpaRepository.findAllByOwner_IdAndCompany_Id(ownerId, companyId).stream()
    ...
    private Animal enrich(Animal animal, Long companyId) {
        weightRecordJpaRepository.findLatestByAnimalId(animal.getId(), companyId)
                .ifPresent(p -> animal.applyCurrentWeight(p.getValue(),
                        WeightType.valueOf(p.getUnit()), p.getMeasuredAt()));
        return animal;
    }

    // Una sola query por empresa (los animales pueden ser de distintas empresas en
    // findAll) → sin
    // N+1.
    private List<Animal> enrichAll(List<Animal> animals) {
        ...
        byCompany.forEach((companyId, group) -> {
            Map<Long, LatestWeightProjection> latest = weightRecordJpaRepository
                    ...
                    .stream().collect(Collectors.toMap(LatestWeightProjection::getAnimalId,
                            Function.identity()));
            group.forEach(a -> {
                LatestWeightProjection p = latest.get(a.getId());
                if (p != null)
                    a.applyCurrentWeight(p.getValue(), WeightType.valueOf(p.getUnit()),
                            p.getMeasuredAt());
            });
        });
        return animals;

...
public class JpaAnimalRepository implements AnimalRepository {
  ...
  public JpaAnimalRepository(
      AnimalJpaRepository jpaRepository,
  ...
  public Animal save(Animal animal) {
    ...
    AnimalJpaEntity saved =
        jpaRepository.save(mapper.toJpa(animal, specie, breed, owner, company, color));
    Animal domain =
        mapper.toDomain(
            saved,
            animal.getSpecie(),
  ...
  public List<Animal> findAllByCompanyId(Long companyId) {
    return enrichAll(
        jpaRepository.findAllByCompany_Id(companyId).stream().map(mapper::toDomain).toList());
  }

  @Override
  public List<Animal> findByOwnerIdAndCompanyId(Long ownerId, Long companyId) {
    return enrichAll(
        jpaRepository.findAllByOwner_IdAndCompany_Id(ownerId, companyId).stream()
            .map(mapper::toDomain)
            .toList());
  }
  ...
  private Animal enrich(Animal animal, Long companyId) {
    weightRecordJpaRepository
        .findLatestByAnimalId(animal.getId(), companyId)
        .ifPresent(
            p ->
                animal.applyCurrentWeight(
                    p.getValue(), WeightType.valueOf(p.getUnit()), p.getMeasuredAt()));
    return animal;
  }

  // Una sola query por empresa (los animales pueden ser de distintas empresas en findAll) → sin
  // N+1.
  private List<Animal> enrichAll(List<Animal> animals) {
    if (animals.isEmpty()) return animals;
    Map<Long, List<Animal>> byCompany =
        animals.stream().collect(Collectors.groupingBy(a -> a.getCompany().id()));
    byCompany.forEach(
        (companyId, group) -> {
          Map<Long, LatestWeightProjection> latest =
              weightRecordJpaRepository
                  .findLatestByAnimalIds(companyId, group.stream().map(Animal::getId).toList())
                  .stream()
                  .collect(
                      Collectors.toMap(LatestWeightProjection::getAnimalId, Function.identity()));
          group.forEach(
              a -> {
                LatestWeightProjection p = latest.get(a.getId());
                if (p != null)
                  a.applyCurrentWeight(
                      p.getValue(), WeightType.valueOf(p.getUnit()), p.getMeasuredAt());
              });
        });
    return animals;

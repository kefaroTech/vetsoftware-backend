...
public class AnimalController {
    ...
    public AnimalResponse update(@PathVariable Long id,
        ...
        return toResponse(updateUseCase.execute(new UpdateAnimalCommand(id, request.name(),
                request.code(), request.specieId(), request.breedId(), request.ownerId(),
                request.gender(), request.weightType(), request.animalType(),
                request.reproductiveState(), request.colorId(), request.bod(), request.size(),
                request.deceased(), request.deceasedDate(), authz.currentCompanyId())));
    }

...
public record MembershipDto(Long id, String name, MembershipStatus status, boolean mandatory,
    ...
    public static MembershipDto from(Membership membership) {
        return new MembershipDto(membership.getId(), membership.getName(), membership.getStatus(),
                membership.isMandatory(), membership.getCreatedDate(), membership.isEnabled());
    }
}

...
public class MembershipController {
  ...
  public MembershipController(
      CreateMembershipUseCase createUseCase,
      UpdateMembershipUseCase updateUseCase,
      FindMembershipUseCase findUseCase,
      ListMembershipsUseCase listUseCase,
  ...
  public MembershipResponse update(
      @PathVariable Long id, @Valid @RequestBody UpdateMembershipRequest request) {
    return toResponse(
        updateUseCase.execute(
            new UpdateMembershipCommand(
                id, request.name(), request.status(), request.mandatory())));
  }
  ...
  private MembershipResponse toResponse(MembershipDto dto) {
    return new MembershipResponse(
        dto.id(), dto.name(), dto.status(), dto.mandatory(), dto.createdDate(), dto.enabled());
  }
}

...
public class CreateLaboratoryTestService implements CreateLaboratoryTestUseCase {
  ...
  public CreateLaboratoryTestService(
  ...
  public LaboratoryTestDto execute(CreateLaboratoryTestCommand command) {
    LaboratoryTestTypeRef testType =
        testTypeQueryPort
            .findById(command.testTypeId())
            .orElseThrow(
                () ->
    ...
    EmployeeRef processedBy =
        command.processedById() == null
            ? null
            : employeeQueryPort
                .findById(command.processedById())
                .orElseThrow(
                    () ->
                        new IllegalArgumentException(
                            "Employee not found: " + command.processedById()));

    LaboratoryTestStatus initialStatus =
    ...
    LaboratoryTestPriority prioridad =
        command.prioridad() == null || command.prioridad().isBlank()
            ? LaboratoryTestPriority.NORMAL
            : LaboratoryTestPriority.valueOf(command.prioridad().toUpperCase());

    // Sede de la muestra: si el request trae branchId debe pertenecer a la empresa y estar ACTIVA;
    // si no, la
    ...
    Long branchId =
        command.branchId() != null
            ...
            : branchQueryPort
                .findDefaultActiveIdByCompanyId(command.companyId())
                .orElseThrow(
                    () ->
                        new IllegalArgumentException(

...
/**
 * Correo "recordar código" con la lista de cuentas del usuario. A diferencia de los demás correos,
 * este NO usa una plantilla de Resend: como el nº de cuentas es variable y Resend limita cada
 * variable a 2.000 caracteres, el listado se rompería. En su lugar renderizamos el HTML completo
 * aquí (plantilla en {@code resources/email-templates/recover-code.html}) y lo enviamos como cuerpo
 * HTML — sin ese límite. Async/best-effort (nunca lanza; si algo falla, se registra un warning y el
 * flujo continúa).
 */
@Component
public class ResendCodeRecoveryEmailSender implements CodeRecoveryEmailSender {
  ...
  // Fila del listado (usa las clases .acct-* del <style> de la plantilla). {DIV} = divisor salvo en
  // la primera.
  ...
  public ResendCodeRecoveryEmailSender(
      ResendEmailClient email, @Value("${vetsoftware.code-recovery.login-url:}") String loginUrl) {
    this.email = email;
  ...
  /**
   * Escapa el texto que se inyecta como HTML (nombres de veterinaria/códigos/nombre) para no romper
   * el markup.
   */

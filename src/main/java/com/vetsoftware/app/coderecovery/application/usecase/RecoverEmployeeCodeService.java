...
/**
 * "Recordar mi código": busca todas las cuentas activas y verificadas con ese correo y le envía sus
 * códigos de acceso (una fila por veterinaria). Anti-enumeración: si no hay coincidencias, termina
 * sin error y sin enviar nada. El código no es un secreto (es el usuario), pero solo se envía al
 * correo registrado.
 */
...
public class RecoverEmployeeCodeService implements RecoverEmployeeCodeUseCase {
  ...
  public RecoverEmployeeCodeService(
      EmployeeAccountsByEmailPort accountsByEmail, CodeRecoveryEmailSender emailSender) {
  ...
  public void execute(RecoverEmployeeCodeCommand command) {
    ...
    if (accounts.isEmpty()) {
      log.info(
          "Solicitud de recuperación de código para un correo sin cuentas elegibles (se ignora)");
      return;
    }

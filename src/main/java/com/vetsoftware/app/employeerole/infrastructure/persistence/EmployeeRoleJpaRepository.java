...
public interface EmployeeRoleJpaRepository extends JpaRepository<EmployeeRoleJpaEntity, Long> {
  ...
  // Roles para la pantalla de listado de empleados: los asignados vigentes (employee_role activo)
  // para
  // cualquiera, MÁS todos los del empleado desactivado aunque su employee_role esté deshabilitado
  // (así un
  // inactivo muestra "el rol que tenía"). Nativa para saltar los @SQLRestriction de ambas tablas.
  @org.springframework.data.jpa.repository.Query(
      value =
          "SELECT er.* FROM employee_roles er JOIN employees e ON e.id = er.employee_id "
  ...
  @org.springframework.data.jpa.repository.Query(
      value = "UPDATE employee_roles SET enabled = true WHERE id = :id",
      nativeQuery = true)
  int reactivate(@org.springframework.data.repository.query.Param("id") Long id);

  @org.springframework.data.jpa.repository.Query(
      value =
          "SELECT id FROM employee_roles WHERE employee_id = :employeeId AND role_id = :roleId AND"

...
/**
 * Lee los documentos electrónicos de la empresa (read-only) para los reportes de F6. Cruce
 * permitido de vertical slicing: la infraestructura de salesreport importa el
 * JpaRepository/JpaEntity de electronicdocument. Convierte enums del dominio ajeno a String en el
 * borde para no acoplar salesreport a ese dominio.
 */
@Component
public class JpaSalesDocumentQueryPort implements SalesDocumentQueryPort {
  ...
  @Override
  public List<SalesDocumentView> findByCompanyAndDateRange(
    ...
    return documentJpaRepository.findByCompanyIdAndOptionalBranch(companyId, branchId).stream()
        ...
        .sorted(
            java.util.Comparator.comparing(
                ElectronicDocumentJpaEntity::getIssueDate,
  ...
  private SalesDocumentView toView(ElectronicDocumentJpaEntity e) {
    ...
    List<PaymentLineView> paymentLines =
        e.getPayments().stream()
            .map(
                p ->
                    new PaymentLineView(
                        p.getPaymentMeans().name(), p.getPaymentMeans().dianCode(), p.getAmount()))
            .toList();
    return new SalesDocumentView(
        ...
        e.getConsecutive(),
        e.getIssueDate(),
        e.getCustomerDocumentId(),
        e.getCustomerName(),
        e.getDianStatus() == null ? null : e.getDianStatus().name(),
        e.getCufe(),
        e.getCude(),
        e.getLineExtensionAmount(),
        e.getTaxInclusiveAmount(),
        e.getPayableAmount(),
        e.getReteFuenteAmount(),
        e.getReteIvaAmount(),
        e.getReteIcaAmount(),
        taxLines,
        paymentLines);

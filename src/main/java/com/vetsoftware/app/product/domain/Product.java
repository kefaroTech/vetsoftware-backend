...
/**
 * Producto = ficha de catálogo (identidad, precio de venta, clasificación fiscal). El STOCK, el
 * MÍNIMO, el COSTO, el vencimiento y el lote NO viven aquí: son responsabilidad del inventario por
 * sede (feature {@code inventory}: stock_balance / stock_lot / stock_movement). Desde F2 el
 * producto no lleva columnas planas de existencias.
 */
      ...
      case GRAVADO, INC -> {
        ...
        if (tax.percentage().signum() <= 0)
          throw new IllegalArgumentException(
              "taxTreatment " + taxTreatment + " requires a tax percentage greater than 0");
      }

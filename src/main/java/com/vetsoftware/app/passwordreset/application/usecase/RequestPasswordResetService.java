...
/**
 * Solicitud de restablecimiento por código de empleado. Anti-enumeración: si el código no existe o la cuenta
...
public class RequestPasswordResetService implements RequestPasswordResetUseCase {
    ...
    public void execute(RequestPasswordResetCommand command) {
        ...
        // Anti-enumeración: no revelamos si el código existe. Solo elegibles con correo verificado reciben correo.
        if (account == null || !account.emailVerified()) {
            log.info("Solicitud de restablecimiento para un código no elegible (se ignora silenciosamente)");
            return;
        }
        ...
        tokenRepository.save(PasswordResetToken.issue(
            account.id(), account.companyId(), PasswordResetTokens.hash(rawToken),

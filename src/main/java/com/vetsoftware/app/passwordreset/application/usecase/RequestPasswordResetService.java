...
/**
 * Solicitud de restablecimiento por código de empleado. Anti-enumeración: si el código no existe o
 * la cuenta no es elegible (correo sin verificar), termina sin error y sin enviar nada. Si es
 * elegible, invalida los tokens previos, guarda el HASH de un token nuevo y envía el enlace por
 * correo (async/best-effort).
 */
...
public class RequestPasswordResetService implements RequestPasswordResetUseCase {
  ...
  public RequestPasswordResetService(
      EmployeeAccountLookupPort employeeLookup,
  ...
  public void execute(RequestPasswordResetCommand command) {
    ...
    // Anti-enumeración: no revelamos si el código existe. Solo elegibles con correo verificado
    // reciben correo.
    ...
    tokenRepository.save(
        PasswordResetToken.issue(
            account.id(),
            account.companyId(),
            PasswordResetTokens.hash(rawToken),

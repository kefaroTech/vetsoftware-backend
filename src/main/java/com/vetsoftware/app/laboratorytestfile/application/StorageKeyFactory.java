...
/**
 * Construye la clave (ruta dentro del bucket) de un LaboratoryTestFile
 * siguiendo la regla:
...
public final class StorageKeyFactory {
    ...
    public static String build(LaboratoryTestStoragePathRef path, String originalFileName) {
        return path.companyId() + "/" + path.ownerId() + "/" + slug(path.animalName()) + "-"
                + path.animalId() + "/" + UUID.randomUUID() + "-"
                + sanitizeFileName(originalFileName);
    }

    private static String slug(String input) {
        String normalized = Normalizer.normalize(input, Normalizer.Form.NFD)
                .replaceAll("\\p{M}+", "") // quita
                                           // diacríticos
                .toLowerCase().replaceAll("[^a-z0-9]+", "-") // todo lo no alfanumérico -> '-'

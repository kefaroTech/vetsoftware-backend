...
public class CreateLaboratoryTestFileService implements CreateLaboratoryTestFileUseCase {
  ...
  public CreateLaboratoryTestFileService(
      LaboratoryTestFileRepository repository,
  ...
  public LaboratoryTestFileDto execute(CreateLaboratoryTestFileCommand command) {
    ...
    EmployeeRef uploadedBy =
        employeeQueryPort
            .findById(command.uploadedById())
            .orElseThrow(
                () ->
                    new IllegalArgumentException("Employee not found: " + command.uploadedById()));

    LaboratoryTestStoragePathRef storagePath =
        laboratoryTestQueryPort

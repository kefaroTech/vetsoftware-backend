...
public class DebtOpenAccount {
    ...
    /**
     * Idempotency key (UUID del cliente): deduplica reintentos del mismo cobro.
    ...
    /**
     * Anula el abono dejando la fila visible (no toca {@code enabled}): registra
     * quién lo anuló, cuándo y el motivo obligatorio. Un abono ya anulado no puede
     * volver a anularse.
     */
    public void voidPayment(EmployeeRef voidedBy, String reason) {
        if (this.voided)
            throw new DebtOpenAccountAlreadyVoidedException(this.id);
        if (voidedBy == null)
    ...
    private static void validate(BigDecimal amount, PaymentMethod paymentMethod,

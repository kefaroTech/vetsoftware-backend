...
public class VoidDebtOpenAccountService implements VoidDebtOpenAccountUseCase {
  ...
  public VoidDebtOpenAccountService(
  ...
  public DebtOpenAccountDto execute(VoidDebtOpenAccountCommand command) {
    ...
    // Lock pesimista de la cuenta antes de leer su estado: serializa la anulación del abono frente
    // a
    ...
    // La anulación mueve dinero en la caja propia del actor y exige que esté abierta en la sede de
    // la cuenta.
    ...
    // Compensa el abono en la caja OPEN del actor (VOID_OUT). voidPayment ya garantizó que no
    // estaba anulado.
    cashPort.reversePayment(
        command.companyId(),
        openAccountId,
        saved.getId(),

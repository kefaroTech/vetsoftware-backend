...
public interface DebtOpenAccountJpaRepository
    ...
    @org.springframework.data.jpa.repository.Query(value = "UPDATE debt_open_accounts SET enabled = true WHERE id = :id "
            + "AND EXISTS (SELECT 1 FROM open_accounts oa "
            + "WHERE oa.id = debt_open_accounts.open_account_id AND oa.company_id = :companyId)", nativeQuery = true)
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(d.amount), 0) FROM DebtOpenAccountJpaEntity d "

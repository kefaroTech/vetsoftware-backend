...
/**
 * Adapter de orquestación cuenta abierta → caja. Único punto de esta feature que conoce el {@code
 * CashLedgerUseCase} de {@code cashregister}; resuelve la sede de la cuenta (vía la persistencia de
 * {@code openaccount}) y traduce el {@link PaymentMethod} al medio de caja ({@link
 * CashPaymentMethod}), registrando el ingreso/compensación con referencia {@link
 * CashReferenceType#OPEN_ACCOUNT_PAYMENT} y el id del abono (idempotencia + compensación).
 */
...
public class CashRegisterAdapter implements CashPort {
  ...
  public CashRegisterAdapter(
      CashLedgerUseCase cashLedger, OpenAccountJpaRepository openAccountJpaRepository) {
  ...
  public void registerPayment(
    ...
    cashLedger.registerInflow(
        new RegisterCashInflowCommand(
            ...
            paymentId,
            List.of(new CashPaymentLine(toCashMethod(method), amount)),
            employeeId));
  }

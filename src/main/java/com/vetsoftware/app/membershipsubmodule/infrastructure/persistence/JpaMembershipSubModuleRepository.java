...
public class JpaMembershipSubModuleRepository implements MembershipSubModuleRepository {
  ...
  public JpaMembershipSubModuleRepository(
      MembershipSubModuleJpaRepository jpaRepository,
  ...
  public MembershipSubModule save(MembershipSubModule membershipSubModule) {
    MembershipJpaEntity membership =
        membershipJpaRepository.getReferenceById(membershipSubModule.getMembership().id());
    SubModuleJpaEntity subModule =
        subModuleJpaRepository.getReferenceById(membershipSubModule.getSubModule().id());
    MembershipSubModuleJpaEntity saved =
        jpaRepository.save(mapper.toJpa(membershipSubModule, membership, subModule));
    return mapper.toDomain(
        saved, membershipSubModule.getMembership(), membershipSubModule.getSubModule());
  }
  ...
  @Override
  public Optional<Long> findDisabledIdByMembershipAndSubModule(
      Long membershipId, Long subModuleId) {

...
public record SupplierInvoicePaymentDto(Long id, BigDecimal amount, LocalDate paymentDate,
    ...
    public static SupplierInvoicePaymentDto from(SupplierInvoicePayment p) {
        return new SupplierInvoicePaymentDto(p.getId(), p.getAmount(), p.getPaymentDate(),
                p.getMethod(), p.getReference(), p.getNote(), p.getCreatedDate(), p.getCreatedBy());
    }
}

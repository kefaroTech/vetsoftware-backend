...
public interface SupplierInvoiceRepository {
  ...
  /**
   * Facturas con saldo pendiente (PENDING/PARTIAL) para el aging; filtra por sede si {@code
   * branchId} != null.
   */
  List<SupplierInvoice> findOutstandingByCompany(Long companyId, Long branchId);

  /**
   * ¿Ya existe una factura activa con ese número para ese proveedor en la empresa? (evita doble
   * registro).
   */

...
/** Abono (pago total o parcial) aplicado a una factura de proveedor. Append-only: nunca se edita ni borra. */
public class SupplierInvoicePayment {
    ...
    public SupplierInvoicePayment(Long id, BigDecimal amount, LocalDate paymentDate,
        ...
        if (method == null) throw new IllegalArgumentException("payment method is required");
        if (reference != null && reference.length() > 80) throw new IllegalArgumentException("reference must be 80 chars or less");
        if (note != null && note.length() > 300) throw new IllegalArgumentException("note must be 300 chars or less");
        this.id = id;
        this.amount = amount;

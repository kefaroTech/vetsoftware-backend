...
/**
 * Factura de proveedor = documento de compra recibido de un tercero + su cuenta por pagar (agregado
 * company-scoped imputado a una sede). Nace en PENDING; los abonos (append-only) la mueven a
 * PARTIAL/PAID; puede anularse solo mientras no tenga abonos. Los montos fiscales
 * (base/impuesto/retención) se capturan a nivel cabecera para alimentar el libro de compras (F4);
 * el total bruto se calcula internamente = base + impuesto.
 *
 * <p>Semántica de dinero:
 *
 * <ul>
 *   <li>{@code total} (bruto) = {@code subtotal} + {@code taxAmount}.
 *   <li>{@code payableAmount} (lo que se le paga al proveedor) = {@code total} − {@code
 *       withholdingAmount}.
 *   <li>{@code balance} (saldo por pagar) = {@code payableAmount} − {@code paidAmount}.
 * </ul>
 */
public class SupplierInvoice {
  ...
  public SupplierInvoice(
      ...
      String notes,
      List<SupplierInvoicePayment> payments,
  ...
  public static SupplierInvoice create(
      CompanyRef company,
      BranchRef branch,
      SupplierRef supplier,
      Long purchaseOrderId,
  ...
  /**
   * Registra un abono y recalcula el estado (PARTIAL/PAID). Rechaza abonos sobre facturas
   * anuladas/pagadas o sobrepagos.
   */
  public void registerPayment(SupplierInvoicePayment payment, Long actorId, Long expectedVersion) {
    ...
    this.status =
        paidAmount().compareTo(payableAmount()) >= 0
  ...
  public BigDecimal paidAmount() {
    return payments.stream()
        .map(SupplierInvoicePayment::getAmount)

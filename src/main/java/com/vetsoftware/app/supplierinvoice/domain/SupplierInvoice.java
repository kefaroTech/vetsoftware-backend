...
/**
 * Factura de proveedor = documento de compra recibido de un tercero + su cuenta por pagar (agregado company-scoped
 * imputado a una sede). Nace en PENDING; los abonos (append-only) la mueven a PARTIAL/PAID; puede anularse solo
 * mientras no tenga abonos. Los montos fiscales (base/impuesto/retención) se capturan a nivel cabecera para
 * alimentar el libro de compras (F4); el total bruto se calcula internamente = base + impuesto.
 *
 * <p>Semántica de dinero:
 * <ul>
 *   <li>{@code total} (bruto) = {@code subtotal} + {@code taxAmount}.</li>
 *   <li>{@code payableAmount} (lo que se le paga al proveedor) = {@code total} − {@code withholdingAmount}.</li>
 *   <li>{@code balance} (saldo por pagar) = {@code payableAmount} − {@code paidAmount}.</li>
 * </ul>
 */
public class SupplierInvoice {
    ...
    public SupplierInvoice(Long id, CompanyRef company, BranchRef branch, SupplierRef supplier,
                           ...
                           SupplierInvoiceStatus status, String notes, List<SupplierInvoicePayment> payments,
    ...
    public static SupplierInvoice create(CompanyRef company, BranchRef branch, SupplierRef supplier,
                                         Long purchaseOrderId, Long goodsReceiptId, String invoiceNumber,
                                         LocalDate issueDate, LocalDate dueDate, BigDecimal subtotal,
                                         BigDecimal taxAmount, BigDecimal withholdingAmount, String notes,
                                         Long createdBy) {
        return new SupplierInvoice(null, company, branch, supplier, purchaseOrderId, goodsReceiptId, invoiceNumber,
            issueDate, dueDate, subtotal, taxAmount, withholdingAmount, SupplierInvoiceStatus.PENDING, notes,
            new ArrayList<>(), LocalDateTime.now(), createdBy, null, null, null, true);
    }

    /** Edición general: solo permitida mientras no tenga abonos (status PENDING). */
    public void update(BranchRef branch, SupplierRef supplier, Long purchaseOrderId, Long goodsReceiptId,
                       String invoiceNumber, LocalDate issueDate, LocalDate dueDate, BigDecimal subtotal,
                       BigDecimal taxAmount, BigDecimal withholdingAmount, String notes, Long updatedBy,
                       Long expectedVersion) {
        if (status != SupplierInvoiceStatus.PENDING) {
            throw new InvalidSupplierInvoiceStateException(
                "Supplier invoice can only be edited while PENDING (no payments), current status: " + status);
        }
        validate(company, branch, supplier, invoiceNumber, issueDate, dueDate, subtotal, taxAmount,
            withholdingAmount, status, notes);
        this.branch = branch;
    ...
    /** Registra un abono y recalcula el estado (PARTIAL/PAID). Rechaza abonos sobre facturas anuladas/pagadas o sobrepagos. */
    public void registerPayment(SupplierInvoicePayment payment, Long actorId, Long expectedVersion) {
    ...
    public void cancel(Long actorId, Long expectedVersion) {
        if (status != SupplierInvoiceStatus.PENDING) {
            throw new InvalidSupplierInvoiceStateException(
                "Only a PENDING invoice (without payments) can be cancelled, current status: " + status);
        }
    ...
    public BigDecimal payableAmount() { return total.subtract(withholdingAmount); }

    /** Suma de abonos aplicados. */
    public BigDecimal paidAmount() {
        return payments.stream().map(SupplierInvoicePayment::getAmount)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    }

    /** Saldo pendiente por pagar (0 si está pagada/anulada). */
    public BigDecimal balance() {
        if (status == SupplierInvoiceStatus.CANCELLED) return BigDecimal.ZERO;
        return payableAmount().subtract(paidAmount());
    }

    private static void validate(CompanyRef company, BranchRef branch, SupplierRef supplier, String invoiceNumber,
        ...
        if (invoiceNumber.length() > 60) throw new IllegalArgumentException("invoiceNumber must be 60 chars or less");
        ...
        if (dueDate.isBefore(issueDate)) throw new IllegalArgumentException("dueDate cannot be before issueDate");
        if (subtotal == null || subtotal.signum() < 0) throw new IllegalArgumentException("subtotal cannot be negative");
        if (taxAmount == null || taxAmount.signum() < 0) throw new IllegalArgumentException("taxAmount cannot be negative");
        if (withholdingAmount == null || withholdingAmount.signum() < 0) throw new IllegalArgumentException("withholdingAmount cannot be negative");
    ...
    public List<SupplierInvoicePayment> getPayments() { return List.copyOf(payments); }

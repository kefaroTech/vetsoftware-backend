...
/**
 * Facturas de proveedor y cuentas por pagar (punto 7 / F3). CRUD company-scoped + abonos parciales
 * + reporte de antigüedad (aging). La sede se resuelve por el alcance del empleado ({@link
 * Authz#resolveAccessibleBranch}).
 */
@RestController
...
public class SupplierInvoiceController {
  ...
  public SupplierInvoiceController(
      CreateSupplierInvoiceUseCase createUseCase,
  ...
  public SupplierInvoiceResponse create(@Valid @RequestBody CreateSupplierInvoiceRequest request) {
    return toResponse(
        createUseCase.execute(
            new CreateSupplierInvoiceCommand(
  ...
  public PageResponse<SupplierInvoiceResponse> search(
    ...
    PageResult<SupplierInvoiceDto> result =
        searchUseCase.execute(
            new SearchSupplierInvoicesCommand(
                authz.currentCompanyId(),
                supplierId,
  ...
  public AccountsPayableAgingResponse aging(
      ...
      @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
          LocalDate asOf) {
    return toAgingResponse(
        agingUseCase.get(authz.currentCompanyId(), authz.resolveAccessibleBranch(branchId), asOf));
  }
  ...
  public SupplierInvoiceResponse update(
      @PathVariable Long id, @Valid @RequestBody UpdateSupplierInvoiceRequest request) {
    return toResponse(
        updateUseCase.execute(
            new UpdateSupplierInvoiceCommand(
                id,
                authz.resolveAccessibleBranch(request.branchId()),
                request.supplierId(),
                request.purchaseOrderId(),
  ...
  @ResponseStatus(HttpStatus.CREATED)
  public SupplierInvoiceResponse registerPayment(
      @PathVariable Long id, @Valid @RequestBody RegisterSupplierPaymentRequest request) {
    return toResponse(
  ...
  public SupplierInvoiceResponse cancel(@PathVariable Long id) {
    return toResponse(
        cancelUseCase.execute(id, authz.currentCompanyId(), authz.currentEmployeeIdOrNull()));
  }
  ...
  private SupplierInvoicePaymentResponse toPaymentResponse(SupplierInvoicePaymentDto p) {
    return new SupplierInvoicePaymentResponse(
        p.id(),
        p.amount(),
        p.paymentDate(),
        p.method(),
  ...
  private AccountsPayableAgingResponse toAgingResponse(AccountsPayableAgingDto d) {
    return new AccountsPayableAgingResponse(
        ...
        d.suppliers().stream()
            .map(
                s ->
                    new AccountsPayableAgingResponse.SupplierRow(
                        s.supplierId(), s.supplierName(), s.taxId(), toBucket(s.bucket())))
            .toList(),
        toBucket(d.totals()));
  }

...
public class SupplierInvoiceJpaMapper {
  ...
  /**
   * Write path: arma la entidad reusando los proxies JPA (getReferenceById) que le pasa el
   * repositorio.
   */
  public SupplierInvoiceJpaEntity toJpa(
        ...
        e.getPayments().stream()
            .sorted(
                java.util.Comparator.comparing(
                    SupplierInvoicePaymentJpaEntity::getId,
  ...
  /**
   * Write-return path: rebuild del dominio reusando los Ref precargados del agregado original
   * (evita hidratar los proxies getReferenceById), tomando del entity persistido los ids generados
   * (factura + abonos, en orden).
   */
  public SupplierInvoice toDomainReusingRefs(
      SupplierInvoiceJpaEntity saved, SupplierInvoice original) {

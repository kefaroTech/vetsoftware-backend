...
public interface ListDayCaresByAnimalUseCase {
    @PreAuthorize("hasRole('SYSTEM') or (hasAuthority('dayCare.read') and @authz.isMyCompany(#companyId))")
    PageResult<DayCareDto> listByAnimal(Long animalId, Long companyId, String query, int page,
            int pageSize);
}

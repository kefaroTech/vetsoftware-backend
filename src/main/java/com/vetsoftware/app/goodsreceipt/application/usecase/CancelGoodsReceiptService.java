...
/**
 * Cancela una recepción CONFIRMED: revierte las entradas de inventario y, si venía de una orden de
 * compra, revierte lo recibido en ella; luego marca CANCELLED.
 */
...
public class CancelGoodsReceiptService implements CancelGoodsReceiptUseCase {
  ...
  public CancelGoodsReceiptService(
  ...
  public GoodsReceiptDto execute(Long id, Long companyId, Long actorId) {
    ...
    if (receipt.getStatus() != GoodsReceiptStatus.CONFIRMED) {
      throw new InvalidGoodsReceiptStatusTransitionException(
          receipt.getStatus(), GoodsReceiptStatus.CANCELLED);
    }
    ...
    if (receipt.getPurchaseOrderId() != null) {
      List<ReceivedLine> receivedLines =
          receipt.getLines().stream()
              ...
              .toList();
      purchaseOrderReceiving.revertReceipt(

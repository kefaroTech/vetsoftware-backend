...
/**
 * Recepción de mercancía = ingreso físico de bienes de un proveedor a una sede (con o sin orden de compra previa).
 * Agregado company-scoped con líneas por producto/lote. Se crea en DRAFT (editable); al CONFIRMAR afecta el
 * inventario (kardex por sede) y, si viene de una orden de compra, registra lo recibido en ella; al CANCELAR se
 * revierte todo. La transición de estado es la guarda de idempotencia frente al ledger de inventario.
 */
public class GoodsReceipt {
    ...
    public GoodsReceipt(Long id, CompanyRef company, BranchRef branch, SupplierRef supplier, Long purchaseOrderId,
                        LocalDate receiptDate, String supplierInvoiceNumber, String notes, GoodsReceiptStatus status,
                        List<GoodsReceiptLine> lines, LocalDateTime createdDate, Long createdBy,
                        LocalDateTime updatedDate, Long updatedBy, Long version, boolean enabled) {
        validate(company, branch, supplier, receiptDate, supplierInvoiceNumber, notes, status, lines);
        this.id = id;
    ...
    public static GoodsReceipt create(CompanyRef company, BranchRef branch, SupplierRef supplier,
                                      Long purchaseOrderId, LocalDate receiptDate, String supplierInvoiceNumber,
                                      String notes, List<GoodsReceiptLine> lines, Long createdBy) {
        return new GoodsReceipt(null, company, branch, supplier, purchaseOrderId, receiptDate,
            supplierInvoiceNumber, notes, GoodsReceiptStatus.DRAFT, lines,
            LocalDateTime.now(), createdBy, null, null, null, true);
    }

    /** Edición general: solo permitida mientras la recepción esté en DRAFT. */
    public void update(BranchRef branch, SupplierRef supplier, Long purchaseOrderId, LocalDate receiptDate,
                       String supplierInvoiceNumber, String notes, List<GoodsReceiptLine> lines,
    ...
    public void confirm(Long actorId) {
        if (status != GoodsReceiptStatus.DRAFT) {
            throw new InvalidGoodsReceiptStatusTransitionException(status, GoodsReceiptStatus.CONFIRMED);
        }
    ...
    public void cancel(Long actorId) {
        if (status != GoodsReceiptStatus.CONFIRMED) {
            throw new InvalidGoodsReceiptStatusTransitionException(status, GoodsReceiptStatus.CANCELLED);
        }
    ...
    private static void validate(CompanyRef company, BranchRef branch, SupplierRef supplier, LocalDate receiptDate,
                                 String supplierInvoiceNumber, String notes, GoodsReceiptStatus status,
        ...
        if (receiptDate == null) throw new IllegalArgumentException("receiptDate is required");

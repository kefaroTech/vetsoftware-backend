...
/**
 * Línea de una recepción de mercancía: qué producto entró, en qué
 * lote/vencimiento, cuánto y a qué costo. {@code purchaseOrderLineId} enlaza
 * (opcionalmente) a la línea de la orden de compra para recepción parcial.
 */
public class GoodsReceiptLine {
    ...
    public static GoodsReceiptLine create(ProductRef product, Long purchaseOrderLineId,
            String lotNumber, LocalDate expireDate, int quantityReceived, BigDecimal unitCost) {
        return new GoodsReceiptLine(null, product, purchaseOrderLineId, lotNumber, expireDate,
                quantityReceived, unitCost);
    }

    private static void validate(ProductRef product, String lotNumber, int quantityReceived,
            BigDecimal unitCost) {
        if (product == null)
            throw new IllegalArgumentException("line product is required");
        if (lotNumber != null && lotNumber.length() > 60)
            throw new IllegalArgumentException("lotNumber must be 60 chars or less");

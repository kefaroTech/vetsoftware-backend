...
public interface PetshopCatalogUseCase {
  ...
  record BundleWrite(
      ...
      String unitMeasureCode,
      BigDecimal salePrice,
      List<BundleItemWrite> items,
      List<String> barcodes,
      Long expectedVersion) {}

  record BundleItemDto(
      Long presentationId,
  ...
  record BarcodeLookupDto(
      String barcode,
      SellableItemType itemType,
      Long itemId,
      String name,
      String unitMeasureCode,
      BigDecimal salePrice,
  ...
  @PreAuthorize(
      "hasRole('SYSTEM') or (hasAuthority('product.create') and @authz.isMyCompany(#companyId))")
  PresentationDto createPresentation(PresentationWrite command, Long companyId, Long actorId);

  @PreAuthorize(
      "hasRole('SYSTEM') or (hasAuthority('product.update') and @authz.isMyCompany(#companyId))")
  PresentationDto updatePresentation(
      Long id, PresentationWrite command, Long companyId, Long actorId);

  @PreAuthorize(
      "hasRole('SYSTEM') or (hasAuthority('product.delete') and @authz.isMyCompany(#companyId))")

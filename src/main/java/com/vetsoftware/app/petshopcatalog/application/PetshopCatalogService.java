...
public class PetshopCatalogService implements PetshopCatalogUseCase {
    ...
    public PetshopCatalogService(CompanyJpaRepository companies, ProductJpaRepository products,
            UnitMeasureCatalogJpaRepository units, ProductPresentationJpaRepository presentations,
            ProductBundleJpaRepository bundles, ProductBundleItemJpaRepository bundleItems,
    ...
    public List<PresentationDto> listPresentations(Long productId, Long companyId) {
        ...
        return presentations
                .findAllByCompany_IdAndProduct_IdOrderByDefaultPresentationDescNameAsc(companyId,
                        productId)
                .stream().map(this::toPresentationDto).toList();
    }

    @Override
    public PresentationDto createPresentation(PresentationWrite command, Long companyId,
            Long actorId) {
        ...
        if (presentations.existsByCompany_IdAndProduct_IdAndName(companyId, product.getId(),
                name)) {
            throw conflict("PRESENTATION_NAME_ALREADY_EXISTS",
                    "Ya existe una presentación con ese nombre para el producto.");
        }
        if (isDefault)
            clearPreviousDefault(companyId, product.getId(), null, actorId);

        ProductPresentationJpaEntity entity = ProductPresentationJpaEntity.create(company, product,
                name, requireUnit(unitCode), factor, price(command.salePrice()), isDefault,
                actorId);
        entity = presentations.save(entity);
        replacePresentationBarcodes(entity, company, command.barcodes(), actorId);
        if (isDefault)
            syncProductBase(product, entity);
        return toPresentationDto(entity);
    }

    @Override
    public PresentationDto updatePresentation(Long id, PresentationWrite command, Long companyId,
            Long actorId) {
        ...
        if (command.productId() != null
                && !command.productId().equals(entity.getProduct().getId())) {
        ...
        if (entity.isDefaultPresentation() && !requestedDefault) {
            throw conflict("DEFAULT_PRESENTATION_REQUIRED",
                    "Asigna otra presentación predeterminada antes de desmarcar esta.");
        }
        defaultFactor(requestedDefault, factor);
        if (presentations.existsByCompany_IdAndProduct_IdAndNameAndIdNot(companyId,
                entity.getProduct().getId(), name, id)) {
            throw conflict("PRESENTATION_NAME_ALREADY_EXISTS",
                    "Ya existe una presentación con ese nombre para el producto.");
        }
        if (requestedDefault) {
        ...
        entity.update(name, requireUnit(unitCode), factor, price(command.salePrice()),
                requestedDefault, actorId);
        replacePresentationBarcodes(entity, entity.getCompany(), command.barcodes(), actorId);
        if (requestedDefault)
            syncProductBase(entity.getProduct(), entity);
        return toPresentationDto(entity);
    }
    ...
    public void deletePresentation(Long id, Long expectedVersion, Long companyId) {
        ...
        if (entity.isDefaultPresentation()) {
            throw conflict("DEFAULT_PRESENTATION_REQUIRED",
                    "La presentación predeterminada no se puede eliminar.");
        }
        if (bundleItems.existsByCompany_IdAndPresentation_IdAndBundle_EnabledTrue(companyId, id)) {
            throw conflict("PRESENTATION_IN_ACTIVE_BUNDLE",
                    "La presentación pertenece a un combo activo.");
        }
    ...
    public List<BundleDto> listBundles(Long companyId) {
        ...
        return bundles.findAllByCompany_IdOrderByNameAsc(companyId).stream().map(this::toBundleDto)
                .toList();
    }
    ...
    public BundleDto createBundle(BundleWrite command, Long companyId, Long actorId) {
        ...
        ProductBundleJpaEntity bundle = ProductBundleJpaEntity.create(company, name, code,
                requireUnit(text(command.unitMeasureCode(), "unitMeasureCode", 10)),
                price(command.salePrice()), actorId);
    ...
    public BarcodeLookupDto findByBarcode(String barcode, Long companyId) {
        ...
        CatalogBarcodeJpaEntity found = catalogBarcodes
        ...
        if (found.getItemType() == SellableItemType.PRESENTATION) {
            ...
            return new BarcodeLookupDto(normalized, found.getItemType(), p.getId(),
                    p.getProduct().getName() + " - " + p.getName(), p.getUnitMeasure().getCode(),
                    p.getSalePrice(), p.getConversionFactor());
        }
        ProductBundleJpaEntity b = found.getBundle();
        return new BarcodeLookupDto(normalized, found.getItemType(), b.getId(), b.getName(),
                b.getUnitMeasure().getCode(), b.getSalePrice(), null);
    }

    private void clearPreviousDefault(Long companyId, Long productId, Long exceptId, Long actorId) {
        ProductPresentationJpaEntity previous = presentations
    ...
    private void syncProductBase(ProductJpaEntity product,
        ...
        for (BundleItemWrite item : items) {
            ...
            if (!presentationIds.add(item.presentationId())) {
                throw new IllegalArgumentException(
                        "A presentation cannot be repeated in the same bundle");
            }
    ...
    private void replaceBundleItems(ProductBundleJpaEntity bundle, CompanyJpaEntity company,
            List<BundleItemWrite> desired) {
        Map<Long, ProductPresentationJpaEntity> resolved = new HashMap<>();
        for (BundleItemWrite item : desired) {
            resolved.put(item.presentationId(),
                    requirePresentation(item.presentationId(), company.getId()));
        }
        bundleItems.deleteAllByCompany_IdAndBundle_Id(company.getId(), bundle.getId());
        bundleItems.flush();
        bundleItems.saveAll(desired.stream()
                .map(item -> ProductBundleItemJpaEntity.create(company, bundle,
                        resolved.get(item.presentationId()), item.quantity(), item.displayOrder()))
                .toList());
    }

    private void replacePresentationBarcodes(ProductPresentationJpaEntity presentation,
            CompanyJpaEntity company, List<String> values, Long actorId) {
        List<String> desired = barcodes(values);
        List<CatalogBarcodeJpaEntity> current = catalogBarcodes
                .findAllByCompany_IdAndPresentation_IdOrderByBarcode(company.getId(),
                        presentation.getId());
        replaceBarcodes(current, desired, company, barcode -> CatalogBarcodeJpaEntity
                .forPresentation(company, barcode, presentation, actorId));
    }

    private void replaceBundleBarcodes(ProductBundleJpaEntity bundle, CompanyJpaEntity company,
            List<String> values, Long actorId) {
        List<String> desired = barcodes(values);
        List<CatalogBarcodeJpaEntity> current = catalogBarcodes
                .findAllByCompany_IdAndBundle_IdOrderByBarcode(company.getId(), bundle.getId());
        replaceBarcodes(current, desired, company,
                barcode -> CatalogBarcodeJpaEntity.forBundle(company, barcode, bundle, actorId));
    }

    private void replaceBarcodes(List<CatalogBarcodeJpaEntity> current, List<String> desired,
            CompanyJpaEntity company,
            java.util.function.Function<String, CatalogBarcodeJpaEntity> factory) {
        Map<String, CatalogBarcodeJpaEntity> existing = current.stream()
                .collect(java.util.stream.Collectors.toMap(CatalogBarcodeJpaEntity::getBarcode,
                        item -> item));
        List<CatalogBarcodeJpaEntity> removed = current.stream()
                .filter(item -> !desired.contains(item.getBarcode())).toList();
        catalogBarcodes.deleteAll(removed);
        if (!removed.isEmpty())
            catalogBarcodes.flush();
        for (String barcode : desired) {
            if (existing.containsKey(barcode))
                continue;
            if (catalogBarcodes.existsByCompany_IdAndBarcode(company.getId(), barcode)) {
                throw conflict("BARCODE_ALREADY_EXISTS",
                        "El código de barras ya está asignado: " + barcode);
            }
    ...
    private PresentationDto toPresentationDto(ProductPresentationJpaEntity entity) {
        List<String> values = catalogBarcodes
                .findAllByCompany_IdAndPresentation_IdOrderByBarcode(entity.getCompany().getId(),
                        entity.getId())
                .stream().map(CatalogBarcodeJpaEntity::getBarcode).toList();
        return new PresentationDto(entity.getId(), entity.getProduct().getId(),
                entity.getProduct().getName(), entity.getName(), entity.getUnitMeasure().getCode(),
                entity.getConversionFactor(), entity.getSalePrice(), entity.isDefaultPresentation(),
                values, entity.getVersion());
    }

    private BundleDto toBundleDto(ProductBundleJpaEntity bundle) {
        List<BundleItemDto> items = bundleItems
                .findAllByCompany_IdAndBundle_IdOrderByDisplayOrderAsc(bundle.getCompany().getId(),
                        bundle.getId())
                .stream()
                .map(item -> new BundleItemDto(item.getPresentation().getId(),
                        item.getPresentation().getName(),
                        item.getPresentation().getProduct().getName(), item.getQuantity(),
                        item.getDisplayOrder()))
                .toList();
        List<String> values = catalogBarcodes
                .findAllByCompany_IdAndBundle_IdOrderByBarcode(bundle.getCompany().getId(),
                        bundle.getId())
                .stream().map(CatalogBarcodeJpaEntity::getBarcode).toList();
        return new BundleDto(bundle.getId(), bundle.getName(), bundle.getCode(),
                bundle.getUnitMeasure().getCode(), bundle.getSalePrice(), items, values,
                bundle.getVersion());
    }
    ...
    private ProductPresentationJpaEntity requirePresentation(Long id, Long companyId) {
        return presentations.findByIdAndCompany_Id(id, companyId)
    ...
    private ProductBundleJpaEntity requireBundle(Long id, Long companyId) {
        return bundles.findByIdAndCompany_Id(id, companyId)

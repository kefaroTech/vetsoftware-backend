...
public class PetshopCatalogService implements PetshopCatalogUseCase {
    ...
    public PresentationDto createPresentation(
        ...
        ProductPresentationJpaEntity entity = ProductPresentationJpaEntity.create(
            company, product, name, requireUnit(unitCode), factor,
            price(command.salePrice()), isDefault, actorId);
    ...
    public BundleDto createBundle(BundleWrite command, Long companyId, Long actorId) {
        ...
        ProductBundleJpaEntity bundle = ProductBundleJpaEntity.create(
            company, name, code,
    ...
    public BarcodeLookupDto findByBarcode(String barcode, Long companyId) {
        ...
        if (found.getItemType() == SellableItemType.PRESENTATION) {
            ...
            return new BarcodeLookupDto(normalized, found.getItemType(), p.getId(),
                ...
                p.getUnitMeasure().getCode(), p.getSalePrice(), p.getConversionFactor());
        }
        ProductBundleJpaEntity b = found.getBundle();
        return new BarcodeLookupDto(normalized, found.getItemType(), b.getId(),
            b.getName(), b.getUnitMeasure().getCode(), b.getSalePrice(), null);
    }

    private void clearPreviousDefault(
            Long companyId, Long productId, Long exceptId, Long actorId) {
    ...
    private void syncProductBase(ProductJpaEntity product, ProductPresentationJpaEntity presentation) {
    ...
    private void replaceBundleItems(
        ...
        bundleItems.saveAll(desired.stream()
            .map(item -> ProductBundleItemJpaEntity.create(
                company, bundle, resolved.get(item.presentationId()),
                item.quantity(), item.displayOrder()))
            .toList());
    }

    private void replacePresentationBarcodes(
            ProductPresentationJpaEntity presentation, CompanyJpaEntity company,
        ...
        replaceBarcodes(current, desired, company,
            barcode -> CatalogBarcodeJpaEntity.forPresentation(
                company, barcode, presentation, actorId));
    }

    private void replaceBundleBarcodes(
            ProductBundleJpaEntity bundle, CompanyJpaEntity company,
            List<String> values, Long actorId) {
        List<String> desired = barcodes(values);
        List<CatalogBarcodeJpaEntity> current =
    ...
    private void replaceBarcodes(
            List<CatalogBarcodeJpaEntity> current, List<String> desired,
    ...
    private PresentationDto toPresentationDto(ProductPresentationJpaEntity entity) {
        ...
        return new PresentationDto(
            entity.getId(), entity.getProduct().getId(), entity.getProduct().getName(),
            entity.getName(), entity.getUnitMeasure().getCode(), entity.getConversionFactor(),
            entity.getSalePrice(), entity.isDefaultPresentation(), values, entity.getVersion());
    }
    ...
    private BundleDto toBundleDto(ProductBundleJpaEntity bundle) {
        List<BundleItemDto> items = bundleItems
            ...
            .stream().map(item -> new BundleItemDto(
                item.getPresentation().getId(), item.getPresentation().getName(),
                item.getPresentation().getProduct().getName(),
                item.getQuantity(), item.getDisplayOrder()))
            .toList();
        List<String> values = catalogBarcodes

...
public class JpaSystemUserPermissionRepository implements SystemUserPermissionRepository {
  ...
  public JpaSystemUserPermissionRepository(
  ...
  public SystemUserPermission save(SystemUserPermission systemUserPermission) {
    SystemUserJpaEntity systemUser =
        systemUserJpaRepository.getReferenceById(systemUserPermission.getSystemUser().id());
    SystemPermissionJpaEntity systemPermission =
        systemPermissionJpaRepository.getReferenceById(
            systemUserPermission.getSystemPermission().id());
    SystemUserPermissionJpaEntity saved =
        jpaRepository.save(mapper.toJpa(systemUserPermission, systemUser, systemPermission));
    return mapper.toDomain(
        saved, systemUserPermission.getSystemUser(), systemUserPermission.getSystemPermission());
  }
  ...
  @Override
  public Optional<Long> findDisabledIdBySystemUserAndSystemPermission(
      Long systemUserId, Long systemPermissionId) {
    return jpaRepository.findDisabledIdBySystemUserAndSystemPermission(
        systemUserId, systemPermissionId);
  }
}

...
public interface SystemUserPermissionJpaRepository
  ...
  @org.springframework.data.jpa.repository.Query(
      value = "UPDATE system_user_permissions SET enabled = true WHERE id = :id",
      nativeQuery = true)
  int reactivate(@org.springframework.data.repository.query.Param("id") Long id);

  @org.springframework.data.jpa.repository.Query(
      value =
          "SELECT id FROM system_user_permissions WHERE system_user_id = :systemUserId AND"
              + " system_permission_id = :systemPermissionId AND enabled = false LIMIT 1",
      nativeQuery = true)
  Optional<Long> findDisabledIdBySystemUserAndSystemPermission(
      ...
      @org.springframework.data.repository.query.Param("systemPermissionId")

...
public interface GeneralChargeOpenAccountJpaRepository
  ...
  @EntityGraph(attributePaths = {"tax", "openAccount", "createdBy", "voidedBy"})
  Optional<GeneralChargeOpenAccountJpaEntity> findByIdAndOpenAccount_Company_Id(
  ...
  @org.springframework.data.jpa.repository.Query(
      value =
          "UPDATE general_charge_open_accounts SET enabled = true WHERE id = :id AND EXISTS (SELECT"
              + " 1 FROM open_accounts oa WHERE oa.id ="
              + " general_charge_open_accounts.open_account_id AND oa.company_id = :companyId)",
      nativeQuery = true)
  int reactivate(
  ...
  // persisted. enabled = true is filtered explicitly (no @SQLRestriction for aggregates).
  @org.springframework.data.jpa.repository.Query(
      "SELECT COALESCE(SUM(c.totalAmount), 0) FROM GeneralChargeOpenAccountJpaEntity c "

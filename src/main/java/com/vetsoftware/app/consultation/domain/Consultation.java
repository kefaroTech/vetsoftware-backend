...
public class Consultation {
    ...
    private static void validate(LocalDate date, ConsultationTypeRef consultationType,
        ...
        if (consultationType == null)
            throw new IllegalArgumentException("consultationType is required");
        if (anamnesis == null || anamnesis.isBlank())
            throw new IllegalArgumentException("anamnesis is required");
        if (anamnesis.length() > 2000)
            throw new IllegalArgumentException("anamnesis must be 2000 chars or less");
        // diagnosis y prognosis son clínicamente opcionales: se aceptan null/vacío y
        // solo se
        // valida el tope de longitud cuando vienen con contenido. El plan
        // diagnóstico/terapéutico
        // ya no es texto libre: vive en las acciones rápidas (laboratorio/imagen,
        // receta/procedimientos).
        if (diagnosis != null && diagnosis.length() > 2000)
            throw new IllegalArgumentException("diagnosis must be 2000 chars or less");
        if (prognosis != null && prognosis.length() > 500)
            throw new IllegalArgumentException("prognosis must be 500 chars or less");
        if (animal == null)
    ...
    public ConsultationTypeRef getConsultationType() {

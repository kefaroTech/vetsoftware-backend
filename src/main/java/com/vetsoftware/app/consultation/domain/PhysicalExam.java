...
/**
 * Value object del examen físico / constantes vitales de una consulta (la "O"
 * de SOAP). Todos los campos son opcionales (una consulta puede no registrar
 * examen). Las invariantes (rangos y longitudes) viven aquí, en el dominio. Un
 * examen "vacío" es válido.
 */
public record PhysicalExam(BigDecimal temperature, Integer heartRate, Integer respiratoryRate,
        String mucousMembranes, String capillaryRefill, String hydration,
        Integer bodyConditionScore, Integer painScore, String attitude, String findings) {
    ...
    public PhysicalExam {
        ...
        if (temperature != null
                && (temperature.signum() < 0 || temperature.compareTo(MAX_TEMPERATURE) > 0))

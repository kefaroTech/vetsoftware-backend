...
public record RoleDto(Long id, String name, String code, CompanySummaryDto company,
        LocalDateTime createdDate, List<PermissionSummaryDto> permissions, boolean enabled) {
    ...
    public static RoleDto from(Role role, List<PermissionSummaryDto> permissions) {
        return new RoleDto(role.getId(), role.getName(), role.getCode(),
                CompanySummaryDto.from(role.getCompany()), role.getCreatedDate(), permissions,
                role.isEnabled());
    }
}

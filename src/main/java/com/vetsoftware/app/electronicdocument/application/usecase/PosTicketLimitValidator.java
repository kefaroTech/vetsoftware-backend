...
/**
 * 3.2 — Control del tope de 5 UVT del documento equivalente POS (Res. DIAN
 * 000165/2023): el DOC_EQUIV_POS solo puede emitirse para ventas cuyo total
 * (payableAmount) sea ≤ 5·UVT. Por encima hay que emitir Factura electrónica
 * (FE_VENTA) con el adquiriente identificado. Enforcement de backend (defensa
 * en profundidad frente al gating del front). Solo aplica a DOC_EQUIV_POS; el
...
public class PosTicketLimitValidator {
    ...
    public void validate(ElectronicDocument document) {
        ...
        BigDecimal uvt = uvtQueryPort.currentUvt()
        ...
        if (document.getPayableAmount().compareTo(limit) > 0) {
            throw new IllegalArgumentException("El documento equivalente POS ("
                    + document.getPayableAmount() + ") supera el límite de 5 UVT (" + limit

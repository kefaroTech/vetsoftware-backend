...
/**
 * Núcleo de construcción del documento PENDIENTE desde una cuenta cerrada. Reutilizable y SIN
 * control de acceso (lo usan el caso de uso de construcción manual y el de emisión end-to-end de
 * F4).
 */
@Component
public class DocumentBuilder {
  ...
  public DocumentBuilder(
      SaleSnapshotQueryPort saleSnapshotQueryPort,
  ...
  public ElectronicDocument build(
      Long openAccountId,
      ElectronicDocumentType documentType,
      Long companyId,
      boolean finalConsumer) {
    SaleSnapshot snapshot =
        saleSnapshotQueryPort
            .findByOpenAccount(openAccountId, companyId)
            .orElseThrow(
                () -> new IllegalArgumentException("Open account not found: " + openAccountId));
    if (!snapshot.accountClosed()) {
      throw new IllegalStateException(
          "La cuenta debe estar cerrada (CLOSE) para emitir el documento electronico.");
    }
    // Consumidor final: el documento usa la identidad genérica DIAN en vez de los datos del Owner
    // de la cuenta.
    CustomerSnapshot customer =
        finalConsumer ? CustomerSnapshot.finalConsumer() : snapshot.customer();
    ElectronicDocument document =
        ElectronicDocument.createPending(
            snapshot.companyId(),
            snapshot.openAccountId(),
            documentType,
            snapshot.issuer(),
            customer,
            snapshot.lines(),
            ...
            snapshot.customerWithholdingAgent(),
            snapshot.reteFuenteRate(),
            // La idempotencia del cierre se maneja por cuenta (existsByOpenAccountId), no por
            // client_request_id.
            // issuedByEmployeeId null: la emisión al cerrar la cuenta no tiene un actor fiscal
            // directo aquí;
            // la autoría del cierre queda registrada en OpenAccount.closedBy.
            snapshot.reteIvaRate(),
            snapshot.reteIcaRate(),
            uvtQueryPort.currentUvt().orElse(null),
            null,
            null,
            snapshot.branchId());
    // 3.2: al cerrar una cuenta como tiquete POS (DOC_EQUIV_POS), tampoco puede superar 5 UVT; por
    // encima
    ...
    // EMISIÓN (justo antes de transmitir), no aquí: así el endpoint provisional /from-account no
    // quema

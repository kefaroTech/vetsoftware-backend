...
/**
 * Emite el documento electrónico al cerrar/cobrar una cuenta. Construye desde la cuenta cerrada el
 * documento PENDIENTE (dentro de la transacción del cierre) y difiere la transmisión + entrega a
 * {@link ClosedAccountEmissionCompleter} tras el commit (A1: el I/O externo NO corre bajo el lock
 * pesimista de la cuenta). Idempotente: si la cuenta ya tiene documento, no emite otro (evita
...
public class EmitElectronicDocumentOnCloseService implements EmitElectronicDocumentOnCloseUseCase {
  private static final Logger log =
      LoggerFactory.getLogger(EmitElectronicDocumentOnCloseService.class);

  private final DocumentBuilder documentBuilder;
  private final ClosedAccountEmissionCompleter emissionCompleter;
  private final ElectronicDocumentRepository repository;

  public EmitElectronicDocumentOnCloseService(
  ...
  public ElectronicDocumentDto execute(EmitElectronicDocumentCommand command) {
    ...
    // Bajo el lock pesimista del cierre: SOLO se persiste el documento PENDIENTE. Un fallo de
    // configuración
    // que valida el builder (p. ej. sin perfil fiscal) sigue haciendo fallar el cierre
    // atómicamente.
    ...
    // A1: la transmisión a MATIAS + entrega (PDF/S3/email) se difiere a DESPUÉS del commit del
    // cierre, en su
    // propia transacción, para no retener el lock pesimista de la cuenta ni la conexión del pool
    // durante el
    // I/O externo (~60s). Si falla, el documento queda PENDIENTE (re-emitible), sin revertir el
    // cierre.

...
/**
 * Construye el documento PENDIENTE desde una venta de POS (payload, no cuenta
 * cerrada). Reusable y SIN control de acceso. El emisor (issuer) y las tarifas
 * de retencion salen del perfil fiscal de la empresa; la clasificacion
 * tributaria de cada linea la pone el catalogo (autoritativa, no el cliente).
 * El documento nace SIN numero: la numeracion fiscal se asigna en la emision
...
public class PosSaleDocumentBuilder {
    ...
    /**
     * Tolerancia (1 peso) al validar el unitPrice del cliente contra el precio
     * canonico: absorbe el redondeo a peso entero (COP sin centavos).
     */
    ...
    public ElectronicDocument build(RegisterPosSaleCommand command) {
        ...
        List<ElectronicDocumentPayment> payments = command.payments().stream()
                .map(p -> new ElectronicDocumentPayment(null, p.means(), p.amount())).toList();

        BigDecimal uvt = uvtQueryPort.currentUvt().orElse(null);
        // Sede emisora: request branchId validado contra la empresa, o la "Principal"
        // por defecto.
        Long branchId = branchResolverPort.resolve(companyId, command.branchId()).orElseThrow(
                () -> new IllegalArgumentException("La empresa no tiene sucursal: " + companyId));
        ElectronicDocument document = ElectronicDocument.createPending(companyId,
                null /* sin cuenta abierta: la venta POS ES el registro */, command.documentType(),
                fiscal.issuer(), customer, lines, payments, PaymentForm.CONTADO, withholdingAgent,
                fiscal.reteFuenteRate(), fiscal.reteIvaRate(), fiscal.reteIcaRate(), uvt,
                command.clientRequestId(), command.issuedByEmployeeId(), branchId);
        // 3.2: un tiquete POS (DOC_EQUIV_POS) no puede superar 5 UVT; por encima exige
        // FE_VENTA.
    ...
    private ElectronicDocumentLine toLine(int lineNumber, SaleLine l, Long companyId,
        ...
        switch (l.kind()) {
            case PRODUCT -> {
                CatalogItem item = requireCatalog(
                        catalogLineQueryPort.findProduct(refId(l), companyId), "Producto", l);
            ...
            case SERVICE -> {
                CatalogItem item = requireCatalog(
                        catalogLineQueryPort.findService(refId(l), companyId), "Servicio", l);
                description = item.name();
                category = item.taxCategory();
                scheme = item.taxScheme();
                rate = item.taxRate();
            ...
            default ->
                throw new IllegalArgumentException("unsupported sale line kind: " + l.kind());
        }

        // El unitPrice del POS viene con IVA incluido (post-promo). Solo se extrae base
        // cuando hay
        // tarifa > 0
        // (GRAVADO/INC). EXENTO conserva su esquema IVA con tasa 0: base = bruto, IVA
        // 0, pero mantiene
        // el
        // esquema para que el XML lo distinga de EXCLUIDO (sin esquema).
        // EXCLUIDO/GENERAL viajan sin
        // esquema.
    ...
    /**
     * El servidor es la autoridad del precio: recalcula el precio canonico (lista
     * del catalogo + promo activa) y rechaza la linea si el unitPrice del cliente
     * se desvia mas alla de la tolerancia. Asi un usuario no puede emitir un
     * documento fiscal con un monto arbitrario aunque manipule el payload del POS.
     */
    private void validateUnitPrice(SaleLine l, CatalogItem item, List<SalePromotion> promos) {
        BigDecimal expected = PromotionPriceCalculator.expectedUnitPrice(item.basePrice(), l.kind(),
                l.refId(), item.categoryId(), promos);
        if (l.unitPrice().subtract(expected).abs().compareTo(PRICE_TOLERANCE) > 0) {
            throw new IllegalArgumentException(
                    "El precio unitario de la linea no coincide con el catalogo (" + l.kind()
                            + " refId " + l.refId() + "): esperado " + expected + ", recibido "
                            + l.unitPrice());
        }
    }
    ...
    private static CatalogItem requireCatalog(java.util.Optional<CatalogItem> item, String label,
            SaleLine l) {

...
/**
 * Emite end-to-end: construye el documento PENDIENTE desde la cuenta cerrada y lo transmite al
 * proveedor. Con MATIAS (asíncrono) el documento queda PENDIENTE y la entrega de la representación
 * la dispara el cierre async (webhook o polling de estado) al validar. (Un proveedor síncrono
 * entregaría en el acto.)
 */
@Observed(name = "electronic.document.emit")
@Service
public class EmitElectronicDocumentFromAccountService
    implements EmitElectronicDocumentFromAccountUseCase {
  private final DocumentBuilder documentBuilder;
  private final ElectronicDocumentEmitter emitter;
  private final DeliverElectronicDocumentService deliverService;

  public EmitElectronicDocumentFromAccountService(
  ...
  public ElectronicDocumentDto execute(EmitElectronicDocumentCommand command) {
    ...
    // Si la empresa tiene BILLING: numera (justo antes de transmitir) y transmite. Si no: el emisor
    // lo

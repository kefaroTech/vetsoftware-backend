...
/**
 * Procesa un webhook async (p. ej. MATIAS): enruta por proveedor, ubica el documento por la clave
 * del proveedor (bitácora), verifica HMAC con el secret de la empresa y aplica la transición DIAN.
...
public class ProcessProviderWebhookService implements ProcessProviderWebhookUseCase {
  ...
  public ProcessProviderWebhookService(
  ...
  public void execute(ProcessProviderWebhookCommand command) {
    ...
    Optional<Long> documentId =
        transmissionLog.findDocumentIdByProviderKey(parsed.providerDocumentKey());
    ...
    // Sin BILLING la empresa no transmite, así que no debería recibir webhooks; ignóralo por
    // seguridad.
    ...
    if (!parser.verifySignature(
        command.rawBody(), command.signatureHeader(), config.webhookSecret())) {
    ...
    if (parsed.outcome() == WebhookOutcome.ACCEPTED) {
      // El webhook NO trae el sello fiscal (CUFE/CUDE, XML firmado, QR). En vez de marcar VALIDADO
      // con
      // el sello vacío del webhook, consultamos el estado autoritativo al proveedor (fetchStatus,
      // vía
      // reconcile), que sí devuelve el sello; recién entonces se valida, se registra la bitácora,
      // se
      // reversa la cartera y se entrega la representación gráfica. Si el proveedor aún no tiene el
      // sello
      // listo, el documento queda PENDIENTE y el job de reconciliación lo reintenta — nunca se
      // marca
      // VALIDADO sin sello.

 ...
 * transmiten a la DIAN; sin el modulo el documento queda PENDIENTE (datos guardados, emision
 * diferida). El companyId lo inyecta el controller desde el contexto (el front nunca lo manda).
 */
public record RegisterPosSaleCommand(
    ...
    /**
     * Actor fiscal: empleado que registra la venta, inyectado por el controller desde el contexto
     * auth.
     */
    ...
    Long branchId) {
  /**
   * true si alguna linea es GENERAL (precio libre): solo SYSTEM puede emitirlas (ver el gate del
   * caso de uso).
   */

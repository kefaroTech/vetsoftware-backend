...
/**
 * Calcula el precio unitario canonico (post-promo) de una linea de catalogo, espejando exactamente la
 * logica del front (tienda/composables/pricing.ts#applyPromo): toma el menor entre el precio de lista y el
 * de cada promocion activa que aplique al item (por id directo) o a su categoria, y redondea a peso entero
 * (HALF_UP, igual que Math.round). Es la fuente de verdad del precio fiscal: el unitPrice del cliente se
 * valida contra esto, no al reves.
 */
    ...
    private static BigDecimal candidate(SalePromotion p, BigDecimal basePrice) {
        return switch (p.promotionType()) {
            ...
            case DISCOUNT -> p.valueType() == ValueType.PERCENTAGE
                    ? basePrice.multiply(BigDecimal.ONE.subtract(p.value().divide(HUNDRED, 10, RoundingMode.HALF_UP)))

...
/**
 * Calcula el precio unitario canonico (post-promo) de una linea de catalogo,
 * espejando exactamente la logica del front
 * (tienda/composables/pricing.ts#applyPromo): toma el menor entre el precio de
 * lista y el de cada promocion activa que aplique al item (por id directo) o a
 * su categoria, y redondea a peso entero (HALF_UP, igual que Math.round). Es la
 * fuente de verdad del precio fiscal: el unitPrice del cliente se valida contra
 * esto, no al reves.
 */
final class PromotionPriceCalculator {
    ...
    static BigDecimal expectedUnitPrice(BigDecimal basePrice, SaleLineKind kind, Long refId,
            Long categoryId, List<SalePromotion> promos) {
        BigDecimal best = basePrice;
        for (SalePromotion p : promos) {
            if (!matches(p, kind, refId, categoryId))
                continue;
    ...
    private static boolean matches(SalePromotion p, SaleLineKind kind, Long refId,
            Long categoryId) {
    ...
    private static BigDecimal candidate(SalePromotion p, BigDecimal basePrice) {
        return switch (p.promotionType()) {
            ...
            case DISCOUNT -> p.valueType() == ValueType.PERCENTAGE
                    ? basePrice.multiply(BigDecimal.ONE
                            .subtract(p.value().divide(HUNDRED, 10, RoundingMode.HALF_UP)))

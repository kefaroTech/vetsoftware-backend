...
/**
 * Aplica el efecto contable de una nota credito VALIDADA: marca la factura referenciada como
 * reversada y propaga el reverso a la cartera (open account). Es el punto unico que subordina el
 * void a la validacion DIAN, invocado tanto por el camino sincrono (DocumentTransmitter) como por
 * el async (ProcessProviderWebhookService). Idempotente y sin control de acceso (corre dentro de la
 * transaccion del que lo llama, que ya valido ownership o es un webhook firmado).
 */
@Component
public class CreditNoteReversalApplier {
  ...
  public CreditNoteReversalApplier(
  ...
  public void applyIfCreditNoteValidated(ElectronicDocument note) {
    ...
    repository
        .findByCufe(ref.cufe(), note.getCompanyId())
        .ifPresent(
            original -> {
              // 3.9 - solo una NC TOTAL (cubre el total de la factura) anula el original y reversa
              // la cartera.
              // Una NC PARCIAL emite el documento fiscal pero NO reversa la venta completa: evita
              // borrar todo el
              // saldo por un crédito parcial. El ajuste parcial de la cartera queda como paso
              // aparte.
              boolean fullCredit =
                  note.getPayableAmount().compareTo(original.getPayableAmount()) >= 0;

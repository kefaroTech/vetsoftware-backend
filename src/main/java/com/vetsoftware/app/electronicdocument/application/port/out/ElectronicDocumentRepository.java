...
public interface ElectronicDocumentRepository {
  ...
  /**
   * Ubica una factura por su CUFE dentro de la empresa (para enlazar la nota credito con el
   * original).
   */
  ...
  /**
   * ¿Ya existe un documento de este TIPO para la cuenta? Idempotencia de la conversión POS→FE: la
   * cuenta ya tiene el DOC_EQUIV_POS, así que se filtra por FE_VENTA para no convertir dos veces
   * (doble ingreso).
   */
  boolean existsByOpenAccountIdAndDocumentType(
      Long openAccountId, ElectronicDocumentType documentType);

  /**
   * Documento previamente registrado con este client_request_id dentro de la empresa. Idempotencia
   * de la venta POS: si el POST se reintenta con la misma key (respuesta perdida), se devuelve el
   * ya emitido en vez de registrar y transmitir otra venta.
   */
  Optional<ElectronicDocument> findByCompanyIdAndClientRequestId(
      Long companyId, String clientRequestId);
  ...
  /**
   * Persiste SOLO los campos del ciclo de vida DIAN (estado + sellos + número) tras una
   * transmisión. No reescribe líneas ni pagos (inmutabilidad del contenido fiscal).
   */

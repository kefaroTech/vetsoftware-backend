 ...
 * {@link EmitElectronicDocumentFromAccountUseCase}, NO exige el permiso `electronicDocument.emit`: el
 * derecho a emitir es consecuencia de haber cerrado la venta (ya autorizado por `openAccount.update`).
 * Solo se exige pertenencia a la empresa (defensa en profundidad). Idempotente: no duplica si la cuenta

 ...
 *
 * <p>TODO: vincular formalmente la FE con el POS (referencedCude) — depende del soporte del
 * proveedor; hoy la conversión emite una FE independiente sobre la misma venta.
 */
...
public class ConvertPosToInvoiceService implements ConvertPosToInvoiceUseCase {
  ...
  public ConvertPosToInvoiceService(
  ...
  public ElectronicDocumentDto execute(ConvertPosToInvoiceCommand command) {
    ElectronicDocument pos =
        repository
            .findById(command.posDocumentId())
    ...
    // 3.11/B4 - idempotencia: si la cuenta ya tiene una FE_VENTA, el POS ya fue convertido. Evita
    // emitir N

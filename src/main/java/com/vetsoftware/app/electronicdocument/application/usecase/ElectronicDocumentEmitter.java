 ...
 * numerar fiscalmente + transmitir al proveedor DIAN (empresa habilitada), o - dejar el documento
 * PENDIENTE (empresa sin el módulo): datos guardados, sin consumir consecutivo y sin MATIAS, pero
 * re-emitible más adelante al habilitar el módulo (la reconciliación lo ignora porque no tiene
 * clave de proveedor: nunca se transmitió).
 *
 * <p>Asume que el documento ya está persistido (PENDIENTE, sin número): {@link DocumentBuilder} /
 * {@link PosSaleDocumentBuilder} lo guardan al construirlo y los casos de uso de notas lo guardan
 * antes.
 */
@Component
public class ElectronicDocumentEmitter {
  ...
  public ElectronicDocumentEmitter(
  ...
  public ElectronicDocument emit(ElectronicDocument document) {
    // Sin BILLING: NO se numera ni se contacta a MATIAS. El documento queda PENDIENTE (ya
    // persistido)
    ...
    // POS: el consecutivo lo asigna el proveedor (MATIAS auto-increment), así que NO consumimos
    // numeración
    // local — solo aportamos resolución+prefijo (requeridos por el endpoint). El resto numera
    // localmente.

 ...
 * respaldo ante webhooks perdidos de proveedores asíncronos (p. ej. MATIAS): si el webhook {@code
 * document.accepted/rejected} nunca llega, el documento quedaría PENDIENTE para siempre.
 *
 * <p>Cada documento se reconcilia en su propia transacción ({@link DocumentTransmitter#reconcile});
 * un fallo no afecta a los demás. El job solo cierra los que el proveedor reporte ya como
 * VALIDADO/RECHAZADO (un proveedor sin polling devolvería vacío y sería no-op).
 */
@Component
public class PendingReconciliationJob {
  ...
  public PendingReconciliationJob(
  ...
  private Outcome executeReconciliation() {
    ...
    log.info(
        "Reconciliando documento(s) PENDIENTE contra el proveedor DIAN: {} candidato(s)",
        pending.size());
    int failures = 0;
    ...
    log.info(
        "Reconciliación DIAN finalizada: intentado(s)={}, fallido(s)={}, resultado={}",
        pending.size(),
        failures,
        outcome.value());
    return outcome;

...
/**
 * Resuelve el derecho a facturación electrónica: empresa → membresía → ¿incluye el submódulo
 * BILLING (habilitado)? Único cruce permitido a la persistencia de otras features (company,
 * membershipsubmodule).
 */
@Component
public class JpaBillingEntitlementQueryPort implements BillingEntitlementQueryPort {
  ...
  public boolean isElectronicInvoicingEnabled(Long companyId) {
    ...
    return companyJpaRepository
        ...
        .map(
            membershipId ->
                membershipSubModuleJpaRepository.hasEnabledSubModuleCode(
                    membershipId, BILLING_CODE))
        .orElse(false);
  }
}

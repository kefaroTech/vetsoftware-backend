...
/**
 * F2 - lectura del documento electronico + construccion PROVISIONAL desde una cuenta cerrada. Sin
 * update/delete (inmutabilidad fiscal). El POST de construccion es el punto de entrada temporal
...
public class ElectronicDocumentController {
  ...
  public ElectronicDocumentController(
      BuildElectronicDocumentFromAccountUseCase buildUseCase,
  ...
  public ElectronicDocumentDto buildFromAccount(
    ...
    return buildUseCase.execute(
        new BuildElectronicDocumentCommand(
  ...
  /**
   * F4: emite end-to-end (construir + transmitir + entregar PDF/QR/correo) desde una cuenta
   * cerrada.
   */
  @PostMapping("/emit")
  @ResponseStatus(HttpStatus.CREATED)
  public ElectronicDocumentDto emit(@Valid @RequestBody BuildElectronicDocumentRequest request) {
    return emitUseCase.execute(
  ...
  /**
   * Registra una venta de POS como documento electrónico (sin cuenta abierta). Con BILLING la
   * transmite a la DIAN; sin el módulo queda PENDIENTE (datos guardados, emisión diferida).
   * Disponible para cualquier usuario con acceso al POS (product.read) — no requiere permisos de
   * facturación electrónica.
   */
  @PostMapping("/from-sale")
  @ResponseStatus(HttpStatus.CREATED)
  public ElectronicDocumentDto registerPosSale(@Valid @RequestBody RegisterPosSaleRequest request) {
    List<RegisterPosSaleCommand.SaleLine> lines =
        request.lines().stream()
            .map(
                l ->
                    new RegisterPosSaleCommand.SaleLine(
                        l.kind(), l.refId(), l.description(), l.quantity(), l.unitPrice()))
            .toList();
    List<RegisterPosSaleCommand.SalePayment> payments =
        request.payments().stream()
            .map(p -> new RegisterPosSaleCommand.SalePayment(p.means(), p.amount()))
            .toList();
    return registerPosSaleUseCase.execute(
        new RegisterPosSaleCommand(
            authz.currentCompanyId(),
            request.documentType(),
            request.finalConsumer(),
            request.customerOwnerId(),
            lines,
            payments,
  ...
  public ElectronicDocumentDto convertToInvoice(@PathVariable Long id) {
    return convertPosUseCase.execute(new ConvertPosToInvoiceCommand(id, authz.currentCompanyId()));
  }
  ...
  @ResponseStatus(HttpStatus.CREATED)
  public ElectronicDocumentDto creditNote(
  ...
  @ResponseStatus(HttpStatus.CREATED)
  public ElectronicDocumentDto debitNote(
      @PathVariable Long id, @Valid @RequestBody IssueDebitNoteRequest request) {
    return debitNoteUseCase.execute(
        new IssueDebitNoteCommand(
            id,
            request.reason(),
            authz.currentCompanyId(),
            authz.currentEmployeeId(),
  ...
  public List<ElectronicDocumentDto> listAll(
    ...
    return listUseCase.listByCompany(
  ...
  /**
   * Documento emitido al cerrar una cuenta (para imprimir su recibo). 404 si la cuenta no generó
   * documento.
   */
  @GetMapping("/by-account/{openAccountId}")
  public ElectronicDocumentDto findByAccount(@PathVariable Long openAccountId) {
    return findByAccountUseCase
        .findByOpenAccount(openAccountId, authz.currentCompanyId())

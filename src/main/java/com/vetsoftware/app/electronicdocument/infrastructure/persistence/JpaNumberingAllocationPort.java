...
/**
 * Adapter: asigna el consecutivo desde la {@code NumberingResolution} activa de la empresa, con
 * bloqueo pesimista (FOR UPDATE) para serializar emisiones concurrentes. Valida vigencia y rango
 * antes de asignar. Corre dentro de la transacción de emisión (el caso de uso es
 * {@code @Transactional}), así que el lock se mantiene hasta el commit. Único cruce permitido de
 * vertical slicing: usa la persistencia de numberingresolution.
 */
@Component
public class JpaNumberingAllocationPort implements NumberingAllocationPort {
  ...
  @Override
  public Optional<AllocatedNumber> allocate(
  ...
  @Override
  public Optional<AllocatedNumber> peekActive(
      Long companyId, Long branchId, ElectronicDocumentType documentType) {
    NumberingResolutionJpaEntity r =
    ...
    // Sin consumir consecutivo: el proveedor (POS auto-increment) lo asigna. Solo resolución +
    // prefijo.
  ...
  public boolean release(
    ...
    // Solo se recupera el consecutivo si fue el último entregado y nadie tomó el siguiente
    // (recuperación
    ...
    //
    // DECISIÓN (no implementar free-list de consecutivos liberados): la doc DIAN/MATIAS exige
    // numeración
    ...
    // (excepto por anulaciones)". Reutilizar un número liberado FUERA DE ORDEN (tras aceptar
    // números
    // mayores) violaría esa regla y MATIAS lo rechazaría. Por eso el hueco que deja un rechazo
    // async ya
    // pasado de turno es DEFINITIVO y justificable (documento rechazado, nunca aceptado por la
    // DIAN): se

...
/**
 * Adapter: carga el Owner (scoped a la empresa) y lo congela en un CustomerSnapshot, igual que el
 * flujo de cuenta cerrada. Devuelve vacio si el owner no existe o no es de la empresa.
 */
@Component
public class JpaSaleCustomerQueryPort implements SaleCustomerQueryPort {
  ...
  public Optional<SaleCustomer> findOwner(Long ownerId, Long companyId) {
    ...
    if (owner == null
        || owner.getCompany() == null
        || !companyId.equals(owner.getCompany().getId())) {

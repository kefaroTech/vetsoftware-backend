...
/**
 * Adapter del {@link TechnicalKeyQueryPort}: lee la clave técnica de la resolución activa (misma
 * que asigna la numeración). Único cruce permitido de vertical slicing (persistence → persistence
 * de numberingresolution).
 */
@Component
public class JpaTechnicalKeyQueryPort implements TechnicalKeyQueryPort {
  ...
  @Override
  public Optional<String> findActiveTechnicalKey(
      Long companyId, Long branchId, ElectronicDocumentType documentType) {
    return repository
        .findActive(companyId, branchId, documentType.name())

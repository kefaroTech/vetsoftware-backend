...
/**
 * Traduce los datos de otras features al read model de esta. Es el unico punto que conoce open
 * account, cargos, perfil fiscal, owner y abonos. Construye los snapshots de emisor/adquiriente,
 * mapea cada cargo no anulado a una linea (deriva categoria/esquema tributario del impuesto
 * congelado) y cada abono no anulado a un pago con medio codificado DIAN.
 */
@Component
public class JpaSaleSnapshotQueryPort implements SaleSnapshotQueryPort {
  ...
  public JpaSaleSnapshotQueryPort(
      OpenAccountJpaRepository openAccountRepository,
  ...
  public Optional<SaleSnapshot> findByOpenAccount(Long openAccountId, Long companyId) {
    OpenAccountJpaEntity account =
        openAccountRepository.findByIdAndCompany_Id(openAccountId, companyId).orElse(null);
    if (account == null) {
      return Optional.empty();
    }

    CompanyFiscalProfile fiscalProfile =
    ...
    CustomerSnapshot customer =
        new CustomerSnapshot(
            ...
            FiscalResponsibility.fromName(
                owner.getFiscalResponsibility() == null
                    ? null
    ...
    // F6 - retenciones: el adquiriente es agente retenedor + las tarifas configuradas por el
    // emisor.
    ...
    return Optional.of(
        new SaleSnapshot(
            ...
            PaymentForm.CONTADO,
            withholdingAgent,
            fiscalProfile.reteFuenteRate(),
            fiscalProfile.reteIvaRate(),
            fiscalProfile.reteIcaRate(),
  ...
  private List<ElectronicDocumentLine> buildLines(Long openAccountId, Long companyId) {
    ...
    int n = 0;
    for (var c :
        productChargeRepository.findByOpenAccount_IdAndOpenAccount_Company_Id(
    ...
    for (var c :
        serviceChargeRepository.findByOpenAccount_IdAndOpenAccount_Company_Id(
            openAccountId, companyId)) {
      if (c.isVoided()) continue;
    ...
    for (var c :
        generalChargeRepository.findByOpenAccount_IdAndOpenAccount_Company_Id(
            openAccountId, companyId)) {
      if (c.isVoided()) continue;
  ...
  /**
   * Deriva la clasificacion tributaria de la linea desde lo congelado en el cargo (taxTreatment +
   * esquema + tasa). EXENTO -> (EXENTO, IVA, 0%): la DIAN lo distingue de EXCLUIDO (sin esquema).
   * GRAVADO/INC -> su esquema y tasa. Los cargos previos a congelar el tratamiento (frozenTreatment
   * null) caen a la heuristica: gravado -> (GRAVADO/INC, esquema); sin impuesto -> (EXCLUIDO,
   * null).
   */
  private ElectronicDocumentLine line(
      int lineNumber,
      String description,
      BigDecimal quantity,
      ...
      BigDecimal total,
      String frozenScheme,
      String frozenTreatment) {
    // EXENTO = gravado a tarifa 0%: lleva esquema IVA con tasa 0 (la base ya es el total y el IVA
    // 0). Es lo
    ...
    // unico esquema que el cierre soportaba. INC tributa con su propia categoria/esquema, como el
    // POS.
    boolean inc = "INC".equals(frozenScheme);
    TaxCategory category =
        !taxed ? TaxCategory.EXCLUIDO : (inc ? TaxCategory.INC : TaxCategory.GRAVADO);
  ...
  private List<ElectronicDocumentPayment> buildPayments(Long openAccountId, Long companyId) {
    ...
    for (var d :
        debtRepository.findByOpenAccount_IdAndOpenAccount_Company_Id(openAccountId, companyId)) {
      if (d.isVoided()) continue;
      payments.add(
          new ElectronicDocumentPayment(null, toPaymentMeans(d.getPaymentMethod()), d.getAmount()));
    }
    return payments;
  }

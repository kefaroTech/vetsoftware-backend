 ...
 *
 * <p>El reintento se acota por dos límites (configurables): un <b>cap de intentos</b>
 * ({@code dian.contingency.max-attempts}) y una <b>ventana de plazo</b> desde la creación
 * ({@code dian.contingency.deadline-hours}). Agotado cualquiera de los dos, el documento se deja en
 * CONTINGENCIA pero deja de reintentarse automáticamente (se registra un error para atención manual:
 * reemisión vía {@code POST /electronic-documents/{id}/transmit} o corrección por nota).
 */
@Component
public class ContingencyRetryJob {
    ...
    private boolean isExhausted(ElectronicDocument document, LocalDateTime deadlineThreshold) {
        ...
        if (attempts >= maxAttempts) {
            log.error("Documento {} agotó el cap de {} reintentos de contingencia DIAN; "
                    + "requiere atención manual (reemitir o corregir por nota).", document.getId(), maxAttempts);
            return true;
        }
        if (document.getCreatedDate() != null && document.getCreatedDate().isBefore(deadlineThreshold)) {
            log.error("Documento {} superó la ventana de {}h del plazo de contingencia DIAN; "
                    + "requiere atención manual (reemitir o corregir por nota).", document.getId(), deadlineHours);
            return true;
        }
        return false;
    }

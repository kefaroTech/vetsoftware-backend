...
/**
 * Documento electronico (cabecera). INMUTABLE: sin @SQLDelete/@SQLRestriction (nunca se
 * soft-deletea ni edita; solo se corrige por nota credito/debito). Snapshots de emisor/adquiriente
 * como columnas.
 */
@Entity
...
public class ElectronicDocumentJpaEntity {
  ...
  // Régimen de IVA del adquiriente congelado (RESPONSABLE_IVA / NO_RESPONSABLE_IVA) → tax_regime_id
  // DIAN.
  @Column(name = "customer_tax_regime", length = 20)
  private String customerTaxRegime;

  // Responsabilidad fiscal del adquiriente congelada (NO_APLICA/GRAN_CONTRIBUYENTE/…) →
  // tax_level_id DIAN.
  ...
  // Pobladas solo en notas. `reversed` marca esta factura como anulada por una nota credito
  // validada.
  ...
  // registrar/transmitir dos veces la misma venta ante un reintento con la respuesta perdida. NULL
  // en los
  ...
  // open_account_id (cruce entre features por id). NULL en emisión al cierre, procesos sistema y
  // legacy.

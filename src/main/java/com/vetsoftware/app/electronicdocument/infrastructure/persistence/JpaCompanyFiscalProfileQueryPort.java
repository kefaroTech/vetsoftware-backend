...
/**
 * Adapter: arma el snapshot del emisor desde CompanyTaxProfile y carga las tarifas de retencion desde
 * WithholdingConfig. Si la empresa no tiene perfil fiscal, devuelve vacio (el caso de uso decide el error).
 */
@Component
public class JpaCompanyFiscalProfileQueryPort implements CompanyFiscalProfileQueryPort {
    ...
    public JpaCompanyFiscalProfileQueryPort(CompanyTaxProfileJpaRepository companyTaxProfileRepository,
    ...
    public Optional<CompanyFiscalProfile> findByCompany(Long companyId) {
        return companyTaxProfileRepository.findByCompany_Id(companyId).map(profile -> {
            ...
            return new CompanyFiscalProfile(issuer,

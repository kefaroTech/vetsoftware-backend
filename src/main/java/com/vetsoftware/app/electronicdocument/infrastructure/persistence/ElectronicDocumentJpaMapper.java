  ...
  public ElectronicDocument toDomain(ElectronicDocumentJpaEntity entity) {
    List<ElectronicDocumentLine> lines =
        entity.getLines().stream()
            ...
            .map(
                le ->
                    new ElectronicDocumentLine(
                        le.getId(),
                        le.getLineNumber(),
                        le.getDescription(),
                        le.getQuantity(),
                        le.getUnitMeasureCode(),
                        le.getUnitPrice(),
                        le.getLineExtensionAmount(),
                        le.getTaxCategory(),
                        le.getTaxScheme(),
                        le.getTaxRate(),
                        le.getTaxAmount(),
    ...
    List<ElectronicDocumentPayment> payments =
        entity.getPayments().stream()
            .map(
                pe ->
                    new ElectronicDocumentPayment(pe.getId(), pe.getPaymentMeans(), pe.getAmount()))
            .toList();
    ...
    IssuerSnapshot issuer =
        new IssuerSnapshot(
            ...
            entity.getIssuerDocumentId(),
            entity.getIssuerVerificationDigit(),
            entity.getIssuerLegalName(),
            entity.getIssuerTaxRegime(),
            entity.getIssuerEmail(),
            splitCodes(entity.getIssuerResponsibilities()));
    CustomerSnapshot customer =
        new CustomerSnapshot(
            entity.getCustomerDocumentType(),
            entity.getCustomerDocumentId(),
            entity.getCustomerVerificationDigit(),
            entity.getCustomerPersonType(),
            entity.getCustomerLegalName(),
            entity.getCustomerName(),
            entity.getCustomerEmail(),
            entity.getCustomerCityDane(),
            TaxRegime.fromName(entity.getCustomerTaxRegime()),
            FiscalResponsibility.fromName(entity.getCustomerFiscalResponsibility()));

    DocumentReference reference =
        entity.getReferencedCufe() == null
            ? null
            : new DocumentReference(
                entity.getReferencedCufe(),
                entity.getReferencedPrefix(),
                entity.getReferencedNumber(),
                entity.getReferencedIssueDate());

    return new ElectronicDocument(
        entity.getId(),
        companyId,
        entity.getOpenAccountId(),
        ...
        entity.getQrUrl(),
        entity.getXmlSigned(),
        entity.getPdfRepresentation(),
        entity.getDianStatus(),
        entity.getDianValidationDate(),
        issuer,
        customer,
        entity.getLineExtensionAmount(),
        entity.getTaxExclusiveAmount(),
        entity.getTaxInclusiveAmount(),
        entity.getPayableAmount(),
        entity.getPaymentForm(),
        lines,
        payments,
        entity.getCreatedDate(),
        entity.isEnabled(),
        reference,
        entity.getNoteReasonCode(),
        entity.getNoteReasonText(),
        entity.isReversed(),
        entity.getReteFuenteAmount(),
        entity.getReteIvaAmount(),
        entity.getReteIcaAmount(),
        entity.getClientRequestId(),
        entity.getIssuedByEmployeeId(),
        entity.getBranchId());
  }

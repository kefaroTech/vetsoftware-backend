...
/**
 * Adapter: deriva la clasificacion tributaria de la linea desde el catalogo (filtrado por
 * enabled=true via @SQLRestriction). taxTreatment espeja 1:1 a TaxCategory; el esquema/tasa salen
 * del impuesto asociado. GRAVADO/INC con impuesto y tasa > 0 -> (categoria, esquema del tax, tasa);
 * EXENTO/EXCLUIDO -> sin tributo.
 */
@Component
public class JpaCatalogLineQueryPort implements CatalogLineQueryPort {
  ...
  public JpaCatalogLineQueryPort(
      ProductJpaRepository productRepository, ServiceJpaRepository serviceRepository) {
    ...
    return serviceRepository
        ...
        .map(
            s ->
                toItem(
                    s.getName(),
                    s.getTaxTreatment().name(),
                    s.getTax(),
                    s.getPrice(),
                    s.getServiceCategory().getId()));
  }

  private CatalogItem toItem(
      String name, String taxTreatment, TaxJpaEntity tax, BigDecimal basePrice, Long categoryId) {
    TaxCategory category =
        TaxCategory.valueOf(taxTreatment); // product/service.TaxTreatment espeja TaxCategory
    // EXENTO = gravado a tarifa 0%: lleva esquema IVA con tasa 0 (la DIAN lo distingue de EXCLUIDO,
    // que
    ...
    boolean taxed =
        (category == TaxCategory.GRAVADO || category == TaxCategory.INC)
            && tax != null
            && tax.getPercentage() != null
            && tax.getPercentage().signum() > 0;
    if (!taxed) {
      return new CatalogItem(name, category, null, null, basePrice, categoryId);
    }
    TaxScheme scheme =
        TaxScheme.valueOf(tax.getTaxScheme().name()); // tax.TaxScheme (IVA/INC) -> doc.TaxScheme

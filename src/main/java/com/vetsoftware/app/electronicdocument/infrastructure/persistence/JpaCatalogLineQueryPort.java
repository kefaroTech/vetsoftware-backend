...
/**
 * Adapter: deriva la clasificacion tributaria de la linea desde el catalogo
 * (filtrado por enabled=true via @SQLRestriction). taxTreatment espeja 1:1 a
 * TaxCategory; el esquema/tasa salen del impuesto asociado. GRAVADO/INC con
 * impuesto y tasa > 0 -> (categoria, esquema del tax, tasa); EXENTO/EXCLUIDO ->
 * sin tributo.
 */
@Component
public class JpaCatalogLineQueryPort implements CatalogLineQueryPort {
    ...
    public Optional<CatalogItem> findService(Long serviceId, Long companyId) {
        return serviceRepository.findById(serviceId)
                ...
                .map(s -> toItem(s.getName(), s.getTaxTreatment().name(), s.getTax(), s.getPrice(),
    ...
    private CatalogItem toItem(String name, String taxTreatment, TaxJpaEntity tax,
        ...
        TaxCategory category = TaxCategory.valueOf(taxTreatment); // product/service.TaxTreatment
                                                                  // espeja TaxCategory
        // EXENTO = gravado a tarifa 0%: lleva esquema IVA con tasa 0 (la DIAN lo
        // distingue de EXCLUIDO,
        // que
        // NO lleva esquema). Independiente de que el catálogo tenga o no un impuesto
        // asociado.
        if (category == TaxCategory.EXENTO) {
            return new CatalogItem(name, category, TaxScheme.IVA, BigDecimal.ZERO, basePrice,
                    categoryId);
        }
        ...
        TaxScheme scheme = TaxScheme.valueOf(tax.getTaxScheme().name()); // tax.TaxScheme (IVA/INC)
                                                                         // -> doc.TaxScheme

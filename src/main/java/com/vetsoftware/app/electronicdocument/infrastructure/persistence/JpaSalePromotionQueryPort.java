...
/**
 * Adapter: trae las promociones de la empresa (filtradas por enabled=true via @SQLRestriction en
 * PromotionJpaEntity) y se queda con las ACTIVAS en la fecha dada (estado ACTIVE + hoy dentro del rango
 * [startDate, endDate]), espejando la derivacion de estado del front (pricing.ts#promoStatus). Mapea a la
 * forma local de SalePromotionQueryPort por nombre de enum para no acoplar el dominio de promotion.
 */
@Component
public class JpaSalePromotionQueryPort implements SalePromotionQueryPort {
    ...
    public List<SalePromotion> findActive(Long companyId, LocalDate today) {
        return promotionRepository.findAllByCompanyId(companyId).stream()
                ...
                .map(JpaSalePromotionQueryPort::toLocal)
                .toList();
    }

    private static boolean isActiveOn(PromotionJpaEntity p, LocalDate today) {
        if (!"ACTIVE".equals(p.getPromotionStatus().name())) return false;
    ...
    private static SalePromotion toLocal(PromotionJpaEntity p) {
        return new SalePromotion(
                PromotionType.valueOf(p.getPromotionType().name()),
                ApplicationType.valueOf(p.getApplicationType().name()),
                p.getApplicationItem(),
                ValueType.valueOf(p.getValueType().name()),
                p.getValue());
    }
}

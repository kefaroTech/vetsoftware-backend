...
public class JpaElectronicDocumentRepository implements ElectronicDocumentRepository {
  ...
  public JpaElectronicDocumentRepository(
      ElectronicDocumentJpaRepository jpaRepository,
  ...
  @Override
  public boolean existsByOpenAccountIdAndDocumentType(
      Long openAccountId,
  ...
  @Override
  public Optional<ElectronicDocument> findByCompanyIdAndClientRequestId(
      Long companyId, String clientRequestId) {
    return jpaRepository
        .findByCompany_IdAndClientRequestId(companyId, clientRequestId)
        .map(mapper::toDomain);
  }
  ...
  public Optional<ElectronicDocument> findByOpenAccountId(Long openAccountId, Long companyId) {
    return jpaRepository
        .findByOpenAccountIdAndCompany_Id(openAccountId, companyId)
        .map(mapper::toDomain);
  }

  @Override
  public ElectronicDocument updateDianResult(ElectronicDocument document) {
    ElectronicDocumentJpaEntity entity =
        jpaRepository
            .findById(document.getId())
            .orElseThrow(
                () ->
                    new com.vetsoftware.app.electronicdocument.domain
    ...
    // F5: el reverso contable de la factura (marcado al validar su nota credito) tambien se
    // persiste aqui.

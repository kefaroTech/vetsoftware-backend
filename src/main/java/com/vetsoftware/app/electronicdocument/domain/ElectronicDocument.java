...
/**
 * Documento electronico (cabecera + lineas + pagos): la entidad fiscal INMUTABLE. Se construye
 * congelando una cuenta cerrada (ver {@link #createPending}). En F2 nace PENDIENTE, sin numero
 * (prefix/consecutive) ni sellos DIAN (cufe/cude/uuid/qr/xml/pdf): los llenan F4 y F3. No expone
 * update ni soft-delete: la unica correccion valida es una nota credito/debito (F5).
 */
public class ElectronicDocument {
  ...
  // de estados forward-only (markValidated/markRejected/markContingency). El contenido fiscal
  // (líneas,
  ...
  // F6 - retenciones que el adquiriente (agente retenedor) practica, congeladas. Cero cuando no
  // aplica.
  ...
  // Idempotencia de la venta POS: UUID que el cliente genera por apertura del cobro. Null en
  // documentos que
  // no vienen del POS (cierre de cuenta, notas). La unicidad (empresa, clientRequestId) la respalda
  // la BD.
  private final String clientRequestId;

  // Actor fiscal: empleado que emite el documento (venta POS, nota credito/debito), tomado del
  // contexto de
  // autenticacion. Da trazabilidad de autoria a acciones sensibles (una NC anula factura y reversa
  // cartera).
   ...
   */
  public static ElectronicDocument createPending(
      Long companyId,
      Long openAccountId,
      ElectronicDocumentType documentType,
      IssuerSnapshot issuer,
    ...
    // Cuadre de caja: toda venta es de contado, así que la suma de pagos debe igualar el total a
    // pagar.
    ...
    // F6/3.10: retenciones del adquiriente agente retenedor. reteIVA SOLO sobre el IVA generado
    // (esquema
    // IVA), NO sobre IVA+INC; reteFuente/reteICA sobre la base de las líneas gravadas (excluye
    // EXCLUIDO/GENERAL).
    WithholdingAmounts wh =
        WithholdingCalculator.compute(
            withholdingAgent,
            withholdingBaseOf(lines),
            ivaTotalOf(lines),
            reteFuenteRate,
            reteIvaRate,
            reteIcaRate,
   ...
   * contenido fiscal del original (lineas, snapshots, totales) y fija la referencia + concepto
   * DIAN. La nota usa CUDE propio y consecutivo de su resolucion (los estampa la validacion DIAN,
   * F3).
   */
  ...
  /**
   * Construye una NOTA DEBITO PENDIENTE (aumento) que referencia una factura VALIDADA. Con {@code
   * additionalAmount} null clona el original (heredado); con un monto {@code > 0} la nota
   * representa ese INCREMENTO real (escala proporcional del original), en vez de duplicar el valor
   * de la factura.
   */
  public static ElectronicDocument createDebitNote(
    ...
    return createNote(
        original,
        ElectronicDocumentType.NOTA_DEBITO,
        reasonCode,
        reasonText,
        issuedByEmployeeId,
        additionalAmount);
  ...
  /**
   * 3.9 — Construye la nota (crédito/débito). {@code amount} null ⇒ nota TOTAL (clona el original:
   * NC de anulación). {@code amount} presente ⇒ nota PARCIAL/INCREMENTAL: escala líneas, totales y
   * retenciones por el ratio {@code amount/total}, preservando la mezcla de tarifas y el IVA
   * proporcional. La NC no puede exceder el total del original; la ND admite cualquier incremento
   * positivo.
   */
  private static ElectronicDocument createNote(
      ElectronicDocument original,
    ...
    DocumentReference ref =
        new DocumentReference(
            original.cufe, original.prefix, original.consecutive, original.issueDate);
    ...
    List<ElectronicDocumentLine> noteLines =
        original.lines.stream().map(l -> scaleLine(l, ratio)).toList();
    List<ElectronicDocumentPayment> notePayments =
        original.payments.stream()
            .map(
                p ->
                    new ElectronicDocumentPayment(
                        null, p.getPaymentMeans(), Money.scaled(p.getAmount().multiply(ratio))))
            .toList();
    ZonedDateTime now = ZonedDateTime.now(COLOMBIA);
    String issueTime = now.toLocalTime().format(TIME_FORMAT) + COLOMBIA_OFFSET;
    return new ElectronicDocument(
        ...
        Money.scaled(original.payableAmount.multiply(ratio)),
        ...
        Money.scaled(original.reteIcaAmount.multiply(ratio)),
        null,
        issuedByEmployeeId,
        original.branchId);
  }
  ...
  /**
   * Clona una línea escalando sus montos por {@code ratio} (NC parcial / ND incremental). ratio=1 ⇒
   * copia.
   */
  private static ElectronicDocumentLine scaleLine(ElectronicDocumentLine l, BigDecimal ratio) {
    ...
    return new ElectronicDocumentLine(
        ...
        Money.scaled(l.getLineExtensionAmount().multiply(ratio)),
        l.getTaxCategory(),
        l.getTaxScheme(),
        l.getTaxRate(),
        Money.scaled(l.getTaxAmount().multiply(ratio)),
  ...
  private static void validateNoteConsistency(
      ElectronicDocumentType documentType, DocumentReference reference, String noteReasonCode) {
    boolean isNote =
   ...
   * registrados, asume EFECTIVO (10). Lo usan los adaptadores de proveedor para
   * `payment_method_code`.
   */
  ...
  /**
   * Asigna la numeración fiscal (resolución DIAN + prefijo + consecutivo) al emitir, antes de
   * transmitir. Tomada de la {@code NumberingResolution} activa de la empresa. Solo sobre un
   * documento PENDIENTE y aún sin numerar (idempotencia/seguridad fiscal: el consecutivo no se
   * reasigna).
   */
  ...
  /**
   * El consecutivo de los documentos equivalentes POS lo asigna el proveedor DIAN (MATIAS
   * auto-increment), no nuestra resolución local; por eso no consumimos consecutivo local para POS.
   */
  ...
  /**
   * Asigna SOLO resolución + prefijo (sin consecutivo) al emitir, para los documentos cuyo número
   * lo asigna el proveedor DIAN ({@link #usesProviderAssignedConsecutive()}). El consecutivo real
   * lo sella {@link #markValidated} con la respuesta del proveedor. No consume numeración local.
   */
  ...
  /**
   * Sella el documento como VALIDADO por la DIAN: número fiscal + sellos del proveedor.
   * Forward-only: solo desde PENDIENTE/CONTINGENCIA. No toca el contenido fiscal.
   */
  ...
  /**
   * Libera la numeración fiscal de un documento RECHAZADO (contrapartida de {@link #assignNumber})
   * para que el consecutivo pueda reutilizarse y no quede un hueco en la secuencia. El intento
   * (número enviado + motivo) queda en la bitácora de transmisión, no en el documento. Solo
   * aplicable tras marcar RECHAZADO, y solo se invoca cuando la resolución pudo recuperar el
   * consecutivo.
   */
  ...
  /**
   * Documento "provisional": en contingencia y aún SIN sello fiscal DIAN (sin CUFE/CUDE). Lo que se
   * entrega al cliente mientras tanto es un comprobante NO fiscal; el documento fiscal real llega
   * al regularizar (cuando el proveedor/DIAN vuelve y la transmisión valida). Útil para etiquetar
  ...
  private void ensureNotTerminal() {
    if (dianStatus == DianStatus.VALIDADO || dianStatus == DianStatus.RECHAZADO) {
      throw new IllegalStateException(
          "El documento ya está en estado terminal ("
              + dianStatus
  ...
  /**
   * Neto a pagar tras restar las retenciones del adquiriente (total − reteFuente − reteIva −
   * reteIca).
   */
  ...
  /**
   * IVA generado del documento = suma del impuesto de las líneas con esquema IVA (a tarifa real).
   * Es la base correcta de la reteIVA: NO incluye el INC (3.10). Recalculado desde las líneas para
   * que el reporte (buildTaxTotals) coincida con el monto retenido en {@link #createPending}.
   */
  ...
  /**
   * Base de retención en fuente / ICA = suma de la base de las líneas GRAVADAS (con esquema
   * tributario: GRAVADO/INC/EXENTO-IVA-0%). Excluye EXCLUIDO y los ítems libres del POS (sin
   * esquema), que no forman parte de la base gravable (3.10).
   */
  ...
  /**
   * Empleado que emitió el documento (POS/NC/ND); null en emisión al cierre, procesos sistema o
   * legacy.
   */

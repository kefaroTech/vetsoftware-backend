 ...
 *
 * <p>B3 — cuantías mínimas: la reteFuente y la reteIVA no se practican bajo una base mínima (E.T. arts.
 * 401/392). Se usa el umbral general de servicios (4 UVT); no se distingue por concepto (compras 27 UVT) por
 * no llevar el concepto en la línea — es una aproximación conservadora documentada. Si no hay UVT disponible
 * no se aplica mínimo (no se bloquea la retención). B7 — la reteICA se expresa en ‰ (por mil), no en %.
 */
public final class WithholdingCalculator {
    ...
    public static WithholdingAmounts compute(boolean withholdingAgent, BigDecimal taxableBase,
                                             ...
                                             BigDecimal reteIvaRate, BigDecimal reteIcaRate, BigDecimal uvt) {
        if (!withholdingAgent) return WithholdingAmounts.NONE;
        // reteFuente/reteIVA: solo si la base supera la cuantía mínima (evita sobre-retención en montos chicos).
        boolean meetsMinimum = meetsMinimum(taxableBase, uvt);
        BigDecimal reteFuente = meetsMinimum ? Money.percentOf(taxableBase, reteFuenteRate) : BigDecimal.ZERO;
        BigDecimal reteIva = meetsMinimum ? Money.percentOf(ivaAmount, reteIvaRate) : BigDecimal.ZERO;
        // reteICA: tarifa en ‰ (por mil), sin mínimo en UVT (los mínimos de ICA son municipales y varían).
    ...
    private static boolean meetsMinimum(BigDecimal taxableBase, BigDecimal uvt) {
        if (uvt == null || uvt.signum() <= 0) return true; // sin UVT no se puede evaluar el mínimo: no bloquear

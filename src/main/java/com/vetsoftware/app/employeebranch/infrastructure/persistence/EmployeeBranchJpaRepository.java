...
public interface EmployeeBranchJpaRepository extends JpaRepository<EmployeeBranchJpaEntity, Long> {
    ...
    // Ids de las sedes asignadas (vigentes) a un empleado. El @SQLRestriction("enabled = true") ya excluye las
    ...
    @Transactional
    @Query(value = "UPDATE employee_branches SET enabled = false WHERE employee_id = :employeeId AND enabled = true",
        nativeQuery = true)
    void disableAllByEmployeeId(@Param("employeeId") Long employeeId);

    // Reactiva una asignación previamente desactivada (evita insertar un duplicado que violaría el unique). Devuelve
    ...
    // Inserta una asignación nueva. Solo se llama cuando reactivate devolvió 0 (no había fila), así que no choca con
    ...
    // Empleados (activos) de la empresa "con todas las sedes" para auto-asignarles una sede recién creada: los que
    // aún no tienen fila para la sede nueva Y ya cubren TODAS las demás sedes de la empresa (cobertura total). Así,
    // al crear una sede, quien tenía todas la hereda; quien estaba limitado a un subconjunto estricto, no. Caveat:
    ...
    @Query(value =
        ...
        + "AND NOT EXISTS (SELECT 1 FROM employee_branches eb2 "
        + "                WHERE eb2.employee_id = e.id AND eb2.branch_id = :newBranchId) "
        + "AND NOT EXISTS (SELECT 1 FROM branches b "
        + "                WHERE b.company_id = :companyId AND b.id <> :newBranchId "
        + "                AND NOT EXISTS (SELECT 1 FROM employee_branches eb "
        + "                                WHERE eb.employee_id = e.id AND eb.branch_id = b.id AND eb.enabled = true))",
        nativeQuery = true)

...
/**
 * Gestión del alcance por sede de un empleado (multi-sucursal). El {@code companyId} lo deriva el
 * backend del contexto (nunca lo elige el cliente); el ownership se valida en el
 * {@code @PreAuthorize} de cada use case.
 */
@RestController
...
public class EmployeeBranchController {
  ...
  public EmployeeBranchController(
      GetEmployeeBranchesUseCase getUseCase, SetEmployeeBranchesUseCase setUseCase, Authz authz) {
  ...
  @PutMapping
  public EmployeeBranchesResponse set(
      @PathVariable Long employeeId, @Valid @RequestBody SetEmployeeBranchesRequest request) {
    boolean allBranches = request.allBranches();
    List<Long> branchIds = request.branchIds();
    // "Todas" significa todas las sedes del alcance explícito del actor; nunca toda la empresa por
    // comodín.
    if (allBranches) {
      allBranches = false;
    ...
    EmployeeBranchesDto dto =
        setUseCase.execute(
            new SetEmployeeBranchesCommand(

...
/**
 * Asignación de una sede ({@link BranchJpaEntity}) a un empleado
 * ({@link EmployeeJpaEntity}). Única fuente del alcance por sede: "todas las
 * sedes" se materializa como una fila por sede; cero filas = sin acceso a
 * recursos scopeados a sede. Soft-delete por {@code enabled}, igual que {@code
 * employee_roles}.
 */
@Entity
@Table(name = "employee_branches", uniqueConstraints = {
        @UniqueConstraint(name = "uq_employee_branches", columnNames = {"employee_id",
                "branch_id"})})

...
public interface AppointmentJpaRepository extends JpaRepository<AppointmentJpaEntity, Long> {
  ...
  @EntityGraph(attributePaths = {"animal", "owner", "employee", "company", "branch"})
  @Query(
      "SELECT a FROM AppointmentJpaEntity a WHERE a.company.id = :companyId "
  ...
  @Query(
      "SELECT a.id FROM AppointmentJpaEntity a WHERE a.company.id = :companyId "
  ...
  // Soft-delete por query nativa (evita el conflicto @SQLDelete + @Version en em.remove).
  @Modifying(flushAutomatically = true, clearAutomatically = true)
  @Transactional
  @Query(
      value = "UPDATE appointments SET enabled = false WHERE id = :id AND company_id = :companyId",
      nativeQuery = true)

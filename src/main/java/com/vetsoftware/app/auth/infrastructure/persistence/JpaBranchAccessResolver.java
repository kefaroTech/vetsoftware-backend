...
/**
 * Resuelve y cachea (por employeeId) el set de sedes asignadas a un empleado, en paralelo a
 * {@link JpaPermissionResolver}. {@code employee_branches} es la única fuente del alcance por sede — "todas" se
 * materializa como una fila por sede, así que no hay flag que leer por request. Se cachea un {@link HashSet} (no un
 * {@code Set.copyOf(...)} inmutable) para round-trippear de forma fiable con {@code GenericJackson2JsonRedisSerializer},
 * igual que {@code employee-permissions}.
 *
 * <p>El {@code @Cacheable} vive en el método público invocado desde otro bean ({@code ResolveAuthContextService}), así
 * la llamada cruza el proxy de cache de Spring. Invalidar con {@link #evict(Long)} tras reasignar sedes.</p>
 */
@Repository

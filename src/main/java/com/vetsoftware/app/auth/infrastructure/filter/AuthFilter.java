...
public class AuthFilter extends OncePerRequestFilter {
  ...
  private static final List<JwtExcludedRoute> JWT_EXCLUDED_PATHS =
      List.of(
          ...
          // Actuator pertenece a su propia SecurityFilterChain; este filtro solo procesa JWT de
          // negocio.
  ...
  protected void doFilterInternal(
    ...
    if (header == null || !header.startsWith("Bearer ")) {
      writeUnauthorized(
          request, response, "TOKEN_MISSING", "Missing or invalid Authorization header");
      return;
    }
  ...
  /**
   * Rechazo de autenticación en formato ProblemDetail (RFC 7807), consistente con {@code
   * GlobalExceptionHandler}. El {@code code} discrimina el motivo para que el front decida: {@code
   * TOKEN_EXPIRED} → intentar refrescar; {@code SESSION_REPLACED}, {@code TOKEN_INVALID} o {@code
   * TOKEN_MISSING} → desloguear.
   */
  private void writeUnauthorized(
      HttpServletRequest request, HttpServletResponse response, String code, String detail)
      throws IOException {

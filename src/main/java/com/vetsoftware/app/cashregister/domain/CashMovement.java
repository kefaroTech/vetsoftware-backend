...
/**
 * Movimiento de caja (entidad hija, append-only e inmutable). El {@code amount} es siempre positivo; el signo lo da
 * el {@link CashMovementType#isInflow()}. Nunca se hace UPDATE/DELETE: las correcciones son movimientos nuevos
 * ({@code VOID_OUT} para reversar una venta/abono, {@code MANUAL_*} para ajustes de operador).
 */
public class CashMovement {
    ...
    public static CashMovement create(CashMovementType type, CashPaymentMethod method, BigDecimal amount,
                                      CashReferenceType referenceType, Long referenceId, Long createdByEmployeeId,
                                      String note) {
        return new CashMovement(null, type, method, amount, referenceType, referenceId, createdByEmployeeId,
            LocalDateTime.now(), note);
    }

    /** Aporte con signo al total del método: {@code +amount} si entra, {@code -amount} si sale. */

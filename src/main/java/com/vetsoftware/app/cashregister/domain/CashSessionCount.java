...
/**
 * Conteo de cierre por medio de pago: lo que el sistema esperaba ({@code expectedAmount}) frente a
 * lo que se declaró contado ({@code countedAmount}); la {@code difference} (contado − esperado) es
 * el sobrante (+) o faltante (−). Se materializa solo al cerrar la sesión. Para no-efectivo suele
 * cuadrar (conciliación por medio); solo el efectivo se cuenta físicamente.
 */
public class CashSessionCount {
  ...
  public CashSessionCount(
      Long id, CashPaymentMethod method, BigDecimal expectedAmount, BigDecimal countedAmount) {
  ...
  public static CashSessionCount create(
      CashPaymentMethod method, BigDecimal expectedAmount, BigDecimal countedAmount) {

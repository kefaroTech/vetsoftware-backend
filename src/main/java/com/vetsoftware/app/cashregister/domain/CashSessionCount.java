...
/**
 * Conteo de cierre por medio de pago: lo que el sistema esperaba
 * ({@code expectedAmount}) frente a lo que se declaró contado
 * ({@code countedAmount}); la {@code difference} (contado − esperado) es el
 * sobrante (+) o faltante (−). Se materializa solo al cerrar la sesión. Para
 * no-efectivo suele cuadrar (conciliación por medio); solo el efectivo se
 * cuenta físicamente.
 */
public class CashSessionCount {
    ...
    public CashSessionCount(Long id, CashPaymentMethod method, BigDecimal expectedAmount,
        ...
        if (countedAmount == null)
            throw new IllegalArgumentException("countedAmount is required");
        this.id = id;
        this.method = method;
    ...
    /**
     * Diferencia de arqueo = contado − esperado. Positiva sobra, negativa falta,
     * cero cuadra.
     */

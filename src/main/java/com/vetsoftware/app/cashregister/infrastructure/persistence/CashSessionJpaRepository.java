...
public interface CashSessionJpaRepository extends JpaRepository<CashSessionJpaEntity, Long> {
  ...
  // Detalle por id + empresa. Las colecciones (movimientos/counts) se hidratan LAZY dentro de la
  // transacción de
  ...
  // Sesión OPEN de (empresa, sede, terminal). Solo puede haber una (índice único condicional en
  // BD).
  ...
  @Query(
      value =
          SUMMARY_SELECT
              ...
              """,
      nativeQuery = true)
  Optional<CashSessionSummaryRow> findOpenSummary(
  ...
  @Query(
      value =
          SUMMARY_SELECT
              ...
              """,
      nativeQuery = true)
  Optional<CashSessionSummaryRow> findOpenSummaryByTerminalId(
      @Param("companyId") Long companyId,
      @Param("branchId") Long branchId,
  ...
  // Los LEFT JOIN nativos incluyen el nombre aunque el empleado haya sido desactivado
  ...
  @Query(
      ...
      countQuery =
          ...
          """,
      nativeQuery = true)
  Page<CashSessionSummaryRow> search(
      @Param("companyId") Long companyId,
      @Param("branchId") Long branchId,

...
public class JpaCashSessionRepository implements CashSessionRepository {
  ...
  // save() de una sesión nueva → persist; de una existente → merge con la versión del dominio
  // (optimistic lock:
  // dos cierres o dos appends concurrentes hacen que el 2º falle con
  // ObjectOptimisticLockingFailureException → 409).
  // Los movimientos/counts nuevos (id == null) se insertan; los existentes (append-only,
  ...
  public Optional<CashSession> findOpen(Long companyId, Long branchId, String terminal) {
    return jpaRepository
        .findFirstByCompanyIdAndBranchIdAndTerminalAndStatus(
  ...
  public Optional<CashSessionView> findOpenSummaryByTerminalId(
    ...
    return jpaRepository
        .findOpenSummaryByTerminalId(companyId, branchId, terminalId)
        .map(this::toSummary);
  }
  ...
  public boolean existsOpenByEmployee(Long companyId, Long employeeId) {
    return employeeId != null
        && jpaRepository.existsByCompanyIdAndOpenedByEmployeeIdAndStatus(
  ...
  public Optional<CashSession> findOpenByEmployee(Long companyId, Long employeeId) {
    ...
    return jpaRepository
        .findFirstByCompanyIdAndOpenedByEmployeeIdAndStatus(
            companyId, employeeId, CashSessionStatus.OPEN)
        .map(this::toDomain);
  }
  ...
  public PageResult<CashSessionView> search(SearchCashSessionsQuery query) {
    ...
    Page<CashSessionSummaryRow> page =
  ...
  public List<CashSessionView> findOpenSummaries(Long companyId, Set<Long> accessibleBranchIds) {
    ...
    List<CashSessionSummaryRow> rows =
  ...
  private CashSessionView toSummary(CashSessionSummaryRow e) {
    return CashSessionView.summary(
        e.getId(),
        e.getBranchId(),
        e.getBranchName(),
        e.getTerminalId(),
        e.getTerminal(),
        CashSessionStatus.valueOf(e.getStatus()),
        e.getOpenedByEmployeeId(),
        e.getOpenedByEmployeeName(),
        e.getOpenedAt(),
        e.getOpeningFloat(),
        e.getClosingTotal(),
        e.getClosedByEmployeeId(),
        e.getClosedByEmployeeName(),
        e.getClosedAt(),
        e.getNote(),
        e.getVersion());
  }
  ...
  private CashSession toDomain(CashSessionJpaEntity e) {
    ...
    List<CashSessionCount> counts =
        e.getCounts().stream()
            .map(
                c ->
                    new CashSessionCount(
                        c.getId(), c.getMethod(), c.getExpectedAmount(), c.getCountedAmount()))
            .toList();

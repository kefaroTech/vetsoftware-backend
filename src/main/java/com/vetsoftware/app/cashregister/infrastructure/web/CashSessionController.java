...
/**
 * Caja / arqueo por sede (punto 5). Apertura/movimiento manual con {@code cashregister.operate}; cierre con
 * {@code cashregister.close}; lecturas con {@code cashregister.read} (gate en los @PreAuthorize de los puertos). La
 * sede se resuelve por el alcance del empleado con {@link Authz#resolveAccessibleBranch}. Los movimientos de venta/
 * abono/reversa NO entran por aquí: los inyecta la orquestación (POS / cuenta abierta).
 */
@RestController
...
public class CashSessionController {
    ...
    public CashSessionView close(@PathVariable Long id, @Valid @RequestBody CloseCashSessionRequest request) {
        return closeUseCase.close(new CloseCashSessionCommand(
            authz.currentCompanyId(), id, authz.currentEmployeeIdOrNull(), request.note(),

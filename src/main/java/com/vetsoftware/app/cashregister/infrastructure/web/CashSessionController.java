...
/**
 * Caja / arqueo por sede (punto 5). Apertura/movimiento manual con
 * {@code cashregister.operate}; cierre con {@code cashregister.close}; lecturas
 * con {@code cashregister.read} (gate en los @PreAuthorize de los puertos). La
 * sede se resuelve por el alcance del empleado con
 * {@link Authz#resolveAccessibleBranch}. Los movimientos de venta/
 * abono/reversa NO entran por aquí: los inyecta la orquestación (POS / cuenta
 * abierta).
 */
@RestController
...
public class CashSessionController {
    ...
    public CashSessionController(OpenCashSessionUseCase openUseCase,
            RegisterCashMovementUseCase registerMovementUseCase,
            CloseCashSessionUseCase closeUseCase, GetCurrentCashSessionUseCase getCurrentUseCase,
            GetCashSessionUseCase getUseCase, ListCashSessionsUseCase listUseCase,
            ListOpenCashSessionsUseCase listOpenUseCase, ExportArqueoUseCase exportArqueoUseCase,
            CashReportPdfPort cashReportPdf, Authz authz) {
    ...
    public CashSessionView open(@Valid @RequestBody OpenCashSessionRequest request) {
        return openUseCase.open(new OpenCashSessionCommand(authz.currentCompanyId(),
                authz.resolveAccessibleBranch(request.branchId()), request.terminalId(),
                request.openingFloat(), authz.currentEmployeeIdOrNull(), request.note()));
    }

    @PostMapping("/{id}/movements")
    @ResponseStatus(HttpStatus.CREATED)
    public CashSessionView registerMovement(@PathVariable Long id,
            @Valid @RequestBody RegisterCashMovementRequest request) {
        return registerMovementUseCase.register(new RegisterCashMovementCommand(
                authz.currentCompanyId(), id, request.type(), request.method(), request.amount(),
                authz.currentEmployeeIdOrNull(), request.note()));
    }

    @PostMapping("/{id}/close")
    public CashSessionView close(@PathVariable Long id,
            @Valid @RequestBody CloseCashSessionRequest request) {
        return closeUseCase.close(
                new CloseCashSessionCommand(authz.currentCompanyId(), id,
                        authz.currentEmployeeIdOrNull(), request.note(), request
                                .counts() == null
                                        ? java.util.List.of()
                                        : request.counts().stream()
                                                .map(c -> new CloseCashSessionCommand.Count(
                                                        c.method(), c.countedAmount()))
                                                .toList()),
                false);
    }
    ...
    public ResponseEntity<byte[]> exportArqueo(@PathVariable Long id,

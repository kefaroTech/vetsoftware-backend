...
final class CashArqueoCsv {
    ...
    static byte[] arqueo(CashArqueoReport r) {
        ...
        for (CashArqueoReport.MethodRow m : r.methods()) {
            row(sb, methodLabel(m.method()), plain(m.opening()), plain(m.salesIn()), plain(m.accountIn()),
                plain(m.manualIn()), plain(m.withdrawals()), plain(m.expenses()), plain(m.voidOut()),
                plain(m.expected()), plain(m.counted()), plain(m.difference()));
        }
        row(sb, "TOTAL", "", "", "", "", "", "", "", plain(r.totalExpected()), plain(r.totalCounted()),
            plain(r.totalDifference()));
        row(sb);
    ...
    /** Monto con signo según el tipo (entra + / sale −), para el listado de movimientos. */

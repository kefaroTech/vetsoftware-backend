...
public record CloseCashSessionRequest(@Valid List<CountLine> counts, String note) {
  ...
  /** Conteo de un método: cuánto se contó físicamente (efectivo) o concilió (no-efectivo). */
  public record CountLine(
      @NotNull CashPaymentMethod method, @NotNull @PositiveOrZero BigDecimal countedAmount) {}
}

...
/**
 * Salida de una sesión de caja. En el detalle / current va completa (totales por método +
 * movimientos + counts); en el listado va como {@link #summary} (sin movimientos ni counts, solo la
 * cabecera).
 */
public record CashSessionView(
    ...
    String note,
    Long version,
    List<MethodTotalView> totals,
    List<CashMovementView> movements,
    List<CashSessionCountView> counts) {

  /**
   * Vista completa (detalle / current / respuesta a open/close/movimiento) con totales, movimientos
   * y counts.
   */
  public static CashSessionView from(CashSession s) {
    List<MethodTotalView> totals =
        s.expectedByMethod().entrySet().stream()
            .map(e -> new MethodTotalView(e.getKey(), e.getValue()))
            .toList();
    BigDecimal closingTotal =
        s.getStatus() == CashSessionStatus.CLOSED
            ? s.getCounts().stream()
                .map(c -> c.getCountedAmount())
  ...
  public static CashSessionView summary(
      Long id,
      Long branchId,
      String branchName,
      Long terminalId,

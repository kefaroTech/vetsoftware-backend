...
/**
 * Puerto INTERNO de orquestación: lo consumen POS y cuenta abierta (vía sus adapters) para afectar
 * la caja sin importar el dominio de otras features. Sin {@code @PreAuthorize}: corre dentro de la
 * transacción del caller, que ya validó ownership/permiso de la acción de negocio (venta/abono),
 * igual que {@code StockLedgerUseCase} en inventario.
 */
public interface CashLedgerUseCase {

  /**
   * Registra el ingreso en la caja OPEN del empleado y valida que corresponda a la sede.
   * Idempotente.
   */
  ...
  /**
   * Bloqueo "caja requerida" (F4): si la empresa exige caja ({@code cashregister.required}, default
   * true) y no hay sesión OPEN en la sede, lanza {@code NoOpenCashSessionException} (→ 409) ANTES
   * de emitir/persistir el cobro. Si la empresa no la exige, es no-op. Lo invocan POS y cuenta
   * abierta al inicio del cobro.
   */

...
/**
 * Abre la caja de una sede con la base inicial. Cada empleado puede mantener
 * una sola sesión OPEN y cada terminal solo una por sede; los índices únicos
 * condicionales de la BD cubren las carreras residuales.
 */
    ...
    public CashSessionView open(OpenCashSessionCommand command) {
        ...
        CashTerminalQueryPort.TerminalRef terminal = terminalQueryPort
                ...
                .orElseThrow(() -> new IllegalArgumentException(
                        "Terminal no válido, inactivo o perteneciente a otra sede: "
                                + command.terminalId()));
        ...
        repository
                .findOpenSummaryByTerminalId(command.companyId(), command.branchId(), terminal.id())
                .ifPresent(open -> {
                    throw new CashSessionAlreadyOpenException(open.branchName(), open.terminal(),
                            open.openedByEmployeeName());
                });
        CashSession session = CashSession.open(command.companyId(), command.branchId(),
                terminal.id(), terminal.code(), command.openedByEmployeeId(),
                command.openingFloat(), command.note());

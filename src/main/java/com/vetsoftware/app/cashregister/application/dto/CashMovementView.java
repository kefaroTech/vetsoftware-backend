...
/**
 * Salida de un movimiento de caja (el {@code amount} es positivo; el signo lo
 * da el tipo).
 */
public record CashMovementView(Long id, CashMovementType type, CashPaymentMethod method,
        BigDecimal amount, CashReferenceType referenceType, Long referenceId,
        Long createdByEmployeeId, LocalDateTime createdAt, String note) {

    public static CashMovementView from(CashMovement m) {
        return new CashMovementView(m.getId(), m.getType(), m.getMethod(), m.getAmount(),

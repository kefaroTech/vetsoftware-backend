...
public interface CashSessionRepository {
  ...
  /**
   * Persiste la sesión con sus movimientos/counts (append-only); devuelve el agregado con IDs y
   * versión.
   */
  ...
  /**
   * Sesión OPEN de (empresa, sede, terminal) con sus movimientos (para totales y para registrar
   * movimientos).
   */
  ...
  Optional<CashSessionView> findOpenSummaryByTerminalId(
      Long companyId, Long branchId, Long terminalId);

  /**
   * ¿Hay ya una sesión OPEN para (empresa, sede, terminal)? Red de negocio antes del índice único
   * de la BD.
   */
  ...
  /**
   * Cajas OPEN visibles. {@code accessibleBranchIds == null} significa todas las sedes de la
   * empresa (solo admin); para empleados normales contiene exclusivamente las sedes asignadas del
   * JWT.
   */

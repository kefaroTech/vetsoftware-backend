...
/**
 * Reporte de arqueo de una sesión de caja: cabecera + desglose por método (base, ventas, abonos,
 * ingresos, retiros, gastos, reversas, esperado vs contado y diferencia) + listado de movimientos.
 * Insumo del export CSV/PDF y del BI.
 */
public record CashArqueoReport(
  ...
  public static CashArqueoReport from(CashSession s) {
    ...
    for (CashPaymentMethod m : CashPaymentMethod.values()) {
      ...
      CashSessionCount count =
          s.getCounts().stream().filter(c -> c.getMethod() == m).findFirst().orElse(null);
    ...
    return new CashArqueoReport(
        ...
        totalExpected,
        closed ? totalCounted : null,
        closed ? totalDifference : null);
  }

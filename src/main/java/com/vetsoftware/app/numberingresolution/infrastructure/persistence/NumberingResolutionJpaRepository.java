...
public interface NumberingResolutionJpaRepository
    extends JpaRepository<NumberingResolutionJpaEntity, Long> {
   ...
   * "una sola resolución activa por (company, sede, tipo)". {@code branchId} null = alcance de
   * EMPRESA (branch_id IS NULL); no null = esa sede concreta. Nativa para el manejo simétrico del
   * null.
   */
  @org.springframework.data.jpa.repository.Query(
      value =
          "SELECT COUNT(*) FROM numbering_resolutions WHERE company_id = :companyId "
  ...
  /**
   * Resolución activa a usar para una emisión, BLOQUEADA para actualización (FOR UPDATE): serializa
   * la asignación concurrente del consecutivo. Multi-sucursal (B-6): resuelve la resolución de la
   * SEDE si existe y, si no, la de EMPRESA (branch_id IS NULL) — {@code ORDER BY (branch_id IS
   * NULL)} pone la de sede primero. Nativa para poder usar FOR UPDATE; filtra enabled=true
   * explícito (la @SQLRestriction no aplica a nativas). {@code documentType} es el nombre del enum
   * (columna document_type se persiste como STRING).
   */
  @org.springframework.data.jpa.repository.Query(
      value =
          "SELECT * FROM numbering_resolutions WHERE company_id = :companyId "
              + "AND document_type = :documentType AND enabled = true "
  ...
  /**
   * Igual que {@link #lockActiveForUpdate} pero SIN bloqueo (lectura): para los casos que solo
   * necesitan resolución+prefijo y NO consumen consecutivo (POS auto-increment: MATIAS asigna el
   * número). Mismo fallback sede→empresa.
   */
  @org.springframework.data.jpa.repository.Query(
      value =
          "SELECT * FROM numbering_resolutions WHERE company_id = :companyId "

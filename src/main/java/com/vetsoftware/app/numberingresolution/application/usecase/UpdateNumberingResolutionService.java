...
public class UpdateNumberingResolutionService implements UpdateNumberingResolutionUseCase {
    ...
    public NumberingResolutionDto execute(UpdateNumberingResolutionCommand command) {
        ...
        // Invariante "una sola activa por (empresa, SEDE, tipo)": si el update mueve la resolución a un alcance
        // distinto que ya tiene otra activa, conflicto. (Si el alcance no cambia, no se chequea para no contarse
        ...
        if (scopeChanged && repository.existsActiveByCompanyBranchAndType(

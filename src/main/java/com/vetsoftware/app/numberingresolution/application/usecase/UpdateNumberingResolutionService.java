...
public class UpdateNumberingResolutionService implements UpdateNumberingResolutionUseCase {
    ...
    public NumberingResolutionDto execute(UpdateNumberingResolutionCommand command) {
        ...
        // Invariante "una sola activa por (empresa, SEDE, tipo)": si el update mueve la
        // resolución a un
        // alcance
        // distinto que ya tiene otra activa, conflicto. (Si el alcance no cambia, no se
        // chequea para no
        // contarse
        // a sí misma; la BD respalda cualquier carrera vía
        // uq_numbering_resolutions_active_scope.)
        boolean scopeChanged = !command.companyId().equals(resolution.getCompany().id())
                || !Objects.equals(command.branchId(), resolution.getBranchId())
                || command.documentType() != resolution.getDocumentType();
        if (scopeChanged && repository.existsActiveByCompanyBranchAndType(command.companyId(),
                command.branchId(), command.documentType())) {
            throw new NumberingResolutionAlreadyActiveException(command.companyId(),
                    command.documentType());
        }
        resolution.update(company, command.documentType(), command.resolutionNumber(),
                command.resolutionDate(), command.prefix(), command.rangeFrom(), command.rangeTo(),
                command.validFrom(), command.validTo(), command.technicalKey(), command.branchId());

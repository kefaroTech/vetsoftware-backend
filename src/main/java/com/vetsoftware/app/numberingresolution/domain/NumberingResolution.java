...
public class NumberingResolution {
    ...
    // Multi-sucursal (B-6): sede (prefijo) a la que aplica la resolución. null =
    // resolución de
    // EMPRESA (prefijo
    // que sirve a todas las sedes). En la asignación, la de sede tiene prioridad
    // sobre la de empresa
    // (fallback).
    ...
    // Próximo consecutivo a emitir dentro del rango. Se gestiona al emitir
    // documentos,
    ...
    public NumberingResolution(Long id, CompanyRef company, ElectronicDocumentType documentType,
            ...
            Long currentNumber, LocalDateTime createdDate, boolean enabled, Long branchId) {
        validate(company, documentType, resolutionNumber, resolutionDate, prefix, rangeFrom,
                rangeTo, validFrom, validTo, technicalKey, currentNumber);
        this.id = id;
    ...
    public static NumberingResolution create(CompanyRef company,
        ...
        return new NumberingResolution(null, company, documentType, resolutionNumber,
                resolutionDate, prefix, rangeFrom, rangeTo, validFrom, validTo, technicalKey,
                rangeFrom, LocalDateTime.now(), true, branchId);
    }
    ...
    public void update(CompanyRef company, ElectronicDocumentType documentType,
            ...
            Long branchId) {
        validate(company, documentType, resolutionNumber, resolutionDate, prefix, rangeFrom,
                rangeTo, validFrom, validTo, technicalKey, this.currentNumber);
        this.company = company;
        this.branchId = branchId;
        this.documentType = documentType;
        this.resolutionNumber = resolutionNumber;
    ...
    private static void validate(CompanyRef company, ElectronicDocumentType documentType,
            String resolutionNumber, LocalDate resolutionDate, String prefix, Long rangeFrom,
            Long rangeTo, LocalDate validFrom, LocalDate validTo, String technicalKey,
            Long currentNumber) {
        if (company == null)
        ...
        if (resolutionDate == null)
            throw new IllegalArgumentException("resolutionDate is required");
        if (prefix != null && prefix.length() > 10)
            throw new IllegalArgumentException("prefix must be 10 chars or less");
        if (rangeFrom == null)
            throw new IllegalArgumentException("rangeFrom is required");
        if (rangeFrom < 1)
            throw new IllegalArgumentException("rangeFrom must be 1 or greater");
        if (rangeTo == null)
            throw new IllegalArgumentException("rangeTo is required");
    ...
    /**
     * Sede (prefijo) de la resolución; null = resolución de empresa (todas las
     * sedes).
     */

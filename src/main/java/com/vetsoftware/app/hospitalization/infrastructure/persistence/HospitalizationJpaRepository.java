...
public interface HospitalizationJpaRepository
    ...
    @EntityGraph(attributePaths = {"animal", "consultation", "company"})
    Page<HospitalizationJpaEntity> findAllByAnimalId(Long animalId, Pageable pageable);

    @org.springframework.data.jpa.repository.Modifying(flushAutomatically = true, clearAutomatically = true)
    @org.springframework.transaction.annotation.Transactional
    @org.springframework.data.jpa.repository.Query(value = "UPDATE hospitalizations SET enabled = true WHERE id = :id", nativeQuery = true)

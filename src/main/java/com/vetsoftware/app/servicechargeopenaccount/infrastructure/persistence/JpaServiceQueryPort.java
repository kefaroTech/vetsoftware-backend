...
public class JpaServiceQueryPort implements ServiceQueryPort {
    ...
    public Optional<ServiceRef> findByIdAndCompanyId(Long serviceId, Long companyId) {
        return serviceJpaRepository.findByIdAndCompany_Id(serviceId, companyId)
                .map(JpaServiceQueryPort::toRef);
    }

    private static ServiceRef toRef(ServiceJpaEntity e) {
        // Gravado tanto GRAVADO (IVA) como INC: ambos llevan impuesto. El esquema
        // (IVA/INC) sale del
        // Tax y
        // se congela en el cargo para que el documento del cierre lo respete igual que
        // la venta POS.
        ...
        TaxRef tax = hasTax && t != null
                ...
                : null;
        // Congela el tratamiento real del catálogo (incl. EXENTO/EXCLUIDO), no solo el
        // hasTax
        // monetario.

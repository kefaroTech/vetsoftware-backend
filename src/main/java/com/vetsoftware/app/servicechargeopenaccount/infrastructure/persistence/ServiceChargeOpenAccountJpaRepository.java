...
public interface ServiceChargeOpenAccountJpaRepository
    ...
    @Override
    @EntityGraph(attributePaths = {"animal", "service", "tax", "openAccount", "createdBy", "voidedBy"})
    ...
    @EntityGraph(attributePaths = {"animal", "service", "tax", "openAccount", "createdBy", "voidedBy"})
    Optional<ServiceChargeOpenAccountJpaEntity> findByIdAndOpenAccount_Company_Id(Long id, Long companyId);
    ...
    @org.springframework.data.jpa.repository.Query(
        value = "UPDATE service_charge_open_accounts SET enabled = true WHERE id = :id "
            + "AND EXISTS (SELECT 1 FROM open_accounts oa "
            + "WHERE oa.id = service_charge_open_accounts.open_account_id AND oa.company_id = :companyId)",
        nativeQuery = true)

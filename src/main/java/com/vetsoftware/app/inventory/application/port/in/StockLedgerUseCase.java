...
/**
 * Único punto de afectación de inventario. Lo consumen POS, cuenta abierta y
...
public interface StockLedgerUseCase {
    ...
    /**
     * Salida por venta/consumo (FEFO). Idempotente por (referenceType,
     * referenceId). Devuelve el consumo por lote.
     */
    List<StockConsumptionDto> recordSale(RecordSaleCommand command);

    /**
     * Consumo clínico (FEFO, tipo CLINICAL_USE, ref CLINICAL_EVENT). Respeta el
     * flag de stock negativo.
     */
    ...
    /**
     * Transferencia entre sedes: salida FEFO en origen + entrada en destino
     * (preserva lote/costo/vencimiento).
     */

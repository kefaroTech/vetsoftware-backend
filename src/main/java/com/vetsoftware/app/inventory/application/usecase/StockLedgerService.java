...
public class StockLedgerService implements StockLedgerUseCase {
  ...
  public void recordPurchase(RecordPurchaseCommand c) {
    ...
    StockLot lot =
        lotRepository
            .findByIdentity(
                c.companyId(), c.branchId(), c.productId(), c.lotNumber(), c.expireDate(), cost)
            .orElseGet(
                () ->
                    StockLot.create(
                        c.companyId(),
  ...
  public List<StockConsumptionDto> recordSale(RecordSaleCommand c) {
    ...
    List<StockConsumptionDto> consumptions =
        consumeFefo(
            c.companyId(),
            c.branchId(),
            c.productId(),
  ...
  public List<StockConsumptionDto> recordClinicalUse(RecordClinicalUseCommand c) {
    ...
    if (c.referenceId() != null
        && movementRepository.existsByReference(
            StockReferenceType.CLINICAL_EVENT, c.referenceId())) {
    ...
    if (balance.getQuantity() < c.quantity()
      ...
      throw new InsufficientStockException(
          "Stock insuficiente para el consumo clínico del producto "
              + c.productId()
              + " en la sede "
              + c.branchId()
              + " (disponible "
      ...
      StockLot lot =
          lotRepository
              ...
              .orElseGet(
                  () ->
                      StockLot.create(
                          c.companyId(), c.branchId(), c.productId(), null, null, 0, cost));
      lot.add(c.delta());
      StockLot savedLot = lotRepository.save(lot);
      movementRepository.save(
          StockMovement.of(
  ...
  public void transfer(TransferStockCommand c) {
    ...
    if (fromBal.getQuantity() < c.quantity()) {
      ...
      throw new InsufficientStockException(
          "Stock insuficiente para transferir el producto "
              + c.productId()
              + " desde la sede "
              + c.fromBranchId()
              + " (disponible "
    ...
    for (StockLot lot : lots) {
      ...
      movementRepository.save(
          StockMovement.of(
              ...
              c.reason(),
              c.createdBy()));
      // Réplica del lote en destino: misma identidad (lote/vencimiento/costo) para preservar FEFO y
      // COGS.
      StockLot destLot =
          lotRepository
              .findByIdentity(
  ...
  public void reverse(StockReferenceType referenceType, Long referenceId, Long createdBy) {
    ...
    boolean alreadyReversed =
        movements.stream()
            .anyMatch(
                m ->
                    m.getType() == StockMovementType.VOID_IN
  ...
  /**
   * Descuenta {@code quantity} de los lotes de la sede en orden FEFO, creando un movimiento por
   * lote consumido. Si los lotes no alcanzan (stock negativo ya permitido por el caller), lleva el
   * sobrante al último lote FEFO (o a un lote genérico si no hay ninguno), dejándolo en negativo.
   * No toca el saldo (lo hace el caller).
   */
  private List<StockConsumptionDto> consumeFefo(
      Long companyId,
      Long branchId,
      Long productId,
      int quantity,
    ...
    if (remaining > 0) {
      StockLot lot =
          lots.isEmpty()
              ? lotRepository.save(
                  StockLot.create(companyId, branchId, productId, null, null, 0, BigDecimal.ZERO))
  ...
  private StockBalance balanceForUpdate(Long companyId, Long branchId, Long productId) {
    return balanceRepository
        .findForUpdate(productId, branchId)
        .orElseGet(
            () -> balanceRepository.save(StockBalance.create(companyId, branchId, productId, 0)));
  }
}

...
public class StockLedgerService implements StockLedgerUseCase {
    ...
    public StockLedgerService(StockLotRepository lotRepository,
            ...
            NegativeStockPolicyPort negativeStockPolicy, InventoryMetrics inventoryMetrics) {
    ...
    public void recordPurchase(RecordPurchaseCommand c) {
        ...
        StockLot lot = lotRepository
                .findByIdentity(c.companyId(), c.branchId(), c.productId(), c.lotNumber(),
                        c.expireDate(), cost)
        ...
        movementRepository.save(StockMovement.of(c.companyId(), c.branchId(), c.productId(),
                savedLot.getId(), StockMovementType.PURCHASE, c.quantity(), cost, c.referenceType(),
                c.referenceId(), null, c.createdBy()));
    ...
    public List<StockConsumptionDto> recordSale(RecordSaleCommand c) {
        ...
        StockBalance balance = balanceForUpdate(c.companyId(), c.branchId(), c.productId());
        // POS fuerza negativo (c.allowNegative()); cuenta abierta respeta el flag de
        // empresa.
        boolean allowNegative = c.allowNegative()
                || negativeStockPolicy.allowsNegative(c.companyId());
        if (balance.getQuantity() < c.quantity() && !allowNegative) {
            ...
            throw new InsufficientStockException("Stock insuficiente para el producto "
                    + c.productId() + " en la sede " + c.branchId() + " (disponible "
                    + balance.getQuantity() + ", requerido " + c.quantity() + ")");
        }
        List<StockConsumptionDto> consumptions = consumeFefo(c.companyId(), c.branchId(),
                c.productId(), c.quantity(), StockMovementType.SALE, c.referenceType(),
                c.referenceId(), null, c.createdBy());
    ...
    public List<StockConsumptionDto> recordClinicalUse(RecordClinicalUseCommand c) {
        ...
        StockBalance balance = balanceForUpdate(c.companyId(), c.branchId(), c.productId());
        if (balance.getQuantity() < c.quantity()
                && !negativeStockPolicy.allowsNegative(c.companyId())) {
            inventoryMetrics.movement(StockMovementType.CLINICAL_USE, Result.INSUFFICIENT_STOCK, 0);
            throw new InsufficientStockException(
                    "Stock insuficiente para el consumo clínico del producto " + c.productId()
                            + " en la sede " + c.branchId() + " (disponible "
                            + balance.getQuantity() + ", requerido " + c.quantity() + ")");
        }
        List<StockConsumptionDto> consumptions = consumeFefo(c.companyId(), c.branchId(),
                c.productId(), c.quantity(), StockMovementType.CLINICAL_USE,
                StockReferenceType.CLINICAL_EVENT, c.referenceId(), c.reason(), c.createdBy());
        balance.subtract(c.quantity());
        balanceRepository.save(balance);
    ...
    public void recordAdjustment(RecordAdjustmentCommand c) {
        ...
        StockMovementType movementType = c.delta() > 0
        ...
        if (c.delta() > 0) {
            ...
            StockLot savedLot = lotRepository.save(lot);
            movementRepository.save(StockMovement.of(c.companyId(), c.branchId(), c.productId(),
                    savedLot.getId(), StockMovementType.ADJUSTMENT_IN, c.delta(), cost,
                    StockReferenceType.ADJUSTMENT, c.referenceId(), c.reason(), c.createdBy()));
            balance.add(c.delta());
        } else {
            int out = -c.delta();
            // Los ajustes SÍ pueden dejar negativo (corrección de conteo), sin chequeo de
            // política.
            consumeFefo(c.companyId(), c.branchId(), c.productId(), out,
                    StockMovementType.ADJUSTMENT_OUT, StockReferenceType.ADJUSTMENT,
                    c.referenceId(), c.reason(), c.createdBy());
    ...
    public void transfer(TransferStockCommand c) {
        ...
        // Bloquea ambos saldos SIEMPRE en el mismo orden (por id de sede) para evitar
        // deadlocks entre
        ...
        if (fromBal.getQuantity() < c.quantity()) {
            ...
            throw new InsufficientStockException("Stock insuficiente para transferir el producto "
                    + c.productId() + " desde la sede " + c.fromBranchId() + " (disponible "
                    + fromBal.getQuantity() + ", requerido " + c.quantity() + ")");
        }
        ...
        for (StockLot lot : lots) {
            ...
            movementRepository.save(StockMovement.of(c.companyId(), c.fromBranchId(), c.productId(),
                    lot.getId(), StockMovementType.TRANSFER_OUT, take, lot.getUnitCost(),
                    StockReferenceType.TRANSFER, null, c.reason(), c.createdBy()));
            // Réplica del lote en destino: misma identidad (lote/vencimiento/costo) para
            // preservar FEFO y
            // COGS.
            StockLot destLot = lotRepository
                    .findByIdentity(c.companyId(), c.toBranchId(), c.productId(),
            ...
            movementRepository.save(StockMovement.of(c.companyId(), c.toBranchId(), c.productId(),
                    savedDest.getId(), StockMovementType.TRANSFER_IN, take, lot.getUnitCost(),
                    StockReferenceType.TRANSFER, null, c.reason(), c.createdBy()));
            remaining -= take;
        }
        if (remaining > 0) {
            inventoryMetrics.movement(StockMovementType.TRANSFER_OUT, Result.INSUFFICIENT_STOCK, 0);
            // Saldo y lotes divergen (no debería): aborta para no crear stock negativo por
            // transferencia.
            throw new InsufficientStockException(
                    "Lotes insuficientes para completar la transferencia del producto "
                            + c.productId());
        }
    ...
    public void reverse(StockReferenceType referenceType, Long referenceId, Long createdBy) {
        ...
        boolean alreadyReversed = movements.stream()
                .anyMatch(m -> m.getType() == StockMovementType.VOID_IN
                        || m.getType() == StockMovementType.VOID_OUT);
        if (alreadyReversed)
            return;
        ...
        for (StockMovement m : movements) {
            ...
            StockBalance balance = balanceForUpdate(m.getCompanyId(), m.getBranchId(),
                    m.getProductId());
            if (m.getType().isInbound()) {
                ...
                movementRepository.save(StockMovement.of(m.getCompanyId(), m.getBranchId(),
                        m.getProductId(), lot.getId(), StockMovementType.VOID_OUT, abs,
                        m.getUnitCost(), referenceType, referenceId, "reversa", createdBy));
                ...
                balance.add(abs);
                movementRepository.save(StockMovement.of(m.getCompanyId(), m.getBranchId(),
                        m.getProductId(), lot.getId(), StockMovementType.VOID_IN, abs,
                        m.getUnitCost(), referenceType, referenceId, "reversa", createdBy));
    ...
    /**
     * Descuenta {@code quantity} de los lotes de la sede en orden FEFO, creando un
     * movimiento por lote consumido. Si los lotes no alcanzan (stock negativo ya
     * permitido por el caller), lleva el sobrante al último lote FEFO (o a un lote
     * genérico si no hay ninguno), dejándolo en negativo. No toca el saldo (lo hace
     * el caller).
     */
    private List<StockConsumptionDto> consumeFefo(Long companyId, Long branchId, Long productId,
            int quantity, StockMovementType type, StockReferenceType referenceType,
        ...
        for (StockLot lot : lots) {
            ...
            lot.consume(take);
            lotRepository.save(lot);
            movementRepository.save(StockMovement.of(companyId, branchId, productId, lot.getId(),
                    type, take, lot.getUnitCost(), referenceType, referenceId, reason, createdBy));
            consumptions.add(new StockConsumptionDto(lot.getId(), take, lot.getUnitCost()));
            remaining -= take;
        }
        if (remaining > 0) {
            StockLot lot = lots.isEmpty()
                    ? lotRepository.save(StockLot.create(companyId, branchId, productId, null, null,
                            0, BigDecimal.ZERO))
                    : lots.get(lots.size() - 1);
            lot.consume(remaining);
            lotRepository.save(lot);
            movementRepository.save(
                    StockMovement.of(companyId, branchId, productId, lot.getId(), type, remaining,
    ...
    /**
     * Saldo bloqueado (FOR UPDATE) del producto en la sede; se crea en 0 si aún no
     * existe.
     */
    private StockBalance balanceForUpdate(Long companyId, Long branchId, Long productId) {
        return balanceRepository.findForUpdate(productId, branchId)
                .orElseGet(() -> balanceRepository
                        .save(StockBalance.create(companyId, branchId, productId, 0)));
    }
}

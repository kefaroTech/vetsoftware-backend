...
/**
 * Construye los modelos de reporte de inventario. El kardex se recorre en orden ascendente
 * arrastrando el saldo corrido (partiendo del saldo previo a {@code from} si hay filtro de fecha);
 * las compras se suman en totales. Las etiquetas de tipo/referencia se traducen a ES aquí para que
 * CSV y PDF compartan la misma nomenclatura.
 */
@Service
public class InventoryReportService implements ExportInventoryUseCase {
  ...
  public KardexReport kardexReport(SearchKardexCommand command) {
    ...
    int opening =
        command.from() == null
            ? 0
            : stockQueryPort.openingBalance(
                command.companyId(), command.productId(), command.branchId(), command.from());
    int running = opening;
    List<KardexReportLine> lines = new ArrayList<>(rows.size());
    for (KardexExportRow r : rows) {
      running += r.quantity();
      lines.add(
          new KardexReportLine(
              r.createdDate(),
              typeLabel(r.type()),
              referenceLabel(r.referenceType(), r.referenceId()),
              r.lotId(),
              r.quantity(),
              r.unitCost(),
              running));
  ...
  public PurchasesReport purchasesReport(SearchPurchasesQuery query) {
    ...
    BigDecimal totalValue =
        lines.stream().map(PurchaseView::total).reduce(BigDecimal.ZERO, BigDecimal::add);
    String branchName =
        query.branchId() == null
            ? "Todas las sedes"

...
/**
 * Salida de una sesión de conteo. {@code lines} viene poblada en el detalle y al confirmar (para
 * ver las diferencias aplicadas); en el listado va vacía y solo se usan {@code totalLines}/{@code
...
public record InventoryCountView(
  ...
  public static InventoryCountView summary(
      Long id,
      Long branchId,
      String note,
      Long countedBy,
      ...
      int adjustedLines) {
    return new InventoryCountView(
        id, branchId, note, countedBy, createdDate, totalLines, adjustedLines, List.of());
  }
}

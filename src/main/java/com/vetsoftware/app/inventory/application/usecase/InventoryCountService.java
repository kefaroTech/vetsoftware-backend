...
/**
 * Conteo físico (cíclico) de inventario. Al confirmar una sesión: lee el saldo de sistema de cada
 * producto en la sede, arma las líneas con su diferencia (contado − sistema), persiste la sesión y
 * genera un ajuste por cada línea con diferencia ≠ 0 delegando en el {@link StockLedgerUseCase}
 * (ADJUSTMENT_IN si sobra, ADJUSTMENT_OUT si falta), referenciando la sesión ({@code reference_id})
 * para trazabilidad en el kardex. Todo en una sola transacción.
 */
@Service
public class InventoryCountService
    implements RecordCountUseCase, ListCountsUseCase, GetCountUseCase {
  ...
  public InventoryCountView record(RecordCountCommand command) {
    ...
    // Saldo de sistema por línea ANTES de conciliar. Las validaciones (líneas vacías, producto
    // repetido,
            ...
            .map(
                l ->
                    InventoryCountLine.create(
                        l.productId(),
                        systemQuantity(l.productId(), command.branchId()),
                        l.countedQuantity()))
            .toList();
    InventoryCount count =
        InventoryCount.create(
            command.companyId(), command.branchId(), command.note(), command.countedBy(), lines);
    InventoryCount saved = countRepository.save(count);

    // Un ajuste por cada línea con diferencia; referencia = id de la sesión para trazar en el
    // kardex.

...
/**
 * Una línea de un conteo físico: cuánto había en el sistema para un producto ({@code systemQuantity}) frente a
 * cuánto se contó físicamente ({@code countedQuantity}) en la sede de la sesión. La {@code difference} (conteo −
 * sistema) es el ajuste que hay que aplicar: positiva ⇒ sobrante (ADJUSTMENT_IN), negativa ⇒ faltante
 * (ADJUSTMENT_OUT), cero ⇒ ya cuadra.
 */
public class InventoryCountLine {
    ...
    public InventoryCountLine(Long id, Long productId, int systemQuantity, int countedQuantity) {
        ...
        if (countedQuantity < 0) throw new IllegalArgumentException("countedQuantity cannot be negative");
        this.id = id;

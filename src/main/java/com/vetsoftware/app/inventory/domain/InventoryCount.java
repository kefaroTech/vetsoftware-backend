...
/**
 * Sesión de conteo físico (cíclico) de inventario en una sede. Agrupa las líneas contadas por producto; al
 * confirmarse, cada línea con {@link InventoryCountLine#difference()} distinta de cero genera un ajuste en el ledger.
 * Es el rastro de auditoría "qué se contó, quién y cuándo"; el efecto sobre el stock vive en el kardex (append-only).
 */
public class InventoryCount {
    ...
    public InventoryCount(Long id, Long companyId, Long branchId, String note, Long countedBy,
        ...
        for (InventoryCountLine line : lines) {
            if (!seen.add(line.getProductId()))
                throw new IllegalArgumentException("producto repetido en el conteo: " + line.getProductId());
        }
        this.id = id;
    ...
    public static InventoryCount create(Long companyId, Long branchId, String note, Long countedBy,
        ...
        return new InventoryCount(null, companyId, branchId, note, countedBy, LocalDateTime.now(), true, lines);
    }

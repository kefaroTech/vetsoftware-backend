...
/**
 * Serializa los reportes de inventario a CSV (UTF-8 con BOM para que Excel respete los acentos).
 */
final class InventoryCsv {
  ...
  static byte[] kardex(KardexReport r) {
    ...
    // Saldo inicial como primera fila de contexto (útil cuando hay rango de fechas).
    row(sb, "", "Saldo inicial", "", "", "", "", String.valueOf(r.openingBalance()));
    for (KardexReportLine l : r.lines()) {
      row(
          sb,
          DT.format(l.createdDate()),
          l.typeLabel(),
          l.referenceLabel(),
  ...
  static byte[] purchases(PurchasesReport r) {
    ...
    for (PurchaseView p : r.lines()) {
      row(
          sb,
          DT.format(p.createdDate()),
          p.productName(),
          p.productCode(),
          p.branchName(),
          p.lotId() == null ? "" : String.valueOf(p.lotId()),
          String.valueOf(Math.abs(p.quantity())),
          plain(p.unitCost()),
          plain(p.total()));
    }
    row(sb, "", "TOTAL", "", "", "", String.valueOf(r.totalQuantity()), "", plain(r.totalValue()));
    return sb.toString().getBytes(StandardCharsets.UTF_8);
  }

...
public interface StockLotJpaRepository extends JpaRepository<StockLotJpaEntity, Long> {
    ...
    @Query("""
            SELECT l
            FROM StockLotJpaEntity l
            WHERE l.productId = :productId
              AND l.branchId = :branchId
              AND l.enabled = true
              AND l.quantityAvailable > 0
            ORDER BY CASE WHEN l.expireDate IS NULL THEN 1 ELSE 0 END ASC, l.expireDate ASC,
                     l.id ASC
            """)
    ...
    // vencido.
    @Query(value = """
            SELECT l.id AS lotId, l.product_id AS productId, p.name AS productName,
                   p.code AS productCode, l.branch_id AS branchId, b.name AS branchName,
                   l.lot_number AS lotNumber, l.expire_date AS expireDate,
                   l.quantity_available AS quantityAvailable,
                   DATEDIFF(l.expire_date, CURRENT_DATE) AS daysToExpire
            FROM stock_lot l
    ...
    @Query(value = """
            SELECT l.product_id AS productId, p.name AS productName, p.code AS productCode,
                   SUM(l.quantity_available) AS quantity,
                   SUM(l.quantity_available * l.unit_cost) AS value
            FROM stock_lot l
            JOIN products p ON p.id = l.product_id
            WHERE l.company_id = :companyId
              AND l.enabled = true
    ...
    @Query("""
            ...
            WHERE l.enabled = true
              AND l.quantityAvailable > 0
              AND l.expireDate IS NOT NULL
              AND l.expireDate < :before
            """)
    ...
    @Query("""
            ...
            WHERE l.enabled = true
              AND l.quantityAvailable > 0
              AND l.expireDate > :fromExclusive
              AND l.expireDate <= :to
            """)

...
public interface StockLotJpaRepository extends JpaRepository<StockLotJpaEntity, Long> {
  ...
  // FEFO: vencimiento más próximo primero, nulls al final, luego por id. Solo lotes vigentes con
  // existencia.
  @Query(
      "SELECT l FROM StockLotJpaEntity l WHERE l.productId = :productId AND l.branchId = :branchId"
  ...
  List<StockLotJpaEntity> findAvailableFefo(
  ...
  @Query(
      "SELECT l FROM StockLotJpaEntity l WHERE l.companyId = :companyId AND l.branchId = :branchId "
          ...
          + "ORDER BY l.id ASC")
  List<StockLotJpaEntity> findByIdentity(
  ...
  // Vista de lotes con existencia (FEFO) para UI/trazabilidad. branchId null = todas las sedes de
  // la empresa.
  @Query(
      "SELECT l FROM StockLotJpaEntity l WHERE l.companyId = :companyId AND l.productId ="
  ...
  List<StockLotJpaEntity> findLotsForView(
      @Param("companyId") Long companyId,
      @Param("productId") Long productId,
      @Param("branchId") Long branchId);

  // Lotes por vencer (o vencidos) con existencia, hasta la fecha límite. daysToExpire negativo = ya
  // vencido.
  @Query(
      value =
          "SELECT l.id AS lotId, l.product_id AS productId, p.name AS productName, p.code AS"
              ...
              + " l.expire_date ASC",
      nativeQuery = true)
  List<ExpiringLotRow> expiringLots(
      @Param("companyId") Long companyId,
      @Param("branchId") Long branchId,
  ...
  @Query(
      value =
          "SELECT l.product_id AS productId, p.name AS productName, p.code AS productCode,"
              ...
              + " <> 0 ORDER BY p.name ASC",
      nativeQuery = true)
  List<ValuationRow> valuationRows(
      @Param("companyId") Long companyId, @Param("branchId") Long branchId);

  @Query(
      "SELECT COUNT(l) FROM StockLotJpaEntity l "
  ...
  long countExpiringAfterUntil(
      @Param("fromExclusive") LocalDate fromExclusive, @Param("to") LocalDate to);
}

...
/**
 * Inventario por sede (F3). Lecturas con {@code inventory.read}; ajustes/entradas/mínimo con {@code
 * inventory.adjust}; transferencias con {@code inventory.transfer} (gate en los @PreAuthorize de
 * los puertos). La sede se resuelve por el alcance del empleado con {@link
 * Authz#resolveAccessibleBranch}: mostrador ve/opera su(s) sede(s), admin todas.
 */
@RestController
...
public class InventoryController {
  ...
  @GetMapping("/stock")
  public PageResponse<StockView> stock(
      @RequestParam(required = false) Long branchId,
      @RequestParam(required = false) String q,
      @RequestParam(defaultValue = "false") boolean lowStock,
      @RequestParam(defaultValue = "0") int page,
      @RequestParam(defaultValue = "20") int pageSize) {
    PageResult<StockView> result =
        listStockUseCase.search(
            new SearchStockCommand(
  ...
  public List<StockLotView> lots(
      @PathVariable Long productId, @RequestParam(required = false) Long branchId) {
    return listLotsUseCase.listLots(
        new ListLotsCommand(
            authz.currentCompanyId(), authz.resolveAccessibleBranch(branchId), productId));
  }

  @GetMapping("/products/{productId}/kardex")
  public PageResponse<StockMovementView> kardex(
      ...
      @RequestParam(defaultValue = "0") int page,
      @RequestParam(defaultValue = "20") int pageSize) {
    PageResult<StockMovementView> result =
        listKardexUseCase.searchKardex(
            new SearchKardexCommand(
                authz.currentCompanyId(),
                authz.resolveAccessibleBranch(branchId),
                productId,
                from,
  ...
  @GetMapping("/alerts")
  public InventoryAlertsView alerts(
      @RequestParam(required = false) Long branchId,
      @RequestParam(defaultValue = "30") int expiringInDays) {
    return alertsUseCase.alerts(
        new InventoryAlertsQuery(
  ...
  public InventoryValuationView valuation(@RequestParam(required = false) Long branchId) {
    return valuationUseCase.valuation(
        new InventoryValuationQuery(
            authz.currentCompanyId(), authz.resolveAccessibleBranch(branchId)));
  }

  @GetMapping("/purchases")
  public PageResponse<PurchaseView> purchases(
      @RequestParam(required = false) Long branchId,
      @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
      @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to,
      @RequestParam(defaultValue = "0") int page,
      @RequestParam(defaultValue = "20") int pageSize) {
    PageResult<PurchaseView> result =
        listPurchasesUseCase.purchases(
  ...
  public void consume(@Valid @RequestBody ConsumeStockRequest request) {
    consumeStockUseCase.consume(
        new RecordClinicalUseCommand(
            authz.currentCompanyId(),
  ...
  public void transfer(@Valid @RequestBody TransferStockRequest request) {
    transferStockUseCase.transfer(
        new TransferStockCommand(
            authz.currentCompanyId(),
            // Origen: acotado al alcance del empleado. Destino: cualquier sede de la empresa
            // (validada en el service).
            authz.resolveAccessibleBranch(request.fromBranchId()),
            request.toBranchId(),
            request.productId(),
            request.quantity(),
            request.reason(),
  ...
  public void setMinStock(
      @PathVariable Long productId, @Valid @RequestBody SetMinStockRequest request) {
    setMinStockUseCase.setMinStock(
        new SetMinStockCommand(
            authz.currentCompanyId(),
            authz.resolveAccessibleBranch(request.branchId()),
            productId,
            request.minStock()));
  ...
  // Conteo físico (cíclico): confirma la hoja de conteo y devuelve la sesión con las diferencias
  // aplicadas.
  @PostMapping("/counts")
  @ResponseStatus(HttpStatus.CREATED)
  public InventoryCountView recordCount(@Valid @RequestBody RecordCountRequest request) {
    return recordCountUseCase.record(
        new RecordCountCommand(
            authz.currentCompanyId(),
            authz.resolveAccessibleBranch(request.branchId()),
            request.note(),
            authz.currentEmployeeIdOrNull(),
  ...
  @GetMapping("/counts")
  public PageResponse<InventoryCountView> counts(
      @RequestParam(required = false) Long branchId,
      @RequestParam(defaultValue = "0") int page,
      @RequestParam(defaultValue = "20") int pageSize) {
    PageResult<InventoryCountView> result =
        listCountsUseCase.list(
            new SearchCountsQuery(
  ...
  public ResponseEntity<byte[]> exportKardex(
    ...
    KardexReport report =
        exportInventoryUseCase.kardexReport(
            new SearchKardexCommand(
                authz.currentCompanyId(),
                authz.resolveAccessibleBranch(branchId),
                productId,
  ...
  public ResponseEntity<byte[]> exportPurchases(
    ...
    PurchasesReport report =
        exportInventoryUseCase.purchasesReport(
            new SearchPurchasesQuery(
  ...
  private static <T> PageResponse<T> toPageResponse(PageResult<T> r) {
    return new PageResponse<>(
        r.content(), r.page(), r.pageSize(), r.totalElements(), r.totalPages());
  }
}

...
/**
 * Inventario por sede (F3). Lecturas con {@code inventory.read}; ajustes/entradas/mínimo con {@code inventory.adjust};
 * transferencias con {@code inventory.transfer} (gate en los @PreAuthorize de los puertos). La sede se resuelve por el
 * alcance del empleado con {@link Authz#resolveAccessibleBranch}: mostrador ve/opera su(s) sede(s), admin todas.
 */
@RestController
...
public class InventoryController {
    ...
    public PageResponse<StockView> stock(@RequestParam(required = false) Long branchId,
        ...
        PageResult<StockView> result = listStockUseCase.search(new SearchStockCommand(
            authz.currentCompanyId(), authz.resolveAccessibleBranch(branchId), q, lowStock, page, pageSize));
    ...
    public List<StockLotView> lots(@PathVariable Long productId,
        ...
        return listLotsUseCase.listLots(new ListLotsCommand(
            authz.currentCompanyId(), authz.resolveAccessibleBranch(branchId), productId));
    }
    ...
    public PageResponse<StockMovementView> kardex(@PathVariable Long productId,
                                                  ...
                                                  @RequestParam(defaultValue = "0") int page,
                                                  @RequestParam(defaultValue = "20") int pageSize) {
        PageResult<StockMovementView> result = listKardexUseCase.searchKardex(new SearchKardexCommand(
            authz.currentCompanyId(), authz.resolveAccessibleBranch(branchId), productId, from, to, page, pageSize));
        return toPageResponse(result);
    }
    ...
    public InventoryAlertsView alerts(@RequestParam(required = false) Long branchId,
        ...
        return alertsUseCase.alerts(new InventoryAlertsQuery(
            authz.currentCompanyId(), authz.resolveAccessibleBranch(branchId), expiringInDays));
    }

    @GetMapping("/valuation")
    public InventoryValuationView valuation(@RequestParam(required = false) Long branchId) {
        return valuationUseCase.valuation(new InventoryValuationQuery(
            authz.currentCompanyId(), authz.resolveAccessibleBranch(branchId)));
    }
    ...
    public PageResponse<PurchaseView> purchases(@RequestParam(required = false) Long branchId,
                                                ...
                                                @RequestParam(defaultValue = "0") int page,
                                                @RequestParam(defaultValue = "20") int pageSize) {
        PageResult<PurchaseView> result = listPurchasesUseCase.purchases(new SearchPurchasesQuery(
    ...
    public void consume(@Valid @RequestBody ConsumeStockRequest request) {
        consumeStockUseCase.consume(new RecordClinicalUseCommand(
            authz.currentCompanyId(), authz.resolveAccessibleBranch(request.branchId()), request.productId(),
    ...
    public void adjust(@Valid @RequestBody AdjustStockRequest request) {
        adjustStockUseCase.adjust(new RecordAdjustmentCommand(
            authz.currentCompanyId(), authz.resolveAccessibleBranch(request.branchId()), request.productId(),
            request.delta(), request.unitCost(), request.reason(), null, authz.currentEmployeeIdOrNull()));
    }

    @PostMapping("/receipts")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void receive(@Valid @RequestBody ReceiveStockRequest request) {
        receiveStockUseCase.receive(new RecordPurchaseCommand(
            authz.currentCompanyId(), authz.resolveAccessibleBranch(request.branchId()), request.productId(),
    ...
    public void transfer(@Valid @RequestBody TransferStockRequest request) {
        transferStockUseCase.transfer(new TransferStockCommand(
            authz.currentCompanyId(),
            // Origen: acotado al alcance del empleado. Destino: cualquier sede de la empresa (validada en el service).
            authz.resolveAccessibleBranch(request.fromBranchId()), request.toBranchId(), request.productId(),
            request.quantity(), request.reason(), authz.currentEmployeeIdOrNull()));
    }

    @PutMapping("/products/{productId}/min-stock")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void setMinStock(@PathVariable Long productId, @Valid @RequestBody SetMinStockRequest request) {
    ...
    private static <T> PageResponse<T> toPageResponse(PageResult<T> r) {
        return new PageResponse<>(r.content(), r.page(), r.pageSize(), r.totalElements(), r.totalPages());
    }
}

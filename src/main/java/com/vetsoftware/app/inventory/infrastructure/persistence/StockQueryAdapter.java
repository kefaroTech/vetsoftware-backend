...
public class StockQueryAdapter implements StockQueryPort {
  ...
  public StockQueryAdapter(
      StockBalanceJpaRepository balanceRepository,
  ...
  public PageResult<StockView> searchStock(SearchStockCommand c) {
    ...
    PageRequest pageRequest =
        PageRequest.of(Math.max(c.page(), 0), c.pageSize() <= 0 ? 20 : c.pageSize());
    Page<StockRow> page =
        balanceRepository.searchStock(c.companyId(), c.branchId(), q, c.lowStock(), pageRequest);
    List<StockView> content =
        page.getContent().stream().map(StockQueryAdapter::toStockView).toList();
  ...
  public PageResult<StockMovementView> searchKardex(SearchKardexCommand c) {
    ...
    PageRequest pageRequest =
        PageRequest.of(Math.max(c.page(), 0), c.pageSize() <= 0 ? 20 : c.pageSize());
    Page<StockMovementJpaEntity> page =
        movementRepository.searchKardex(
            c.companyId(), c.productId(), c.branchId(), from, to, pageRequest);
    List<StockMovementView> content =
  ...
  public InventoryAlertsView alerts(Long companyId, Long branchId, int expiringInDays) {
    List<StockView> lowStock =
        balanceRepository.lowStockRows(companyId, branchId).stream()
            .map(StockQueryAdapter::toStockView)
            .toList();
    LocalDate limit = LocalDate.now().plusDays(Math.max(0, expiringInDays));
    List<ExpiringLotView> expiring =
        lotRepository.expiringLots(companyId, branchId, limit).stream()
            .map(
                r ->
                    new ExpiringLotView(
                        r.getProductId(),
  ...
  public InventoryValuationView valuation(Long companyId, Long branchId) {
    List<ProductValuationView> byProduct =
        lotRepository.valuationRows(companyId, branchId).stream()
            .map(
                r ->
                    new ProductValuationView(
                        r.getProductId(),
                        r.getProductName(),
                        r.getProductCode(),
    ...
    BigDecimal total =
        byProduct.stream()
            .map(ProductValuationView::value)
  ...
  public PageResult<PurchaseView> purchases(SearchPurchasesQuery c) {
    ...
    Page<PurchaseRow> page =
        movementRepository.searchPurchases(c.companyId(), c.branchId(), from, to, pageRequest);
    List<PurchaseView> content =
        page.getContent().stream()
            .map(
                r -> {
  ...
  public List<KardexExportRow> kardexForExport(SearchKardexCommand c) {
    ...
    return movementRepository
        .kardexForExport(c.companyId(), c.productId(), c.branchId(), from, to)
        .stream()
        .map(
            r ->
                new KardexExportRow(
  ...
  public int openingBalance(Long companyId, Long productId, Long branchId, LocalDate from) {
    return movementRepository.sumQuantityBefore(
        companyId, productId, branchId, from.atStartOfDay());
  }

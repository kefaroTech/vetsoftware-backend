...
public interface StockMovementJpaRepository extends JpaRepository<StockMovementJpaEntity, Long> {
    ...
    @Query(value = """
            SELECT m.id AS id, m.product_id AS productId, p.name AS productName,
                   p.code AS productCode, m.lot_id AS lotId, m.branch_id AS branchId,
                   b.name AS branchName, m.quantity AS quantity, m.unit_cost AS unitCost,
                   m.reference_id AS referenceId, m.created_date AS createdDate
            ...
            WHERE m.company_id = :companyId
              ...
              AND (:from IS NULL OR m.created_date >= :from)
              AND (:to IS NULL OR m.created_date < :to)
    ...
    @Query(value = """
            SELECT p.name AS productName, p.code AS productCode, b.name AS branchName,
                   m.created_date AS createdDate, m.type AS type,
                   m.reference_type AS referenceType, m.reference_id AS referenceId,
                   m.lot_id AS lotId, m.quantity AS quantity, m.unit_cost AS unitCost
            FROM stock_movement m
            JOIN products p ON p.id = m.product_id
            JOIN branches b ON b.id = m.branch_id
            WHERE m.company_id = :companyId

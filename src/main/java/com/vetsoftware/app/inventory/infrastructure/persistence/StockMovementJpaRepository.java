...
public interface StockMovementJpaRepository extends JpaRepository<StockMovementJpaEntity, Long> {
  ...
  List<StockMovementJpaEntity> findByReferenceTypeAndReferenceId(
      String referenceType, Long referenceId);

  // Kardex paginado de un producto. branchId null = todas las sedes; from/to (inclusive/exclusivo)
  // opcionales.
  @Query(
      "SELECT m FROM StockMovementJpaEntity m WHERE m.companyId = :companyId AND m.productId ="
  ...
  Page<StockMovementJpaEntity> searchKardex(
  ...
  // Libro de compras: movimientos de entrada real (type=PURCHASE) con nombre de producto/sede,
  // paginado.
  @Query(
      value =
          "SELECT m.id AS id, m.product_id AS productId, p.name AS productName, p.code AS"
              ...
              + " m.created_date < :to) ORDER BY m.created_date DESC, m.id DESC",
      countQuery =
          "SELECT COUNT(*) FROM stock_movement m WHERE m.company_id = :companyId AND m.type ="
              + " 'PURCHASE' AND (:branchId IS NULL OR m.branch_id = :branchId) AND (:from IS NULL"
              + " OR m.created_date >= :from) AND (:to IS NULL OR m.created_date < :to)",
      nativeQuery = true)
  Page<PurchaseRow> searchPurchases(
      @Param("companyId") Long companyId,
      @Param("branchId") Long branchId,
      @Param("from") LocalDateTime from,
      @Param("to") LocalDateTime to,
  ...
  // Kardex completo con nombres de producto/sede, orden ASCENDENTE (para arrastrar el saldo
  // corrido).
  @Query(
      value =
          "SELECT p.name AS productName, p.code AS productCode, b.name AS branchName,"
              ...
              + " :branchId) AND (:from IS NULL OR m.created_date >= :from) AND (:to IS NULL OR"
              + " m.created_date < :to) ORDER BY m.created_date ASC, m.id ASC",
      nativeQuery = true)
  List<KardexRow> kardexForExport(
      @Param("companyId") Long companyId,
      @Param("productId") Long productId,
      @Param("branchId") Long branchId,
      @Param("from") LocalDateTime from,
  ...
  // Saldo (suma con signo) del producto ANTES de `before`, para el saldo inicial del rango
  // exportado.
  @Query(
      "SELECT COALESCE(SUM(m.quantity), 0) FROM StockMovementJpaEntity m WHERE m.companyId ="
          + " :companyId AND m.productId = :productId AND (:branchId IS NULL OR m.branchId ="
          + " :branchId) AND m.createdDate < :before")
  int sumQuantityBefore(
      @Param("companyId") Long companyId,
      @Param("productId") Long productId,
      @Param("branchId") Long branchId,
  ...
  @Query(
      value =
          "SELECT m.id AS id, m.product_id AS productId, p.name AS productName, p.code AS"
              ...
              + " m.created_date < :to) ORDER BY m.created_date ASC, m.id ASC",
      nativeQuery = true)
  List<PurchaseRow> purchasesForExport(
      @Param("companyId") Long companyId,
      @Param("branchId") Long branchId,
      @Param("from") LocalDateTime from,
      @Param("to") LocalDateTime to);

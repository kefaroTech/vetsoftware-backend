...
public class JpaInventoryCountRepository implements InventoryCountRepository {
  ...
  public PageResult<InventoryCountView> search(SearchCountsQuery query) {
    Page<InventoryCountJpaEntity> page =
        jpaRepository.search(
            query.companyId(), query.branchId(), PageRequest.of(query.page(), query.pageSize()));
    List<InventoryCountView> content =
        page.getContent().stream()
            .map(
                e ->
                    InventoryCountView.summary(
                        e.getId(),
                        e.getBranchId(),
                        e.getNote(),
                        e.getCountedBy(),
            ...
            .map(
                l ->
                    new InventoryCountLine(
                        l.getId(), l.getProductId(), l.getSystemQuantity(), l.getCountedQuantity()))
            .toList();

...
public interface StockBalanceJpaRepository extends JpaRepository<StockBalanceJpaEntity, Long> {
    ...
    @Query("SELECT b FROM StockBalanceJpaEntity b WHERE b.productId = :productId AND b.branchId ="
            + " :branchId")
    ...
    @Query(value = "SELECT sb.product_id AS productId, p.name AS productName, p.code AS productCode,"
            ...
            + " BY p.name ASC, b.name ASC", countQuery = "SELECT COUNT(*) FROM stock_balance sb JOIN products p ON p.id = sb.product_id WHERE"

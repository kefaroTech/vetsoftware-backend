...
public interface RolePermissionJpaRepository extends JpaRepository<RolePermissionJpaEntity, Long> {
  ...
  @org.springframework.data.jpa.repository.Query(
      value = "UPDATE role_permissions SET enabled = true WHERE id = :id",
      nativeQuery = true)
  ...
  @org.springframework.data.jpa.repository.Query(
      value =
          "UPDATE role_permissions rp JOIN roles r ON r.id = rp.role_id SET rp.enabled = true WHERE"
              + " rp.id = :id AND r.company_id = :companyId",
      nativeQuery = true)
  int reactivate(
  ...
  @org.springframework.data.jpa.repository.Query(
      value =
          "UPDATE role_permissions SET enabled = false WHERE role_id = :roleId AND enabled = true",
      nativeQuery = true)
  int disableAllByRoleId(@org.springframework.data.repository.query.Param("roleId") Long roleId);

  @org.springframework.data.jpa.repository.Query(
      value =
          "SELECT id FROM role_permissions WHERE role_id = :roleId AND permission_id ="
              + " :permissionId AND enabled = false LIMIT 1",
      nativeQuery = true)
  ...
  @org.springframework.data.jpa.repository.Query(
      value =
          "SELECT id AS id, permission_id AS permissionId FROM role_permissions WHERE role_id ="
              + " :roleId AND permission_id IN (:permissionIds) AND enabled = false",
      nativeQuery = true)
  List<DisabledRolePermissionRow> findDisabledByRoleAndPermissions(
      @org.springframework.data.repository.query.Param("roleId") Long roleId,
      @org.springframework.data.repository.query.Param("permissionIds")
          java.util.Collection<Long> permissionIds);

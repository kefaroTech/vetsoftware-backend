...
public class SyncRolePermissionsService implements SyncRolePermissionsUseCase {
  ...
  public SyncRolePermissionsService(
  ...
  public List<RolePermissionDto> execute(SyncRolePermissionsCommand command) {
    ...
    Set<Long> desired =
        command.permissionIds() == null ? new HashSet<>() : new HashSet<>(command.permissionIds());
    ...
    List<Long> toRemoveRpIds =
        existingPermToRpId.entrySet().stream()
    ...
    if (!toAddPermIds.isEmpty()) {
      ...
      if (!disabled.isEmpty()) {
        List<Long> idsToReactivate =
            disabled.stream().map(DisabledRolePermissionLookup::id).toList();
      ...
      if (!toCreatePermIds.isEmpty()) {
        List<RolePermission> nuevos =
            toCreatePermIds.stream()
                .map(
                    pid -> {
    ...
    List<RolePermissionDto> result =
        repository.findAllByRoleId(command.roleId()).stream().map(RolePermissionDto::from).toList();

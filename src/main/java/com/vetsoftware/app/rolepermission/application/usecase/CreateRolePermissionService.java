  ...
  public RolePermissionDto execute(CreateRolePermissionCommand command) {
    ...
    PermissionRef permission =
        (command.companyId() == null
                ...
                : permissionQueryPort.findByIdAndCompanyId(
                    command.permissionId(), command.companyId()))
            .orElseThrow(
                () ->
    ...
    Optional<Long> disabledId =
        repository.findDisabledIdByRoleAndPermission(command.roleId(), command.permissionId());
    if (disabledId.isPresent()) {
      Long id = disabledId.get();
      repository.reactivate(id);
      RolePermission refreshed =

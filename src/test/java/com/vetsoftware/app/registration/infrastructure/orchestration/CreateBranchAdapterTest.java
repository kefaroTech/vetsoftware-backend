...
/**
 * Sembrado de la sede "Principal" durante el registro público. La invariante
 * "toda empresa nace con ≥1 sede" se cumple mapeando los datos de la empresa a
 * un CreateBranchCommand, y ejecutando la creación SIEMPRE dentro del contexto
 * de sistema (ROLE_SYSTEM) — porque el registro es anónimo y el use case exige
 * autoridad.
 */
...
class CreateBranchAdapterTest {
    ...
    @InjectMocks
    private CreateBranchAdapter adapter;

    @Test
    void siembra_la_principal_con_los_datos_mapeados_bajo_contexto_de_sistema() {
        // Hacemos que el runner ejecute el Runnable que se le pasa (simula el
        // envoltorio ROLE_SYSTEM).
        doAnswer(inv -> {
        ...
        ArgumentCaptor<CreateBranchCommand> captor = ArgumentCaptor
                .forClass(CreateBranchCommand.class);
    ...
    void la_creacion_ocurre_solo_dentro_del_system_auth_runner() {
        // Sin stubear el runner, el Runnable no corre ⇒ el use case NO debe invocarse
        // fuera del
        // contexto.

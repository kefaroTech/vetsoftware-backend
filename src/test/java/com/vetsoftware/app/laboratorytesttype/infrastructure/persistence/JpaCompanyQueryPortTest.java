...
class JpaCompanyQueryPortTest {
    ...
    class Busqueda {
        ...
        void mapea_la_entidad_encontrada_a_su_companion_vo() {
            when(companyEntity.getId()).thenReturn(LaboratoryTestTypeMother.CLINICA.id());
            when(companyEntity.getName()).thenReturn(LaboratoryTestTypeMother.CLINICA.name());
            when(companyEntity.getIdentifier())
                    .thenReturn(LaboratoryTestTypeMother.CLINICA.identifier());
            when(companyJpaRepository.findById(LaboratoryTestTypeMother.COMPANY_ID))
                    .thenReturn(Optional.of(companyEntity));

            Optional<CompanyRef> resultado = port.findById(LaboratoryTestTypeMother.COMPANY_ID);

            assertThat(resultado).contains(LaboratoryTestTypeMother.CLINICA);
        }

 ...
 * <b>ciega la auditoría en silencio</b> — los eventos siguen llegando a Loki, pero con {@code ***}
 * en los campos con los que se investiga un incidente.
 *
 * <p>Ejercita cada evento de {@link AuditLogger} y el contexto de request que lo acompaña, y afirma
 * que ningún campo que la auditoría emite a propósito sale enmascarado. Añadir un {@code
 * addKeyValue(...)} nuevo sin declararlo en {@link LogFieldPolicy} rompe esta prueba.
 *
...
class AuditFieldsSurviveRedactionTest {
          ...
          .allSatisfy(
              pair ->
                  assertThat(pair.value)
                      .as(
                          "el campo de auditoría '%s' del evento '%s' quedó enmascarado; "
                              + "declararlo en LogFieldPolicy",
                          pair.key, event)
                      .isNotEqualTo(LogRedactor.MASK));
    }
    assertThat(emitted.getMDCPropertyMap())
        .as(
            "un campo del MDC del evento '%s' quedó enmascarado; declararlo en " + "LogFieldPolicy",
            event)
  ...
  void everyAuditFieldIsAllowlisted() {
    assertNothingMasked(
        "http_mutation", audit -> audit.mutation("PATCH", "/api/v1/animals/9", 200, "SUCCESS", 37));
    assertNothingMasked(
        "company_registered",
    ...
    assertNothingMasked(
        "employee_invitation_resent", audit -> audit.employeeInvitationResent(88L, "EMP0042", 3L));
    ...
    assertNothingMasked(
        "login_failure", audit -> audit.loginFailure("/auth/login/employee", "bad_credentials"));
    assertNothingMasked(
        "login_blocked_email_not_verified", audit -> audit.loginBlockedEmailNotVerified("EMP0042"));
    assertNothingMasked("access_denied", audit -> audit.accessDenied("DELETE", "/api/v1/owners/5"));
    assertNothingMasked(
        "unauthenticated",

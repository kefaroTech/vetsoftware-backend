...
class AuditFilterTest {
    ...
    void derivesTheOutcomeFromTheResponseStatus() throws Exception {
        ...
        verify(auditLogger).mutation(eq("POST"), eq("/api/v1/animals"), eq(201), eq("SUCCESS"),
                anyLong());
        verify(auditLogger).mutation(eq("POST"), eq("/api/v1/animals"), eq(400), eq("REJECTED"),
                anyLong());
        verify(auditLogger).mutation(eq("POST"), eq("/api/v1/animals"), eq(401), eq("DENIED"),
                anyLong());
        verify(auditLogger).mutation(eq("POST"), eq("/api/v1/animals"), eq(403), eq("DENIED"),
                anyLong());
        verify(auditLogger).mutation(eq("POST"), eq("/api/v1/animals"), eq(500), eq("ERROR"),
                anyLong());
    }
    ...
    void skipsInfrastructureEndpoints() throws Exception {
        for (String path : List.of("/actuator/health", "/swagger-ui/index.html", "/v3/api-docs")) {
            filter.doFilter(new MockHttpServletRequest("POST", path), new MockHttpServletResponse(),
                    PASS_THROUGH);
        }

...
class TokenCleanupMetricsTest {
  ...
  void exposesRowsPurgedTotalsAndConfiguredGrowthThreshold() {
    ...
    assertThat(
            registry
                .get(TokenCleanupMetrics.ROWS_METRIC)
                .tag("token.type", "refresh")
                .gauge()
                .value())
        .isEqualTo(10);
    assertThat(
            registry
                .get(TokenCleanupMetrics.ROWS_METRIC)
                .tag("token.type", "email_verification")
                .gauge()
                .value())
        .isEqualTo(20);
    assertThat(
            registry
                .get(TokenCleanupMetrics.PURGED_METRIC)
                .tag("token.type", "password_reset")
                .counter()
                .count())
        .isEqualTo(3);

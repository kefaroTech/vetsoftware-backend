...
class RemoteConnectionValidatorTest {
  ...
  void rejectsLocalhostInDevOrProd() {
    ...
    MockEnvironment environment =
        validRemoteEnvironment().withProperty("spring.data.redis.url", "redis://localhost:6379");
  ...
  void requiresOtlpAuthenticationHeaders() {
    RemoteConnectionValidator validator = validatorWithValidRemoteEnvironment();
    MockEnvironment environment =
        validRemoteEnvironment().withProperty("OTEL_EXPORTER_OTLP_HEADERS", "");
    validator.setEnvironment(environment);
  ...
  private static MockEnvironment validRemoteEnvironment() {
    return new MockEnvironment()
        .withProperty("spring.datasource.url", "jdbc:mysql://db.dev.example.com:3306/vetsoftware")
        .withProperty("spring.data.redis.url", "rediss://redis.dev.example.com:6380")
        .withProperty(
            "management.otlp.metrics.export.url", "https://otlp.example.com/otlp/v1/metrics")
        .withProperty(
        ...
        .withProperty("vetsoftware.code-recovery.login-url", "https://app.dev.example.com/login")
        .withProperty("vetsoftware.employee.login-url", "https://app.dev.example.com/login")
        .withProperty(
            "cors.allowed-origins", "https://app.dev.example.com,https://admin.dev.example.com")

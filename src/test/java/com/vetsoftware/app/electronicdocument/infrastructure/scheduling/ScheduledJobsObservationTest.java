...
class ScheduledJobsObservationTest {
  ...
  void contingencyRetryReportsPartialFailureWithoutHighCardinalityTags() throws Exception {
    ...
    ContingencyRetryJob job =
        new ContingencyRetryJob(
            repository, transmitter, transmissionLog, capture.telemetry(), 12, 48);
    ...
    assertThat(context.getLowCardinalityKeyValues())
        .noneMatch(
            keyValue ->
                keyValue.getKey().contains("document")
  ...
  private static void runAsSpringScheduledTask(
    ...
    new ScheduledMethodRunnable(
            target, target.getClass().getMethod(methodName), null, () -> registry)
        .run();
  }

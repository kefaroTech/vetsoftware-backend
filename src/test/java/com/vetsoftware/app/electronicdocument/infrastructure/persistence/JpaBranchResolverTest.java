...
/**
 * Resolución de la sede emisora de una venta POS. Una venta NO se emite desde una sede fuera de
 * operación: toda rama exige ACTIVA. Cascada: branchId del request (activo y de la empresa); si no
 * viene, la "Principal" activa; si no, la primera sede activa; si no hay ninguna activa, vacío (el
 * builder lanza). Nunca cae a una sede de otra empresa ni a una inactiva.
 */
...
class JpaBranchResolverTest {
  ...
  /**
   * Entidad activa (para el camino de branchId explícito, que filtra por isActive() y luego lee el
   * id).
   */
  ...
  void rechaza_la_sede_solicitada_INACTIVA_devolviendo_vacio() {
    ...
    assertThat(resolver.resolve(COMPANY, 11L))
        .as("no se emite POS desde una sede desactivada")
        .isEmpty();
  }
  ...
  void sin_branchId_cae_a_la_principal_activa() {
    ...
    when(branchJpaRepository.findFirstByCompany_IdAndCodeIgnoreCaseAndActiveTrue(
            COMPANY, "PRINCIPAL"))

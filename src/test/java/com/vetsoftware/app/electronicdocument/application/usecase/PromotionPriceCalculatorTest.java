...
/**
 * Precio canónico (post-promoción) de una línea de venta. Es la fuente de verdad del precio fiscal:
 * el unitPrice que manda el POS se valida contra este cálculo, así que una desviación aquí permite
 * emitir un documento con un monto que el catálogo no respalda. Debe espejar exactamente el
 * redondeo del front (peso entero, HALF_UP).
 */
class PromotionPriceCalculatorTest {
  ...
  class ConcurrenciaDePromociones {
    ...
    void el_orden_de_la_lista_no_cambia_el_resultado() {
      ...
      assertThat(
              PromotionPriceCalculator.expectedUnitPrice(
                  bd("50000"), SaleLineKind.PRODUCT, 1L, 9L, a))

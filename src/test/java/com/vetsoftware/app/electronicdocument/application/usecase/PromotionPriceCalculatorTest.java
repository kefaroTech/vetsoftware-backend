...
/**
 * Precio canónico (post-promoción) de una línea de venta. Es la fuente de verdad del precio fiscal: el
 * unitPrice que manda el POS se valida contra este cálculo, así que una desviación aquí permite emitir un
 * documento con un monto que el catálogo no respalda. Debe espejar exactamente el redondeo del front
 * (peso entero, HALF_UP).
 */
class PromotionPriceCalculatorTest {
    ...
    private static SalePromotion percentage(ApplicationType type, Long item, String percent) {
        return new SalePromotion(PromotionType.DISCOUNT, type, item, ValueType.PERCENTAGE, bd(percent));
    }
    ...
    class SinPromociones {
        ...
        void devuelve_el_precio_de_lista() {
            BigDecimal price = PromotionPriceCalculator.expectedUnitPrice(
                    bd("50000"), SaleLineKind.PRODUCT, 1L, 9L, List.of());
        ...
        void redondea_el_precio_de_lista_a_peso_entero() {
            BigDecimal price = PromotionPriceCalculator.expectedUnitPrice(
                    bd("50000.5"), SaleLineKind.PRODUCT, 1L, 9L, List.of());
    ...
    class DescuentoPorcentual {
        ...
        void aplica_el_porcentaje_sobre_el_precio_de_lista() {
            BigDecimal price = PromotionPriceCalculator.expectedUnitPrice(
                    bd("50000"), SaleLineKind.PRODUCT, 1L, 9L,
        ...
        void redondea_half_up_como_el_front() {
            ...
            BigDecimal price = PromotionPriceCalculator.expectedUnitPrice(
                    bd("33333"), SaleLineKind.PRODUCT, 1L, 9L,
    ...
    class DescuentoDeValorFijo {
        ...
        void nunca_produce_un_precio_negativo() {
            BigDecimal price = PromotionPriceCalculator.expectedUnitPrice(
                    bd("10000"), SaleLineKind.PRODUCT, 1L, 9L,
    ...
    class Coincidencia {
        ...
        void una_promo_de_producto_no_aplica_a_un_servicio_con_el_mismo_id() {
            BigDecimal price = PromotionPriceCalculator.expectedUnitPrice(
                    bd("50000"), SaleLineKind.SERVICE, 1L, 9L,
        ...
        void una_promo_de_categoria_no_aplica_si_el_item_no_tiene_categoria() {
            BigDecimal price = PromotionPriceCalculator.expectedUnitPrice(
                    bd("50000"), SaleLineKind.PRODUCT, 1L, null,
                    List.of(percentage(ApplicationType.CATEGORY, 9L, "10")));
    ...
    class ConcurrenciaDePromociones {
        ...
        void gana_siempre_la_mas_barata_para_el_cliente() {
            List<SalePromotion> promos = List.of(
                    percentage(ApplicationType.PRODUCT, 1L, "10"),      // 45.000
                    specialPrice(ApplicationType.CATEGORY, 9L, "39990"), // 39.990
                    fixed(ApplicationType.PRODUCT, 1L, "5000"));         // 45.000

            BigDecimal price = PromotionPriceCalculator.expectedUnitPrice(
                    bd("50000"), SaleLineKind.PRODUCT, 1L, 9L, promos);
        ...
        void el_orden_de_la_lista_no_cambia_el_resultado() {
            List<SalePromotion> a = List.of(
                    percentage(ApplicationType.PRODUCT, 1L, "10"),
                    specialPrice(ApplicationType.CATEGORY, 9L, "39990"));
            List<SalePromotion> b = List.of(
                    specialPrice(ApplicationType.CATEGORY, 9L, "39990"),
                    percentage(ApplicationType.PRODUCT, 1L, "10"));

            assertThat(PromotionPriceCalculator.expectedUnitPrice(bd("50000"), SaleLineKind.PRODUCT, 1L, 9L, a))
                    .isEqualByComparingTo(PromotionPriceCalculator.expectedUnitPrice(
                            bd("50000"), SaleLineKind.PRODUCT, 1L, 9L, b));
        }
    }
}

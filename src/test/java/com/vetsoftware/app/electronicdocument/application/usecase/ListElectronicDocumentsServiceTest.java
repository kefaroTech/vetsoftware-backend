...
/**
 * Listado paginado de documentos de la empresa, con filtro OPCIONAL por sede.
 * El servicio no reinventa la paginacion: proyecta el contenido y traslada los
 * metadatos tal cual, que es lo que el front usa para pintar el paginador.
 */
...
class ListElectronicDocumentsServiceTest {
    ...
    void proyecta_cada_documento_conservando_los_metadatos() {
        ...
        when(repository.findAllByCompanyId(9L, null, 1, 20))
                .thenReturn(new PageResult<>(documentos, 1, 20, 41L, 3));

        PageResult<ElectronicDocumentDto> pagina = service.listByCompany(9L, null, 1, 20);
    ...
    void traslada_el_filtro_de_sede_al_repositorio() {
        when(repository.findAllByCompanyId(9L, 7L, 0, 20))
                .thenReturn(new PageResult<>(List.of(), 0, 20, 0L, 0));

        service.listByCompany(9L, 7L, 0, 20);

        verify(repository).findAllByCompanyId(9L, 7L, 0, 20);
    }
    ...
    void una_empresa_sin_documentos_devuelve_pagina_vacia() {
        when(repository.findAllByCompanyId(9L, null, 0, 20))
                .thenReturn(new PageResult<>(List.of(), 0, 20, 0L, 0));

        assertThat(service.listByCompany(9L, null, 0, 20).content()).isEmpty();
    }
    ...
    void el_listado_no_escribe_nada() {
        ...
        service.listByCompany(9L, null, 0, 20);

        verify(repository).findAllByCompanyId(9L, null, 0, 20);

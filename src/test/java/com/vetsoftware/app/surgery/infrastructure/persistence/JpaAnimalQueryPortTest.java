...
/**
 * Adaptador de resolucion de animal para cirugias. Consulta SIEMPRE el finder
 * acotado por empresa: el puerto no ofrece variante ancha, para que una cirugia
 * no pueda colgarse del animal de otro tenant.
 */
...
class JpaAnimalQueryPortTest {
    ...
    void mapea_el_animal_encontrado_a_su_companion_vo() {
        ...
        when(animalJpaRepository.findByIdAndCompany_Id(100L, EMPRESA))
                .thenReturn(Optional.of(animal));

        Optional<AnimalRef> ref = port.findByIdAndCompanyId(100L, EMPRESA);
    ...
    void el_animal_de_otra_empresa_no_existe_para_mi() {
        when(animalJpaRepository.findByIdAndCompany_Id(100L, OTRA_EMPRESA))
                .thenReturn(Optional.empty());

        assertThat(port.findByIdAndCompanyId(100L, OTRA_EMPRESA)).isEmpty();
    }
}

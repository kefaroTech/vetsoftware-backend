...
class JpaConsultationQueryPortTest {
    ...
    void mapea_la_consulta_encontrada_a_su_companion_vo() {
        LocalDate fecha = LocalDate.of(2026, 3, 9);
        ConsultationJpaEntity consulta = consultaEncontrada(200L, fecha);
        when(consultationJpaRepository.findByIdAndCompany_Id(200L, EMPRESA))
                .thenReturn(Optional.of(consulta));

        Optional<ConsultationRef> ref = port.findByIdAndCompanyId(200L, EMPRESA);
    ...
    void la_consulta_de_otra_empresa_no_existe_para_mi() {
        when(consultationJpaRepository.findByIdAndCompany_Id(200L, OTRA_EMPRESA))
                .thenReturn(Optional.empty());

        assertThat(port.findByIdAndCompanyId(200L, OTRA_EMPRESA)).isEmpty();
    }
}

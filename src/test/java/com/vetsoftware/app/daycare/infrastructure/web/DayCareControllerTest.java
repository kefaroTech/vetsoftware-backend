...
class DayCareControllerTest {
    ...
    @Test
    @DisplayName("POST /daycares sin startDate responde 400 y no llega al caso de uso")
    void post_sin_start_date_responde_400() throws Exception {
        mockMvc.perform(post("/daycares").contentType(MediaType.APPLICATION_JSON)
                .content("{\"date\":\"2026-02-01\",\"type\":\"DAYCARE\",\"animalId\":1}"))
    ...
    @Test
    @DisplayName("PUT /daycares/{id} traduce el request al command con el companyId del contexto")
    void put_traduce_el_request_al_command() throws Exception {
        ...
        verify(updateUseCase).execute(new UpdateDayCareCommand(5L, LocalDate.of(2026, 2, 1),
                LocalDate.of(2026, 2, 1), LocalDate.of(2026, 2, 3), DayCareType.DAYCARE, "Correa",
                "Sin novedades", 1L, 10L));
    }

    @Test
    @DisplayName("DELETE /daycares/{id} responde 204 sin cuerpo")

...
class ListDayCaresByAnimalServiceTest {
    ...
    void delega_la_paginacion_y_mapea_el_contenido() {
        PageResult<DayCare> pagina = new PageResult<>(List.of(DayCareMother.guarderiaValida()), 0,
                20, 1, 1);
        when(repository.findAllByAnimalIdAndCompanyId(DayCareMother.FIRULAIS.id(),
                DayCareMother.CLINICA.id(), "correa", 0, 20)).thenReturn(pagina);

        PageResult<DayCareDto> resultado = service.listByAnimal(DayCareMother.FIRULAIS.id(),
                DayCareMother.CLINICA.id(), "correa", 0, 20);

        assertThat(resultado.content()).extracting(DayCareDto::id)
                .containsExactly(DayCareMother.guarderiaValida().getId());
        verify(repository).findAllByAnimalIdAndCompanyId(DayCareMother.FIRULAIS.id(),
                DayCareMother.CLINICA.id(), "correa", 0, 20);
    }
}

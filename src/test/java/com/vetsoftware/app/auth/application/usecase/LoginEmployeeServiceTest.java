...
class LoginEmployeeServiceTest {
    ...
    @Mock
    private EmployeeActivationPort employeeActivationPort;
    @Mock
    private AuthEmployeeRepository authEmployeeRepository;
    @Mock
    private RefreshTokenRepository refreshTokenRepository;
    @InjectMocks
    private LoginEmployeeService service;

    @Test
    void login_rota_la_version_y_revoca_la_sesion_anterior_antes_de_emitir_tokens() {
        var credentials = new EmployeeCredentialsRepository.EmployeeCredentials(7L, 3L, 4L, "hash",
                true);
        ...
        InOrder order = inOrder(authEmployeeRepository, refreshTokenRepository, tokenGenerator,
                refreshTokenIssuer);
    ...
    void un_codigo_inexistente_falla_igual_que_una_clave_incorrecta() {
        ...
        verify(passwordHasher, never()).matches(org.mockito.ArgumentMatchers.anyString(),
    ...
    void la_verificacion_del_correo_se_evalua_despues_de_la_clave() {
        // Si se evaluara antes, un atacante sabría qué cuentas existen sin conocer la
        // contraseña.
        var credentials = new EmployeeCredentialsRepository.EmployeeCredentials(7L, 3L, 4L, "hash",
                false);
        when(credentialsRepository.findByCode("EMP-1")).thenReturn(Optional.of(credentials));
    ...
    void un_empleado_desactivado_no_obtiene_sesion() {
        ...
        verify(tokenGenerator, never()).generate(org.mockito.ArgumentMatchers.anyLong(),
    ...
    void el_token_lleva_la_empresa_y_la_version_recien_rotadas_no_las_del_lookup() {
        // credentials trae authVersion 4; la rotación devuelve 5: el token debe firmar
        // la 5,

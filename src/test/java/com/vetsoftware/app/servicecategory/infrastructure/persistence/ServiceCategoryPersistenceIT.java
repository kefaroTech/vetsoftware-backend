...
/**
 * Rodaja de persistencia de categorias de servicio contra MySQL real.
 *
 * <p>
 ...
 * columna es {@code utf8mb4_..._ci}, asi que para MySQL «Consultas» y
 * «consultas» son el mismo nombre. Un {@code Map} en memoria compara con
...
class ServiceCategoryPersistenceIT extends AbstractDataJpaTest {
    ...
    private static final String CIRUGIAS = "Cirugias";
    private static final String VACUNACION = "Vacunacion";

    @Autowired
    private JpaServiceCategoryRepository repository;
    ...
    private ServiceCategory nueva(String nombre, CompanyRef empresa) {
        return new ServiceCategory(null, nombre, "Categoria de prueba", empresa, CREADO, null, null,
                null, true);
    }

    private ServiceCategory guardar(String nombre) {
        return repository.save(nueva(nombre, EMPRESA));
    }

    private ServiceCategory guardarEn(CompanyRef empresa, String nombre) {
        return repository.save(nueva(nombre, empresa));
    }
    ...
    class UnicidadDelNombre {
        ...
        void el_nombre_choca_aunque_cambie_la_caja() {
            ...
            assertThat(repository.existsByCompanyIdAndName(COMPANY_ID, "consultas")).isTrue();
        }
        ...
        void el_mismo_nombre_queda_libre_en_otra_empresa() {
            ...
            assertThat(repository.existsByCompanyIdAndName(OTRA_COMPANY_ID, CONSULTAS)).isFalse();
        }

        @Test
        @DisplayName("deshabilitar una categoria libera su nombre")
        void deshabilitar_una_categoria_libera_su_nombre() {
            ServiceCategory pausada = guardar(CONSULTAS);

            deshabilitar(pausada.getId());

            assertThat(repository.existsByCompanyIdAndName(COMPANY_ID, CONSULTAS)).isFalse();
        }

        @Test
        @DisplayName("al excluir su propio id, una categoria no choca consigo misma")
        void una_categoria_no_choca_consigo_misma() {
            ServiceCategory consultas = guardar(CONSULTAS);

            assertThat(repository.existsByCompanyIdAndNameExcludingId(COMPANY_ID, CONSULTAS,
                    consultas.getId())).isFalse();
        }
        ...
        void el_nombre_repetido_choca_al_excluir_otro_id() {
            guardar(CONSULTAS);
            ServiceCategory cirugias = guardar(CIRUGIAS);

            assertThat(repository.existsByCompanyIdAndNameExcludingId(COMPANY_ID, "CONSULTAS",
                    cirugias.getId())).isTrue();
        }
    }
    ...
    class Tenencia {
        ...
        void no_devuelve_la_categoria_de_otra_empresa() {
            ServiceCategory ajena = guardarEn(OTRA_EMPRESA, CIRUGIAS);
        ...
        void find_by_id_sin_empresa_alcanza_la_ajena() {
            ...
            assertThat(repository.findById(ajena.getId())).map(ServiceCategory::getId)
        ...
        void el_listado_por_empresa_no_mezcla_las_dos() {
            ServiceCategory propia = guardar(CIRUGIAS);
            ServiceCategory ajena = guardarEn(OTRA_EMPRESA, VACUNACION);
    ...
    class IdaYVuelta {
        ...
        void guardar_y_releer_conserva_cada_campo() {
            ServiceCategory guardada = guardar(CIRUGIAS);

            ServiceCategory leida = repository.findByIdAndCompanyId(guardada.getId(), COMPANY_ID)
                    .orElseThrow();

            assertThat(leida.getName()).isEqualTo(CIRUGIAS);

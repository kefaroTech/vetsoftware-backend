...
import static com.vetsoftware.app.procedureschedule.testsupport.ProcedureScheduleMother.sinFechaDeInicio;
import static com.vetsoftware.app.procedureschedule.testsupport.ProcedureScheduleMother.sinHoraDeInicio;
 ...
 */
@DisplayName("ProcedureScheduleGenerator — cuantas tomas y a que hora")
class ProcedureScheduleGeneratorTest {

    private static List<LocalDateTime> horasDe(List<ProcedureSchedule> tomas) {
        return tomas.stream().map(ProcedureSchedule::getCurrentDateTime).toList();
    }
    ...
    class Generacion {
        ...
        void cada_8_horas_durante_3_dias_son_nueve_tomas() {
            List<ProcedureSchedule> tomas = ProcedureScheduleGenerator
        ...
        void la_cuenta_sale_de_las_horas_partidas_por_el_intervalo(String frecuencia, int dias,
                int esperadas) {
            assertThat(ProcedureScheduleGenerator.generate(orden(frecuencia, "FIXED", "DAYS", dias),
                    EMPLEADO)).hasSize(esperadas);
        }
        ...
        void una_pauta_por_dosis_genera_exactamente_esas() {
            ...
            assertThat(ProcedureScheduleGenerator.generate(orden("EVERY_8H", "FIXED", "DOSES", 5),
                    EMPLEADO)).hasSize(5);
        }
        ...
        @Test
        @DisplayName("un procedimiento continuo no genera tomas: no hay horas que marcar")
        void una_infusion_continua_no_genera_tomas() {
            // La monitorizacion continua no se marca por horas: es permanente.
            assertThat(ProcedureScheduleGenerator.generate(orden("CONTINUOUS", "FIXED", "DAYS", 3),
                    EMPLEADO)).isEmpty();
        }
        ...
        void una_frecuencia_no_discreta_no_se_agenda(String frecuencia) {
            ...
            assertThat(ProcedureScheduleGenerator.generate(orden(frecuencia, "FIXED", "DAYS", 3),
                    EMPLEADO)).isEmpty();
        }
        ...
        void la_frecuencia_se_reconoce_sin_importar_el_formato() {
            assertThat(ProcedureScheduleGenerator
        ...
        void sin_fecha_de_inicio_no_se_agenda_nada() {
            assertThat(ProcedureScheduleGenerator.generate(sinFechaDeInicio(), EMPLEADO)).isEmpty();
        }
    ...
    class Topes {
        ...
        void sin_duracion_cae_al_horizonte() {
            assertThat(ProcedureScheduleGenerator.generate(orden("EVERY_24H", "FIXED", null, null),
                    EMPLEADO)).hasSize(14);
        }
        ...
        void una_cantidad_no_positiva_no_cuenta_como_duracion() {
            ...
            assertThat(ProcedureScheduleGenerator.generate(orden("EVERY_24H", "FIXED", "DAYS", 0),
                    EMPLEADO)).hasSize(14);
            assertThat(ProcedureScheduleGenerator.generate(orden("EVERY_24H", "FIXED", "DOSES", -3),
                    EMPLEADO)).hasSize(14);
        }
        ...
        void nunca_se_pasa_de_noventa_tomas() {
            ...
            assertThat(ProcedureScheduleGenerator.generate(orden("EVERY_4H", "FIXED", "DAYS", 30),
                    EMPLEADO)).hasSize(90);
            assertThat(ProcedureScheduleGenerator.generate(orden("EVERY_8H", "FIXED", "DOSES", 500),
                    EMPLEADO)).hasSize(90);
        }
        ...
        void una_duracion_mas_corta_que_el_intervalo_deja_una_toma() {
            ...
            assertThat(ProcedureScheduleGenerator.generate(orden("EVERY_24H", "FIXED", "DAYS", 1),
                    EMPLEADO)).hasSize(1);
        }
    }
    ...
    class Regeneracion {
        ...
        void con_dos_aplicadas_quedan_siete_pendientes() {
            List<ProcedureSchedule> pendientes = ProcedureScheduleGenerator
        ...
        void en_pauta_fija_se_conservan_las_horas_de_reloj() {
            List<ProcedureSchedule> pendientes = ProcedureScheduleGenerator.generatePending(
        ...
        void en_pauta_por_intervalo_se_recronometra_desde_la_hora_real() {
            ...
            // a las 00:00: adelantarla acortaria el intervalo y podria dañar el tejido.
        ...
        void si_ya_se_aplicaron_todas_no_queda_nada() {
            assertThat(ProcedureScheduleGenerator.generatePending(cada8hDurante3Dias(), 9, null,
                    EMPLEADO)).isEmpty();
        }
        ...
        void mas_aplicadas_que_el_total_no_genera_negativos() {
            ...
            assertThat(ProcedureScheduleGenerator.generatePending(cada8hDurante3Dias(), 20, null,
                    EMPLEADO)).isEmpty();
        }
        ...
        @Test
        @DisplayName("un procedimiento continuo tampoco genera pendientes")
        ...
        void sin_fecha_de_inicio_no_se_regenera_nada() {
            assertThat(ProcedureScheduleGenerator.generatePending(sinFechaDeInicio(), 0, null,
                    EMPLEADO)).isEmpty();
        }
        ...
        void una_frecuencia_desconocida_no_regenera_nada() {
            assertThat(ProcedureScheduleGenerator.generatePending(orden("PRN", "FIXED", "DAYS", 3),
        ...
        void las_tomas_regeneradas_nacen_pendientes() {
            ProcedureSchedule primera = ProcedureScheduleGenerator
            ...
            assertThat(primera.getHospitalizationProcedure().name())
                    .isEqualTo("Curacion de herida");
    ...
    class TablaDeFrecuencias {
        ...
        void cada_frecuencia_discreta_tiene_sus_horas(String frecuencia, int horas) {
            assertThat(ProcedureScheduleGenerator.intervalHours(frecuencia)).isEqualTo(horas);
        }
        ...
        void lo_que_no_es_discreto_devuelve_null(String frecuencia) {
            ...
            assertThat(ProcedureScheduleGenerator.intervalHours(frecuencia)).isNull();
        }

        @Test
        @DisplayName("tolera minusculas y espacios")
        void tolera_minusculas_y_espacios() {
            assertThat(ProcedureScheduleGenerator.intervalHours("  every_12h ")).isEqualTo(12);
        }
    }
}

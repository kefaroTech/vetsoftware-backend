...
/**
 * Adaptador de resolucion de hospitalizacion para las notas de evolucion.
 * <b>Con</b> filtro de empresa, y sin variante ancha: el scope del repositorio
 * de la propia nota ({@code findByIdAndHospitalization_Company_Id}) protege la
 * fila que ya existe, pero no impide colgar una nota nueva de la
 * hospitalizacion de otro tenant. Eso solo lo impide acotar aqui la resolucion
 * de la referencia.
 */
...
class JpaHospitalizationQueryPortTest {
    ...
    void mapea_la_hospitalizacion_encontrada_a_su_companion_vo() {
        ...
        when(hospitalizationJpaRepository.findByIdAndCompany_Id(55L, 9L))
                .thenReturn(Optional.of(entity));

        Optional<HospitalizationRef> ref = port.findByIdAndCompanyId(55L, 9L);

 ...
 * se propaga a la base gravable, al IVA, a las retenciones y al total transmitido a la DIAN, así
 * que cada regla se fija explícitamente (escala 2, HALF_UP) incluidos los bordes de .005 y los
 * nulos.
 */
class MoneyTest {
  ...
  class ExtractBase {
    ...
    void tarifa_negativa_no_se_ignora_y_produce_una_base_mayor() {
      // Documenta el comportamiento actual: extractBase solo cortocircuita en null/0, no en
      // negativos.
    ...
    void es_una_clase_de_utilidad_sin_estado_y_no_extensible() throws Exception {
      ...
      assertThat(
              java.lang.reflect.Modifier.isPrivate(
                  Money.class.getDeclaredConstructor().getModifiers()))
          .isTrue();

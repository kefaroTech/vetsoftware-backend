...
/**
 * Adapter de resolución de sede para citas. Aquí vive el filtro {@code isActive()} del camino
 * explícito y la cascada Principal-activa→primera-activa del camino por defecto (los service tests
 * mockean el puerto, así que NO ejercen este filtro). Verifica que una sede inactiva no se mapee y
 * que la cascada respete el orden.
 */
...
class JpaBranchQueryPortTest {
  ...
  void findActive_mapea_la_sede_cuando_esta_activa() {
    ...
    assertThat(ref).contains(new BranchRef(11L, "Sede Norte", "NORTE"));
  }
